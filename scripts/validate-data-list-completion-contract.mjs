...
export function validateDataListCompletionContract(decoded) {
  ...
  children.forEach((child, index) => {
    ...
    if (!defaultView) {
      findings.push(error("DATA_LIST_COMPLETION_DEFAULT_VIEW_REQUIRED", "A completed Type 1 Data List requires one default Type 0 data view.", `${pointer}.Layouts`));
    }
    ...
    if (!settings || typeof settings !== "object") {
      findings.push(error("DATA_LIST_COMPLETION_LAYOUTVIEW_REQUIRED", "A completed Type 1 Data List requires parseable ListModel.LayoutView routing.", `${pointer}.List.LayoutView`));
      return;
    }
    for (const route of ["add", "edit", "view"]) {
      ...
      if (!layoutId || layoutId === "default" || !formIds.has(layoutId)) {
        findings.push(error(
          ...
          `A completed Type 1 Data List requires LayoutView.${route} to resolve to a same-list Type 1 custom form.`,
