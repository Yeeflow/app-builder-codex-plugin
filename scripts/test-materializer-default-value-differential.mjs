...
const corpusPath = resolve(root, "compatibility/differential-fixtures/materializer-default-value-for-field-type.v0.1.0.json");
...
if (typeof coreModule.defaultValueForFieldType !== "function") throw new Error("MATERIALIZER_DEFAULT_VALUE_CORE_EXPORT_MISSING");
console.log("MATERIALIZER_DEFAULT_VALUE_CORE_SHADOW_IMPLEMENTED");
...
for (const fixture of corpus.cases) {
  ...
  const coreResult = invoke(coreModule.defaultValueForFieldType, fixture, coreInput);
...
console.log(`MATERIALIZER_DEFAULT_VALUE_DIFFERENTIAL_PARITY_PASSED cases=${corpus.cases.length}`);
console.log(`MATERIALIZER_DEFAULT_VALUE_IMMUTABILITY_PASSED cases=${immutableCases}`);
...
function extractLegacyFunction() {
  ...
  const declaration = ast.statements.find((statement) => ts.isFunctionDeclaration(statement) && statement.name?.text === "defaultValueForFieldType");
  if (!declaration) throw new Error("MATERIALIZER_DEFAULT_VALUE_LEGACY_FUNCTION_MISSING");
  ...
  if (digest !== corpus.legacy.functionSourceSha256) throw new Error("MATERIALIZER_DEFAULT_VALUE_LEGACY_SOURCE_MISMATCH");
