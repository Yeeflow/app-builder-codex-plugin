...
const fixture = JSON.parse(readFileSync(resolve(root, "compatibility/differential-fixtures/materializer-normalize-hex-materializer-integration.v0.1.0.json"), "utf8"));
const temporary = mkdtempSync(resolve(tmpdir(), "yeeflow-materializer-normalize-hex-routing-"));
...
try {
  ...
  assert.equal(count(materializerSource, "coreNormalizeHexColor("), fixture.expectedCallSites.postCutoverCoreCallCount, "Post-cutover Core call-site count must match the approved scope.");
  assert.equal(count(materializerSource, "normalizeHexColor("), fixture.expectedCallSites.postCutoverLegacyCallCount + 1, "Only the retained Legacy helper declaration may retain the Legacy function name.");
  assert.match(materializerSource, /function normalizeHexColor\(value\)/u, "Legacy helper must remain intact for rollback.");
  ...
  console.log("MATERIALIZER_CORE_ADAPTER_SOURCE_ROUTING_PASSED");
  console.log("MATERIALIZER_NORMALIZE_HEX_MATERIALIZER_DETERMINISM_PASSED");
  ...
  console.log("MATERIALIZER_CORE_ADAPTER_ARCHIVE_ROUTING_PASSED");
  ...
  console.log("MATERIALIZER_CORE_ADAPTER_INSTALLED_ROUTING_PASSED");

  console.log("MATERIALIZER_NORMALIZE_HEX_MATERIALIZER_INTEGRATION_PARITY_PASSED");
  console.log("MATERIALIZER_NORMALIZE_HEX_LEGACY_ROLLBACK_PASSED");
...
function createLegacyRollbackSurface(target, sourceRoot) {
  ...
  const legacy = source
    .replace('  normalizeHexColor as coreNormalizeHexColor,\n', "")
    .replaceAll("coreNormalizeHexColor(", "normalizeHexColor(");
...
async function materialize(surfaceRoot, surface, run) {
  ...
  assert.deepEqual(colors, fixture.expectedColors, `${surface} materialization must route planned colors through normalizeHexColor.`);
