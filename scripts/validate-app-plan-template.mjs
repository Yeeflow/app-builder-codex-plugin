...
const REQUIRED_PATTERNS = [
  ...
  ["DATA_LIST_CUSTOM_FORM_FIELD_BEHAVIOR_TABLE", /\|\s*Form\s*\|\s*Section\s*\|\s*Field ID\s*\|\s*Display Name\s*\|\s*Type\s*\|\s*Binding\s*\|\s*Required\s*\|\s*Read Only\s*\|\s*Default Value\s*\|\s*Auto Fill \/ Source/i],
  ...
  ["FORM_CONTROL_DETAIL_TABLE", /\|\s*Field ID\s*\|\s*Display Name\s*\|\s*Type\s*\|\s*Binding\s*\|\s*Required\s*\|\s*Read Only\s*\|\s*Default Value\s*\|\s*Auto Fill \/ Source\s*\|\s*Description/i],
  ["FORM_FIELD_BEHAVIOR_RULES", /Read-only, default value, and autofill rules must be explicit/i],
  ["WORKFLOW_NODE_TABLE", /\|\s*Step Name\s*\|\s*Type\s*\|\s*Description\s*\|\s*Email Enabled\s*\|\s*Task\/Decision Settings\s*\|\s*Transition Rules/i],
