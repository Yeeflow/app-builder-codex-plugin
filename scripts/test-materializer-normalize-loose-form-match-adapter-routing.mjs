...
const fixture = JSON.parse(readFileSync(resolve(root, "compatibility/differential-fixtures/materializer-normalize-loose-form-match-integration.v0.1.0.json"), "utf8"));
const temporary = mkdtempSync(resolve(tmpdir(), "yeeflow-materializer-loose-form-match-routing-"));
...
try {
  ...
  assert.equal(count(materializerSource, "coreNormalizeForLooseFormMatch("), fixture.expectedCallSites.postCutoverCoreCallCount, "Post-cutover Core call-site count must match the approved scope.");
  assert.equal(count(materializerSource, "normalizeForLooseFormMatch("), fixture.expectedCallSites.postCutoverLegacyCallCount + 1, "Only the retained Legacy helper declaration may retain the Legacy function name.");
  assert.match(materializerSource, /function normalizeForLooseFormMatch\(value\)/u, "Legacy helper must remain intact for rollback.");
  ...
  console.log("MATERIALIZER_LOOSE_FORM_MATCH_SOURCE_ROUTING_PASSED");
  console.log("MATERIALIZER_LOOSE_FORM_MATCH_MATERIALIZER_DETERMINISM_PASSED");
  ...
  console.log("MATERIALIZER_LOOSE_FORM_MATCH_ARCHIVE_ROUTING_PASSED");
  ...
  console.log("MATERIALIZER_LOOSE_FORM_MATCH_INSTALLED_ROUTING_PASSED");

  console.log("MATERIALIZER_LOOSE_FORM_MATCH_MATERIALIZER_INTEGRATION_PARITY_PASSED");
  console.log("MATERIALIZER_LOOSE_FORM_MATCH_LEGACY_ROLLBACK_PASSED");
...
function createLegacyRollbackSurface(target, sourceRoot) {
  ...
  const legacy = source
    .replace('  normalizeForLooseFormMatch as coreNormalizeForLooseFormMatch,\n', "")
    .replaceAll("coreNormalizeForLooseFormMatch(", "normalizeForLooseFormMatch(");
...
async function materialize(surfaceRoot, surface, run) {
  ...
  assertReverseRelatedSection(decoded, surface);
  return { normalizedDecoded: normalizeDecoded(decoded), outputFiles };
}

function assertReverseRelatedSection(decoded, surface) {
  const customers = decoded.Childs?.find((child) => child.List?.Title === "Customers");
  assert.ok(customers, `${surface} package must contain the Customers data list.`);
  const layout = customers.Layouts?.find((candidate) => candidate.Title === fixture.expectedFormTitle);
  assert.ok(layout, `${surface} package must contain the expected Customers View form.`);
  assert.match(layout.LayoutView, new RegExp(fixture.expectedReverseRelatedSectionTitle), `${surface} form must retain the reverse-related section selected through loose form matching.`);
}
