...
const corpusPath = resolve(root, "compatibility/differential-fixtures/materializer-strip-planning-document-suffix.v0.1.0.json");
...
if (typeof coreModule.stripPlanningDocumentSuffix !== "function") throw new Error("MATERIALIZER_STRIP_PLANNING_SUFFIX_CORE_EXPORT_MISSING");
console.log("MATERIALIZER_STRIP_PLANNING_SUFFIX_CORE_SHADOW_IMPLEMENTED");
...
for (const fixture of corpus.cases) {
  ...
  const coreResult = invoke(coreModule.stripPlanningDocumentSuffix, fixture, coreInput);
...
console.log(`MATERIALIZER_STRIP_PLANNING_SUFFIX_DIFFERENTIAL_PARITY_PASSED cases=${corpus.cases.length}`);
console.log(`MATERIALIZER_STRIP_PLANNING_SUFFIX_IMMUTABILITY_PASSED cases=${immutableCases}`);
...
function extractLegacyFunction() {
  ...
  const declaration = ast.statements.find((statement) => ts.isFunctionDeclaration(statement) && statement.name?.text === "stripPlanningDocumentSuffix");
  if (!declaration) throw new Error("MATERIALIZER_STRIP_PLANNING_SUFFIX_LEGACY_FUNCTION_MISSING");
  const exactSource = text.slice(declaration.getStart(ast), declaration.getEnd());
  if (createHash("sha256").update(exactSource).digest("hex") !== corpus.legacy.functionSourceSha256) throw new Error("MATERIALIZER_STRIP_PLANNING_SUFFIX_LEGACY_SOURCE_MISMATCH");
