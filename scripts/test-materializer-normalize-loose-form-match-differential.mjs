...
const corpusPath = resolve(root, "compatibility/differential-fixtures/materializer-normalize-loose-form-match.v0.1.0.json");
...
if (typeof coreModule.normalizeForLooseFormMatch !== "function") throw new Error("MATERIALIZER_LOOSE_FORM_MATCH_CORE_EXPORT_MISSING");
console.log("MATERIALIZER_LOOSE_FORM_MATCH_CORE_SHADOW_IMPLEMENTED");
...
for (const fixture of corpus.cases) {
  ...
  const coreResult = invoke(coreModule.normalizeForLooseFormMatch, fixture, coreInput);
...
console.log(`MATERIALIZER_LOOSE_FORM_MATCH_DIFFERENTIAL_PARITY_PASSED cases=${corpus.cases.length}`);
console.log(`MATERIALIZER_LOOSE_FORM_MATCH_IMMUTABILITY_PASSED cases=${immutableCases}`);
...
function extractLegacyFunction() {
  ...
  const declaration = ast.statements.find((statement) => ts.isFunctionDeclaration(statement) && statement.name?.text === "normalizeForLooseFormMatch");
  if (!declaration) throw new Error("MATERIALIZER_LOOSE_FORM_MATCH_LEGACY_FUNCTION_MISSING");
  ...
  if (digest !== corpus.legacy.functionSourceSha256) throw new Error("MATERIALIZER_LOOSE_FORM_MATCH_LEGACY_SOURCE_MISMATCH");
