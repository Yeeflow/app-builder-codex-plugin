...
const corpusPath = resolve(root, "compatibility/differential-fixtures/materializer-dependency-name.v0.1.0.json");
...
if (typeof coreModule.dependencyName !== "function") throw new Error("MATERIALIZER_DEPENDENCY_NAME_CORE_EXPORT_MISSING");
console.log("MATERIALIZER_DEPENDENCY_NAME_CORE_SHADOW_IMPLEMENTED");
...
for (const fixture of corpus.cases) {
  ...
  const coreResult = invoke(coreModule.dependencyName, fixture, coreInput);
...
console.log(`MATERIALIZER_DEPENDENCY_NAME_DIFFERENTIAL_PARITY_PASSED cases=${corpus.cases.length}`);
console.log(`MATERIALIZER_DEPENDENCY_NAME_IMMUTABILITY_PASSED cases=${immutableCases}`);
...
function extractLegacyFunction() {
  ...
  const declaration = ast.statements.find((statement) => ts.isFunctionDeclaration(statement) && statement.name?.text === "dependencyName");
  if (!declaration) throw new Error("MATERIALIZER_DEPENDENCY_NAME_LEGACY_FUNCTION_MISSING");
  const exactSource = text.slice(declaration.getStart(ast), declaration.getEnd());
  if (createHash("sha256").update(exactSource).digest("hex") !== corpus.legacy.functionSourceSha256) throw new Error("MATERIALIZER_DEPENDENCY_NAME_LEGACY_SOURCE_MISMATCH");
