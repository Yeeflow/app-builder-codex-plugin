...
const fixture = JSON.parse(readFileSync(resolve(root, "compatibility/differential-fixtures/materializer-strip-planning-document-suffix-integration.v0.1.0.json"), "utf8"));
const temporary = mkdtempSync(resolve(tmpdir(), "yeeflow-materializer-strip-planning-suffix-routing-"));
...
try {
  ...
  assert.equal(count(materializerSource, "coreStripPlanningDocumentSuffix("), fixture.expectedCallSites.postCutoverCoreCallCount, "Post-cutover Core call-site count must match the approved scope.");
  assert.equal(count(materializerSource, "stripPlanningDocumentSuffix("), fixture.expectedCallSites.postCutoverLegacyCallCount + 1, "Only the retained Legacy helper declaration may retain the Legacy function name.");
  assert.match(materializerSource, /function stripPlanningDocumentSuffix\(value\)/u, "Legacy helper must remain intact for rollback.");

  const legacyRoot = createLegacyRollbackSurface(resolve(temporary, "legacy-baseline"), root);
  const legacyBaseline = await materializeAll(legacyRoot, "legacy-baseline", 1);

  const sourceFirst = await materializeAll(root, "source", 1);
  const sourceSecond = await materializeAll(root, "source", 2);
  ...
  console.log("MATERIALIZER_STRIP_PLANNING_SUFFIX_SOURCE_ROUTING_PASSED");
  console.log("MATERIALIZER_STRIP_PLANNING_SUFFIX_MATERIALIZER_DETERMINISM_PASSED");
  ...
  const archiveResult = await materializeAll(resolve(archiveRoot, "yeeflow-app-builder-plugin"), "archive", 1);
  assertParity(legacyBaseline, archiveResult, "archive");
  console.log("MATERIALIZER_STRIP_PLANNING_SUFFIX_ARCHIVE_ROUTING_PASSED");

  const installedRoot = resolve(temporary, "installed/yeeflow-app-builder-plugin");
  cpSync(pluginDist, installedRoot, { recursive: true });
  const installedResult = await materializeAll(installedRoot, "installed", 1);
  assertParity(legacyBaseline, installedResult, "installed");
  console.log("MATERIALIZER_STRIP_PLANNING_SUFFIX_INSTALLED_ROUTING_PASSED");

  console.log("MATERIALIZER_STRIP_PLANNING_SUFFIX_MATERIALIZER_INTEGRATION_PARITY_PASSED");
  console.log("MATERIALIZER_STRIP_PLANNING_SUFFIX_LEGACY_ROLLBACK_PASSED");
...
function createLegacyRollbackSurface(target, sourceRoot) {
  ...
  const legacy = source
    .replace('  stripPlanningDocumentSuffix as coreStripPlanningDocumentSuffix,\n', "")
    .replaceAll("coreStripPlanningDocumentSuffix(", "stripPlanningDocumentSuffix(");
...
async function materializeAll(surfaceRoot, surface, run) {
  const results = [];
  for (const testCase of fixture.cases) results.push(await materialize(surfaceRoot, surface, run, testCase));
  return results;
}

async function materialize(surfaceRoot, surface, run, testCase) {
  const fixtureRoot = resolve(temporary, `${surface}-${run}-${testCase.id}`);
  ...
  const moduleUrl = `${pathToFileURL(resolve(surfaceRoot, "scripts/materialize-full-app-generated-final.mjs")).href}?surface=${surface}-${run}-${testCase.id}`;
  ...
  assert.equal(result.status, "pass", `${surface} ${testCase.id} materialization must pass.`);
  const outputFiles = readdirSync(output).sort();
  assert.ok(outputFiles.length > 0 && outputFiles.every((file) => file.startsWith(testCase.expectedSlug)), `${surface} ${testCase.id} output names must use the suffix-stripped application title.`);
  const decoded = JSON.parse(readFileSync(result.outputs.decodedResource, "utf8"));
  return { id: testCase.id, outputFiles, normalizedDecoded: normalizeDecoded(decoded) };
}
...
function assertParity(legacy, actual, surface) {
  assert.deepEqual(actual, legacy, `${surface} normalized decoded output and output-file shape must match the Legacy baseline.`);
}
