...
const corpusPath = resolve(root, "compatibility/differential-fixtures/materializer-normalize-hex-color.v0.1.0.json");
...
if (typeof coreModule.normalizeHexColor !== "function") throw new Error("MATERIALIZER_NORMALIZE_HEX_CORE_EXPORT_MISSING");
console.log("MATERIALIZER_NORMALIZE_HEX_CORE_SHADOW_IMPLEMENTED");
...
for (const fixture of corpus.cases) {
  ...
  const coreResult = invoke(coreModule.normalizeHexColor, fixture, coreInput);
...
console.log(`MATERIALIZER_NORMALIZE_HEX_DIFFERENTIAL_PARITY_PASSED cases=${corpus.cases.length}`);
console.log(`MATERIALIZER_NORMALIZE_HEX_IMMUTABILITY_PASSED cases=${immutableCases}`);
...
function extractLegacyFunction() {
  ...
  const declaration = ast.statements.find((statement) => ts.isFunctionDeclaration(statement) && statement.name?.text === "normalizeHexColor");
  if (!declaration) throw new Error("MATERIALIZER_NORMALIZE_HEX_LEGACY_FUNCTION_MISSING");
  ...
  if (digest !== corpus.legacy.functionSourceSha256) throw new Error("MATERIALIZER_NORMALIZE_HEX_LEGACY_SOURCE_MISMATCH");
  return vm.runInNewContext(`(${exactSource})`, { String });
}
