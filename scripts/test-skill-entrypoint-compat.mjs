...
function runNode(args) {
  return spawnSync(process.execPath, [path.resolve(ROOT, args[0]), ...args.slice(1)], { cwd: os.tmpdir(), encoding: "utf8", maxBuffer: 32 * 1024 * 1024 });
}
...
try {
  ...
  const invalidFieldReport = parseReport(runNode(["skills/yeeflow-application-generator/scripts/validate-yap-package.js", writePackage(tmp, "invalid-fieldtype", app({ fields: invalidFields }))]));
  ...
  const validFieldReport = parseReport(runNode(["skills/yeeflow-application-generator/scripts/validate-yap-package.js", writePackage(tmp, "valid-fieldtype", app({ fields: validFields }))]));
  assert.equal(validFieldReport.errors.some((error) => /^YAP_DATALIST_FIELDTYPE_/.test(error.code)), false);

  const invalidApprovalReport = parseReport(runNode(["skills/yeeflow-application-generator/scripts/validate-yap-package.js", writePackage(tmp, "invalid-approval", app({ fields: validFields, def: approvalDef({ invalidPage: true, noTask: true }) }))]));
  ...
  const validApprovalReport = parseReport(runNode(["skills/yeeflow-application-generator/scripts/validate-yap-package.js", writePackage(tmp, "valid-approval", app({ fields: validFields }))]));
