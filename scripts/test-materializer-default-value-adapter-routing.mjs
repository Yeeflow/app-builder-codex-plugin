...
const fixture = JSON.parse(readFileSync(resolve(root, "compatibility/differential-fixtures/materializer-default-value-for-field-type-integration.v0.1.0.json"), "utf8"));
const temporary = mkdtempSync(resolve(tmpdir(), "yeeflow-materializer-default-value-routing-"));
...
try {
  ...
  assert.equal(count(materializerSource, "coreDefaultValueForFieldType("), fixture.expectedCallSites.postCutoverCoreCallCount, "Post-cutover Core call-site count must match the approved scope.");
  assert.equal(count(materializerSource, "defaultValueForFieldType("), fixture.expectedCallSites.postCutoverLegacyCallCount + 1, "Only the retained Legacy helper declaration may retain the Legacy function name.");
  assert.match(materializerSource, /function defaultValueForFieldType\(fieldType\)/u, "Legacy helper must remain intact for rollback.");
  ...
  console.log("MATERIALIZER_DEFAULT_VALUE_SOURCE_ROUTING_PASSED");
  console.log("MATERIALIZER_DEFAULT_VALUE_MATERIALIZER_DETERMINISM_PASSED");
  ...
  console.log("MATERIALIZER_DEFAULT_VALUE_ARCHIVE_ROUTING_PASSED");
  ...
  console.log("MATERIALIZER_DEFAULT_VALUE_INSTALLED_ROUTING_PASSED");

  console.log("MATERIALIZER_DEFAULT_VALUE_MATERIALIZER_INTEGRATION_PARITY_PASSED");
  console.log("MATERIALIZER_DEFAULT_VALUE_LEGACY_ROLLBACK_PASSED");
...
function createLegacyRollbackSurface(target, sourceRoot) {
  ...
  const legacy = source
    .replace('  defaultValueForFieldType as coreDefaultValueForFieldType,\n', "")
    .replace("coreDefaultValueForFieldType(", "defaultValueForFieldType(");
...
async function materialize(surfaceRoot, surface, run) {
  ...
  const fields = fieldDefaults(decoded);
  assert.deepEqual(fields, fixture.expectedFields, `${surface} materialization must route field defaults through defaultValueForFieldType.`);
  return { fields, normalizedDecoded: normalizeDecoded(decoded), outputFiles };
}

function fieldDefaults(decoded) {
  const departments = decoded.Childs?.find((child) => child.List?.Title === "Departments");
  assert.ok(departments, "Materialized package must contain the Departments data list.");
  return Object.fromEntries(Object.entries(fixture.expectedFields).map(([fieldName]) => {
    const field = departments.Fields?.find((candidate) => candidate.FieldName === fieldName);
    assert.ok(field, `Materialized Departments list must contain ${fieldName}.`);
    return [fieldName, { fieldType: field.FieldType, type: field.Type, defaultValue: field.DefaultValue }];
  }));
}
...
function assertParity(legacy, actual, surface) {
  assert.deepEqual(actual.fields, legacy.fields, `${surface} materialized field defaults must match the Legacy baseline.`);
