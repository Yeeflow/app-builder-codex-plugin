...
function validateTempVarClosure({ findings, page, pageName }) {
  ...
  for (const [index, definition] of definitions.entries()) {
    ...
    if (id && name && id !== name) {
      add(findings, "error", "DASHBOARD_TEMP_VAR_DECLARATION_NAME_MISMATCH", "Dashboard tempVars[] id and name must match for generated KPI/Summary bindings.", {
        page: pageName,
        pointer: `tempVars[${index}]`,
        id,
        name,
  ...
  walk(page, (node, pointer) => {
    ...
    if (rawId.startsWith(TEMP_RUNTIME_PREFIX)) {
      add(findings, "error", "DASHBOARD_TEMP_VAR_DOUBLE_RUNTIME_PREFIX", "Dashboard temp variable runtime reference double-prefixes __temp_.", {
        page: pageName,
        pointer,
        runtimeId: usesPrefixTemp ? `${TEMP_RUNTIME_PREFIX}${rawId}` : runtimeId,
        recommendedFix: "Keep tempVars[].id unprefixed and reference it exactly once as __temp_<declared-id>.",
      });
      return;
    }
    if (usesExpressionTemp && name && name !== rawId) {
      add(findings, "error", "DASHBOARD_TEMP_VAR_REFERENCE_NAME_MISMATCH", "Dashboard temp expression name must equal the raw declared ID while expression id uses __temp_<declared-id>.", {
        page: pageName,
        pointer,
        runtimeId,
        name,
    ...
    if (!rawId || !declared.has(rawId)) {
      add(findings, "error", "DASHBOARD_TEMP_VAR_REFERENCE_UNRESOLVED", "Dashboard temp variable runtime reference does not resolve to page.tempVars[].", {
        page: pageName,
        pointer,
        runtimeId: usesPrefixTemp ? `${TEMP_RUNTIME_PREFIX}${rawId}` : runtimeId,
        expectedDeclaration: rawId || null,
        recommendedFix: "Declare the raw id in page.tempVars[] or remove the stale reference.",
      });
    }
  });
}
