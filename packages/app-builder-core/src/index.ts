import type {
  ...
  CanonicalFieldIntent,
  CoreApplicationBuildResult,
  CoreApplicationPlan,
  CoreFinding,
  CoreMaterialization,
  CoreRepairAction,
...
import { CORE_APPLICATION_INTENT_VERSION } from "@yeeflow/app-builder-core-contracts";
import { projectDataListScalarField } from "@yeeflow/app-builder-core-materializer";
import { projectPlanningLabel } from "@yeeflow/app-builder-core-planning";
...
export function buildApplicationFromCanonicalIntent(intent: CanonicalApplicationIntent): CoreApplicationBuildResult {
...
function projectPlan(intent: CanonicalApplicationIntent, findings: CoreFinding[]): CoreApplicationPlan | null {
  ...
  const resources: CoreApplicationPlan["application"]["resources"][number][] = [];
  application.resources.forEach((resource, resourceIndex) => {
    ...
    if (!resourceLabel.cleanLabel || resourceLabel.isPlaceholder) findings.push(finding("CORE_APPLICATION_RESOURCE_NAME_REQUIRED", `${path}.name`, "A concrete resource name is required."));
    if (resource.kind !== SUPPORTED_RESOURCE_KIND) findings.push(finding("CORE_APPLICATION_RESOURCE_KIND_UNSUPPORTED", `${path}.kind`, "The resource kind is unsupported."));
    const resourceKey = resourceLabel.normalizedLabel;
    if (resourceKey && resourceKeys.has(resourceKey)) findings.push(finding("CORE_APPLICATION_RESOURCE_NAME_DUPLICATE", `${path}.name`, "Resource names must be unique."));
    if (resourceKey) resourceKeys.add(resourceKey);
    if (!Array.isArray(resource.fields) || resource.fields.length === 0) findings.push(finding("CORE_APPLICATION_FIELD_REQUIRED", `${path}.fields`, "At least one field is required."));

    const fieldKeys = new Set<string>();
    const fields: CanonicalFieldIntent[] = [];
    if (Array.isArray(resource.fields)) resource.fields.forEach((field, fieldIndex) => {
      ...
      if (!fieldLabel.cleanLabel || fieldLabel.isPlaceholder) findings.push(finding("CORE_APPLICATION_FIELD_NAME_REQUIRED", `${fieldPath}.name`, "A concrete field name is required."));
      if (!String(field.fieldType || "").trim()) findings.push(finding("CORE_APPLICATION_FIELD_TYPE_REQUIRED", `${fieldPath}.fieldType`, "A field type is required."));
      if (!String(field.controlType || "").trim()) findings.push(finding("CORE_APPLICATION_CONTROL_TYPE_REQUIRED", `${fieldPath}.controlType`, "A control type is required."));
      if (fieldKey && fieldKeys.has(fieldKey)) findings.push(finding("CORE_APPLICATION_FIELD_NAME_DUPLICATE", `${fieldPath}.name`, "Field names must be unique within a resource."));
...
function materializePlan(plan: CoreApplicationPlan, findings: CoreFinding[]): CoreMaterialization | null {
    ...
    resource.fields.forEach((field, fieldIndex) => {
      ...
      for (const projectedFinding of result.findings) {
        findings.push(finding(
          projectedFinding.code,
          `application.resources[${resourceIndex}].fields[${fieldIndex}].${projectedFinding.path}`,
          projectedFinding.message,
          projectedFinding.severity,
        ));
      }
      ...
      fields.push(result.projection as unknown as Readonly<Record<string, JsonValue>>);
    });
...
function buildRepair(findings: readonly CoreFinding[]): Readonly<{ required: boolean; actions: readonly CoreRepairAction[] }> {
...
function finding(code: string, path: string, message: string, severity: "error" | "warning" = "error"): CoreFinding {
