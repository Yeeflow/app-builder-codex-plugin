...
const SOURCE_TYPES = Object.freeze({ DATA_LIST: 1, DOCUMENT_LIBRARY: 16, FORM_REPORT: 32 });
export const WORKFLOW_QUERY_DATA_MODES = Object.freeze([...new Set(MODE_ALIASES.values())]);
export const WORKFLOW_QUERY_DATA_SOURCE_TYPES = SOURCE_TYPES;
...
export function buildWorkflowQueryDataProperties(config = {}) {
  ...
  const listType = Number.isInteger(Number(config.listType)) ? Number(config.listType) : SOURCE_TYPES.DATA_LIST;
  if (!Object.values(SOURCE_TYPES).includes(listType)) throw new Error(`Unsupported export-proven Workflow Query Data source type: ${listType}`);
  ...
  return { name: clean(config.name) || "Query data", appid: Number.isInteger(Number(config.appId)) ? Number(config.appId) : 41, listsetid: String(config.listSetId || ""), listid: String(config.listId || ""), listtype: listType, filters: Array.isArray(config.filters) ? structuredClone(config.filters) : [], sorts, result };
}

export function buildWorkflowListVariable({ id, name, listRefId, fields = [] } = {}) {
  const variableId = clean(id); const refId = clean(listRefId);
  return { variable: { idx: variableId, id: variableId, name: clean(name) || variableId, type: "list", editable: true, value: refId }, listref: { id: refId, name: clean(name) || refId, fields: fields.map((field) => ({ idx: clean(field.idx) || clean(field.id), id: clean(field.id), name: clean(field.name) || clean(field.id), type: clean(field.type) || "text", editable: field.editable !== false })), idx: refId } };
}

export function buildWorkflowListLoopProperties({ name, listVariableId } = {}) { return buildWorkflowLoopProperties({ name, loopType: "list", sourceParent: "__variables_", source: listVariableId }); }
export function buildWorkflowLoopProperties({ name, loopType = "list", sourceParent = "__variables_", source, expression } = {}) {
  ...
  if (mode === "list") return { name: clean(name) || "Loop through list items", loopType: "list", loopValue: { prefix: sourceParent === "__list_" ? "__list_" : "__variables_", value: clean(source) } };
  if (mode === "values" || mode === "number") return { name: clean(name) || (mode === "values" ? "Loop through multiple values" : "Loop for fixed times"), loopType: mode, loopValue: { type: 2, value: Array.isArray(expression) ? structuredClone(expression) : [] } };
  return { name: clean(name) || "Loop", loopType: mode, loopValue: {} };
}
...
export function parseWorkflowFieldMap(value) {
  ...
  const out = {};
  for (const pair of String(value || "").split(/\s*;\s*/).filter(Boolean)) { const parts = pair.split(/\s*(?:->|=>|→)\s*/); if (parts.length !== 2) continue; const source = clean(parts[0]); const target = clean(parts[1]); if (source && target) out[source] = target; }
  return out;
}

export function parseWorkflowSorts(value) {
  const out = [];
  for (const entry of String(value || "").split(/\s*;\s*/).filter(Boolean)) { const match = entry.match(/^(.+?)\s+(asc|ascending|desc|descending)$/i); if (!match) continue; out.push({ SortName: clean(match[1]), SortByDesc: /^desc/i.test(match[2]) }); }
  return out;
}

function normalizeFieldMap(value) { if (!value || typeof value !== "object" || Array.isArray(value)) return {}; return Object.fromEntries(Object.entries(value).map(([source, target]) => [clean(source), clean(target)]).filter(([source, target]) => source && target)); }
