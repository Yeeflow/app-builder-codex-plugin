...
export function workflowSetDataListProjectionRecordLegacy(record = {}) {
  return {
    ...record,
...
export function buildWorkflowVariablesFromSetDataListRecordsLegacy(records = []) {
  const declarations = records.flatMap((record) => workflowSetDataListProjectionRecordLegacy(record).workflowVariableDeclarations);
  ...
  for (const group of groups.values()) {
    ...
    if (!isList) {
      ...
      basic.push({ id: group.id, idx: group.id, name: group.name, title: group.name, type: variableType(declaration.type || declaration.valueType), source: "workflow-set-data-list-plan" });
      continue;
    }
...
export function mergeWorkflowVariableProjectionLegacy(target, projected) {
  ...
  for (const variable of projected.basic || []) {
    ...
    if (!existing) { target.basic.push(variable); basicById.set(key, variable); }
  ...
  for (const definition of projected.listref || []) {
    ...
    if (!existing) { target.listref.push(resolvedDefinition); listrefById.set(key, resolvedDefinition); continue; }
