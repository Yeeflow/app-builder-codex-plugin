 ...
 */
export function lowerDataListLookupResolutionAtHost(intent: HostDataListLookupIntent, targetMap: DataListLookupTargetIdentityMap, source: DataListLookupSourceContext): DataListLookupResolutionResult {
  if (!intent || intent.surface !== "data-list" || !intent.resolutionRequest) throw new Error("DATA_LIST_LOOKUP_TARGET_UNRESOLVED");
  if (!intent.declaredTargetKey || intent.resolutionRequest.candidateKeys.length === 0) throw new Error("DATA_LIST_LOOKUP_TARGET_UNRESOLVED");
  ...
  if (!matches.length) throw new Error(`LOOKUP_RESOLUTION_TARGET_MAPPING_MISSING: ${intent.declaredTargetKey}`);
  if (matches.length > 1) throw new Error(`LOOKUP_RESOLUTION_TARGET_MAPPING_AMBIGUOUS: ${matches.map((entry) => entry.candidate).join(",")}`);
...
function validateSourceContext(source: DataListLookupSourceContext): void {
  ...
  if (sourceListId !== sourceFieldListId) throw new Error("LOOKUP_RESOLUTION_SOURCE_TARGET_RELATIONSHIP_BROKEN");
}

function validateLosslessId(value: unknown, key: string): string {
  if (typeof value !== "string" || !/^\d{1,30}$/.test(value)) throw new Error(`LOOKUP_RESOLUTION_TARGET_LIST_ID_INVALID: ${key}`);
  return value;
}
