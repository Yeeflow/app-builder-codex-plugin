...
Public Form field-entry regions may reuse Data List Form field templates `data_list_form_fields_grid_v1_1` and `data_list_form_control_sublist_v1_1` when the mapped fields are public-form-compatible. This does not replace the Public Form page layout template: the field Grid/Sub List must be placed inside an approved Public Form content card's `section_content_area`. Preserve the Data List Form grid/sub-list structure, spacing, responsive columns, zero-margin discipline, and designer labels while remapping only field labels, bindings, field IDs, option rules, placeholders, column spans, and business-specific `nv_label` / `nav_label`. Never use these templates to materialize login-dependent or public-form-disallowed fields into anonymous Public Forms.
