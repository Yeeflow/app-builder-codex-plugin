...
function compileExamples() {
  ...
  const repositoryTsc = resolve(repository, "node_modules/typescript/bin/tsc");
  const bundledTsc = resolve(homedir(), ".cache/codex-runtimes/codex-primary-runtime/dependencies/node/node_modules/typescript/bin/tsc");
  const configuredTsc = process.env.YEEFLOW_VALIDATOR_TSC ? resolve(process.env.YEEFLOW_VALIDATOR_TSC) : "";
  const tsc = configuredTsc && existsSync(configuredTsc) ? configuredTsc : existsSync(repositoryTsc) ? repositoryTsc : bundledTsc;
  if (!existsSync(tsc)) { finding("ATTACHMENT_PATTERN_TSC_MISSING", files.uploader, "Set YEEFLOW_VALIDATOR_TSC or install node_modules/typescript/bin/tsc"); return; }
