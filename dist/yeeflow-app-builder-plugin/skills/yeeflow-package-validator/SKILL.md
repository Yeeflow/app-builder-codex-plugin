...
Generated Dashboard pages must also pass Dashboard Page Layouts v1.1 validation in addition to Event Portfolio Golden Reference conformance. The page-level gate enforces the copied v1.1 shell, `main > content`, `#f4f7fb` page background, zero root padding, Full width structural containers, section title/content areas, real Operations actions, controlled business-content slots, registered repeatable/removable modules only, canonical runtime route proof, and dashboard-only upgrade scope boundaries.
