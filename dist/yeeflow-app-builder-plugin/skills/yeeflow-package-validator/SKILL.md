...
Data Analytics golden-reference validation is a signing-readiness gate. Run `scripts/validate-data-analytics-golden-references.mjs --package <package.yapk>` whenever Pie chart, Column chart, Bar chart, Line chart, or Pivot table controls are generated or upgraded. The approved templates are `data_analytics_pie_chart_with_title`, `data_analytics_column_chart_with_title`, `data_analytics_bar_chart_with_title`, `data_analytics_line_chart_with_title`, and `data_analytics_pivot_table_standard`. Fail packages that place Data Analytics controls on Approval forms, omit approved template provenance, omit the chart wrapper/title contract, simplify chart/pivot controls, or place analytics templates outside `2_columns_section`/`3_columns_section` on Dashboard Page Layouts v1.1 pages.
