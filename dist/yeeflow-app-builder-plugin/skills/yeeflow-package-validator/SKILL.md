...
Generated-final `.yapk` output must use API-issued numeric content IDs from `GET /utils/generate/ids?count=<n>` and must emit a redacted `dist/<app-name>-id-provenance-report.json` with `sourceMarker: "api-generated"`, path-to-purpose mappings, duplicate checks, unused-ID accounting, generator provenance metadata, and no non-API IDs. Local preview/local-draft packages may exist only for local structural validation and must never be promoted into generated-final by patching/replacing IDs. Generated-final generation must be a fresh ID-first build from approved logical refs plus a complete `logicalRef -> apiIssuedId` map. Local ID generation, hardcoded generated IDs, copied sample/export IDs, random values, timestamps, UUID fallback, and deterministic local-only seeds are forbidden for generated-final `.yapk`. After API ID allocation, generated-final validation must recursively reject unresolved draft sentinels such as `local-draft`, `localDraft`, `local-draft-*`, `sourceMarker: local-draft-no-api`, equivalent generator markers, and unresolved logical references anywhere in decoded package JSON, parsed page resources, form `Ext`/`DefResource`, control bindings, links, action targets, workflow/navigation metadata, theme payloads, arrays, or version fields. Runtime navigation groups must include API-issued `ID`, `AppID`, `ListSetID`, `Type: "classes"`, `Title`, `Icon`, and `list[]`; children must include `AppID`, `Title`, `ListID`, `ListSetID`, and `Type`, with dashboards/pages as `Type: 103` plus `LayoutID`, approval forms as `Type: 105`, and data lists as `Type: 1`. Run `scripts/validate-yapk-id-provenance.mjs`, `scripts/validate-generated-final-draft-placeholders.mjs --mode generated-final`, and `scripts/validate-yapk-navigation-runtime-metadata.mjs`; stop before signing, install, upgrade-check, or handoff if any gate fails. `setsign`/`verifysign` and install acceptance do not prove ID provenance, recursive placeholder/logical-ref replacement, or navigation runtime metadata completeness.

Signing readiness is a separate generated-final gate. Wrapper `TenantID` must be present, non-placeholder, not `"0"`, and must match resolved OAuth tenant context when available. It is tenant metadata, not generated app content ID, so ID provenance validation must ignore `wrapper.TenantID` while `scripts/validate-yapk-signing-readiness.mjs` validates it before `setsign`. Signing reports must record redacted request/response metadata, parse top-level JSON string signatures and supported object-field signatures, and never save raw signature values.
...
Summary/KPI validation must keep three proof states separate: designer-configured Summary control, validator-valid Summary contract, and runtime-proven visible dynamic KPI rendering. Run `scripts/inspect-dashboard-summary-control-contract.mjs` and `scripts/inspect-visible-kpi-runtime-bindings.mjs`. Fallback KPI values pass only when explicitly labeled fallback, and dynamic visible binding remains a gap unless the exact UUID Summary v1.0.1 shape plus before/after mutation proof is present or another focused runtime proof exists.
...
Generated YAPK validation must run before signing, not only after runtime failure. Use `scripts/yapk-first-generation-preflight.mjs --package <file.yapk> --json` for generated packages. It combines canonical schema checks, decoded `AppPackageInfo` checks, data-list system schema gates, dashboard shell/Data table/Text-control gates, ID safety gates, recursive generated-final draft-placeholder absence, navigation runtime metadata, and redacted output checks. A package that fails this gate is not eligible for `setsign`, install, upgrade check, upgrade apply, or user handoff.
