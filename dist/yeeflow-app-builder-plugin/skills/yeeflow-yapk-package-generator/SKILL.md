...
Generated-final `.yapk` output must use API-issued numeric content IDs from `GET /utils/generate/ids?count=<n>` and must emit a redacted `dist/<app-name>-id-provenance-report.json` with `sourceMarker: "api-generated"`, path-to-purpose mappings, duplicate checks, unused-ID accounting, generator provenance metadata, and no non-API IDs. Local preview/local-draft packages may exist only for local structural validation and must never be promoted into generated-final by patching/replacing IDs. Generated-final generation must start from approved logical refs, allocate all required API IDs, build a complete `logicalRef -> apiIssuedId` map, and write the package directly from that map. Local ID generation, hardcoded generated IDs, copied sample/export IDs, random values, timestamps, UUID fallback, and deterministic local-only seeds are forbidden for generated-final `.yapk`. After API ID allocation, finalization must recursively replace local draft placeholders and unresolved logical references in runtime-bearing fields, including `exts.ListID`, filter/control `binding`, `attrs.table.link`, `attrs.data.page.PageID`, `attrs.control_action`, page resource `ver`, Collection/Kanban/Timeline `children`, theme payloads, navigation/workflow targets, and encoded approval/workflow definitions. Generated-final packages must contain no `local-draft`, `localDraft`, `local-draft-*`, `sourceMarker: local-draft-no-api`, equivalent draft sentinel, or unresolved logical reference. Runtime navigation groups must include API-issued `ID`, `AppID`, `ListSetID`, `Type: "classes"`, `Title`, `Icon`, and `list[]`; children must include `AppID`, `Title`, `ListID`, `ListSetID`, and `Type`, with dashboards/pages as `Type: 103` plus `LayoutID`, approval forms as `Type: 105`, and data lists as `Type: 1`. Run `scripts/validate-yapk-id-provenance.mjs`, `scripts/validate-generated-final-draft-placeholders.mjs --mode generated-final`, and `scripts/validate-yapk-navigation-runtime-metadata.mjs`; stop before signing, install, upgrade-check, or handoff if any gate fails. `setsign`/`verifysign` and install acceptance do not prove ID provenance, recursive placeholder/logical-ref replacement, or navigation runtime metadata completeness.

Generated-final signing readiness also requires wrapper `TenantID` from the resolved OAuth tenant context. `TenantID = "0"`, missing/empty TenantID, local draft placeholders, placeholder-like TenantID values, and OAuth tenant mismatches must stop before `setsign`. Treat wrapper `TenantID`, `CreatedBy`, and `ModifiedBy` as tenant/user metadata, not generated app content IDs; do not require them in the API-issued content ID manifest. Run `scripts/validate-yapk-signing-readiness.mjs --package <app.yapk> --expected-tenant-id <oauth-tenant-id>` before signing. `setsign` responses may return a top-level JSON string signature or a supported object-field signature; parse both, redact saved signature values, and fail clearly for unknown response shapes.
