...
If package API automation is requested, prefer OAuth auth, run a dry run first, and require explicit target workspace confirmation before auto-installing a new YAPK. `YEEFLOW_WORKSPACE_ID` is only an optional default/override; workspace discovery should use `node scripts/yeeflow-workspace-list.mjs --category <category>` and redacted ID previews. For existing app changes, generate a versioned YAPK and call upgrade automation only after the target app/package is clearly identified and confirmed. Classify API results as `success`, `already_installed`, `api_rejected`, or `http_rejected`; keep duplicate/already-installed responses separate from unknown failures and suggest upgrade, cleanup, or a renamed/new-version package rather than retrying blindly.
