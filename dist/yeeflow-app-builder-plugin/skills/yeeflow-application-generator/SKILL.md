...
For Approval workflows, calculate `graphposition` through the shared Designer viewport helper. Its `x/y` values translate model coordinates into the initial visible canvas (`screen = model * graphzoom + graphposition`); they are not `minNode - margin`. Keep the transformed node extent visible on open and reject an entirely offscreen initial viewport before standalone or packaged handoff.
