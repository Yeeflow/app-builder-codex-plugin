...
Generated-final `.yapk` output must use API-issued numeric content IDs from `GET /utils/generate/ids?count=<n>` and must emit a redacted `dist/<app-name>-id-provenance-report.json` with `sourceMarker: "api-generated"`, path-to-purpose mappings, duplicate checks, unused-ID accounting, generator provenance metadata, and no non-API IDs. Local preview/local-draft packages may exist only for local structural validation and must never be promoted into generated-final by patching/replacing IDs. Generated-final generation is a fresh ID-first build: resolve logical refs from the approved App Plan, Page Function Plan, and Blueprints; allocate all required API IDs; build a complete `logicalRef -> apiIssuedId` map; then write generated-final resources directly from that map. Local ID generation, hardcoded generated IDs, copied sample/export IDs, random values, timestamps, UUID fallback, and deterministic local-only seeds are forbidden for generated-final `.yapk`. After API ID allocation, finalization must recursively replace nested runtime-bearing draft placeholders and unresolved logical references, not only top-level IDs. Generated-final packages must contain no `local-draft`, `localDraft`, `local-draft-*`, `sourceMarker: local-draft-no-api`, equivalent draft sentinel, or unresolved logical reference in decoded package JSON, parsed `LayoutInResources[].Resource`, form `Ext`/`DefResource`, control bindings, table/action links, open-dashboard/open-form targets, workflow/navigation metadata, theme payloads, arrays, or version fields. Runtime navigation groups must include API-issued `ID`, `AppID`, `ListSetID`, `Type: "classes"`, `Title`, `Icon`, and `list[]`; children must include `AppID`, `Title`, `ListID`, `ListSetID`, and `Type`, with dashboards/pages as `Type: 103` plus `LayoutID`, approval forms as `Type: 105`, and data lists as `Type: 1`. Run `scripts/validate-yapk-id-provenance.mjs`, `scripts/validate-generated-final-draft-placeholders.mjs --mode generated-final`, and `scripts/validate-yapk-navigation-runtime-metadata.mjs`; stop before signing, install, upgrade-check, or handoff if any gate fails. `setsign`/`verifysign` and install acceptance do not prove ID provenance, recursive placeholder/logical-ref replacement, or navigation runtime metadata completeness.

Before signing generated-final `.yapk`, populate wrapper `TenantID` from resolved OAuth tenant context and run `scripts/validate-yapk-signing-readiness.mjs`. `TenantID = "0"`, missing/empty TenantID, local/draft placeholders, placeholder-like TenantID values, and tenant-context mismatches are signing blockers. Treat wrapper `TenantID`, `CreatedBy`, and `ModifiedBy` as tenant/user metadata, not app content IDs; do not include them in generated API ID manifests or `Resource.ReplaceIds`. `setsign` may return a top-level JSON string signature or an object-field signature, so signing helpers must parse both shapes, save redacted request/response metadata, and fail unknown response shapes before handoff.
