...
Plan conformance v0.6.22: treat the approved app plan as the implementation contract, not loose inspiration. Generated applications must preserve planned data lists, fields, approval forms, request/task pages, workflows, pages, reports, print pages, agents/copilots/tools, knowledge resources, integrations/connections, permissions/admin resources, and navigation/information architecture. Every future app plan must include `Generation Contract and Hard Gates`; if it is missing, stop and repair the plan before package generation unless the user explicitly asks for planning-free experimental output. If a planned item cannot be implemented, explicitly mark it `unsupported_unproven_export_shape` or `intentionally_deferred_with_reason`; do not silently omit it. Run `scripts/validate-app-plan-conformance.mjs --plan <plan> --package <package>` or the aggregate `scripts/inspect-generated-app-quality.mjs --plan <plan> --package <package>` and include the plan coverage report before claiming completion. Schema/package validity does not equal plan fidelity.
