...
For plugin-only clean-room generation, inspect `docs/reference/full-app-generation-entrypoints.json` with `scripts/inspect-full-app-generation-entrypoints.mjs` before claiming that the plugin has no full-app generation path. Full application generation is a first-class callable Codex skill entrypoint through `yeeflow-application-builder` and `yeeflow-application-generator`; it is not equivalent to a delivery-decision helper, package API helper, runtime-proof demo generator, or sample-specific generator. When the registry and planning gates pass and business defaults are `user-default-approved-for-generation`, continue into skill-orchestrated generated-final `.yapk` generation instead of stopping solely because no standalone CLI exists. Do not treat `scripts/yeeflow-application-delivery-workflow.mjs`, `scripts/yeeflow-package-api-automation.mjs`, `generate-*-runtime-proof.mjs`, or `generate-vendor-onboarding-yapk-schema-v2.mjs` as generic Functional Spec + App Plan to full-app YAPK generators. If a run explicitly requires a standalone CLI rather than a callable Codex skill entrypoint, state that boundary and do not invent a package.
