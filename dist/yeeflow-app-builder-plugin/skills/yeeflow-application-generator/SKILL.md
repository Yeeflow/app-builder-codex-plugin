...
Plan conformance v0.6.22: treat the approved app plan as the implementation contract, not loose inspiration. Full app plans must follow `docs/app-plan-standard-template.md` unless the user explicitly requested a lightweight plan, and lightweight plans still require `Generation Contract and Hard Gates`. Generated applications must preserve planned data lists, fields, approval forms, request/task pages, workflows, pages, reports, print pages, agents/copilots/tools, knowledge resources, integrations/connections, permissions/admin resources, and navigation/information architecture. Every future app plan must include `Generation Contract and Hard Gates`; if it is missing, stop and repair the plan before package generation unless the user explicitly asks for planning-free experimental output. If a planned item cannot be implemented, explicitly mark it `unsupported_unproven_export_shape` or `intentionally_deferred_with_reason`; do not silently omit it. Run `scripts/validate-app-plan-template.mjs <plan>` before generation when the plan is Markdown, then run `scripts/validate-app-plan-conformance.mjs --plan <plan> --package <package>` or the aggregate `scripts/inspect-generated-app-quality.mjs --plan <plan> --package <package>` after generation. Include plan section conformance in the report before claiming completion. Schema/package validity does not equal plan fidelity.
...
When local `.env.local` contains `YEEFLOW_API_KEY` and `YEEFLOW_WORKSPACE_ID`, ask the user whether to auto-install the generated YAPK. Do not auto-install without explicit confirmation. For existing app changes, produce a versioned YAPK and upgrade only when the target app/package is clearly identified and approved. Package API result classes are `success`, `already_installed`, `api_rejected`, and `http_rejected`; an `already_installed` result should lead to upgrade flow, cleanup, or a renamed/new-version package, not blind retries.
