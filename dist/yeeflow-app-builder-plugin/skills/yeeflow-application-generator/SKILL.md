...
Use `scripts/materialize-full-app-generated-final.mjs --functional-spec functional-specification.md --app-plan yeeflow-app-plan.md --out-dir dist --api-id-manifest api-issued-ids.json --json` as the concrete materialization handoff. The command emits generated-final `.yapk`, decoded resource JSON, ID provenance report, and generation report. It never signs, installs/imports, upgrades, seeds data, runs Version Management proof, or runs browser/runtime proof. Fixture-ID mode is allowed only for plugin regression tests and is not signing/install eligible.
