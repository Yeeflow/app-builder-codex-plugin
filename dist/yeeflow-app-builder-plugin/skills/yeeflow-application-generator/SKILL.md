...
When signing is authorized after generated-final preflight, use only `node scripts/yeeflow-yapk-sign.mjs --package <unsigned.yapk> --output <signed.yapk> --execute`. Do not create task-local signing scripts. The helper preserves the unsigned package and unchanged `Resource`, accepts only the supported response shapes with an exact 32-byte base64 signature, verifies before writing the separate signed output, redacts all diagnostics, and keeps install blocked on failure.
