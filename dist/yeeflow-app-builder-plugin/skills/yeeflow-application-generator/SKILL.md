...
Plan conformance v0.6.22: treat the approved app plan as the implementation contract, not loose inspiration. Full app plans must follow `docs/app-plan-standard-template.md` unless the user explicitly requested a lightweight quick outline; lightweight plans still require Data Model and Lists, Forms and Approval Forms, Application Navigation, UI/UX and Control Mapping, Generation Contract and Hard Gates, Proof Boundary, and Assumptions/Deferred Items. Generated applications must preserve planned data lists, fields, data-list views, custom list forms/public forms, list workflows, scheduled workflows, notifications, approval forms, request/task pages, workflows, pages, reports, print pages, agents/copilots/tools, knowledge resources, integrations/connections, custom code/custom CSS, permissions/admin resources, golden/template references, and navigation/information architecture. Every future app plan must include `Generation Contract and Hard Gates`; if it is missing, stop and repair the plan before package generation unless the user explicitly asks for planning-free experimental output. If a planned item cannot be implemented, explicitly mark it `unsupported_unproven_export_shape` or `intentionally_deferred_with_reason`; do not silently omit it. Run `scripts/validate-app-plan-template.mjs <plan>` before generation when the plan is Markdown, then run `scripts/validate-app-plan-conformance.mjs --plan <plan> --package <package>` or the aggregate `scripts/inspect-generated-app-quality.mjs --plan <plan> --package <package>` after generation. Include plan section conformance in the report before claiming completion. Schema/package validity does not equal plan fidelity.
...
Do not generate a `.yap` or `.yapk` package directly from a broad app request unless the user explicitly skips planning. Start from a saved or visible Markdown app plan that follows `docs/app-plan-standard-template.md` where present: numbered plan status, purpose, roles, business process, capability coverage, navigation, data lists, forms, dashboards/pages/reports, UI/control mapping, reference strategy, actions/workflows, permissions, integrations, documents, analytics, data validation, generation hard gates, validation plan, proof boundary, assumptions, deferred items, and recommended next prompt.

The `Generation Contract and Hard Gates` section is binding during generation. It must cover output package decision, YAPK signing gate, approval-form contract, navigation runtime contract, plan-to-package conformance contract, proof-boundary contract, and runtime inspection checklist. Do not generate an approval app with `Forms: []`; do not use `children`/`Childs` for runtime grouped navigation; do not call a placeholder-`Sign` YAPK upload-ready; and do not merge API install/import acceptance with runtime UI success.
...
When package automation is requested, prefer OAuth auth, run dry-run first, and require explicit target workspace confirmation before installing or upgrading a generated YAPK. `YEEFLOW_WORKSPACE_ID` is optional for normal OAuth and workspace discovery; package writes must resolve the target from `--workspace-id`, optional environment default, or a user-selected workspace listed with `node scripts/yeeflow-workspace-list.mjs --category <category>`. Do not auto-install without explicit confirmation. For existing app changes, produce a versioned YAPK and upgrade only when the target app/package is clearly identified and approved. Package API result classes are `success`, `already_installed`, `api_rejected`, and `http_rejected`; an `already_installed` result should lead to upgrade flow, cleanup, or a renamed/new-version package, not blind retries.
