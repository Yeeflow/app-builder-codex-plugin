...
5. Require control-property fidelity for high-quality UI. Section presence is not enough. Use `scripts/inspect-ui-control-property-fidelity.mjs` for declared/redacted control specs before claiming dashboard/content fidelity. Use the Yeeflow control property knowledge base before generating controls, and do not invent property paths. Product catalog paths are the baseline; the extension registry can add study-backed paths over time, but export/runtime proof is still required for visual fidelity claims. Use Container controls for visual composition rows/cards unless a real data grid is required. Use real Data Filter controls for filters, not static Text. Data Filters must declare filter variables, and target Summary, Collection, or List controls must consume those variables. KPI cards must follow one golden card/icon/text-stack pattern consistently across peer cards. Runtime screenshots must be refreshed before capture, and signing/upgrade success is not visual proof.
...
- `scripts/inspect-ui-control-property-fidelity.mjs` for UI control-property fidelity. It validates declared/redacted metadata only, checks Container attrs, filter/action row layout, Data Filter control requirements, filter variable consumption, action sizes, KPI icon tiles, KPI text stack consistency, Container-vs-Grid choice, Data Filter-vs-static Text choice, mock/live KPI boundaries, and dynamic KPI before/after proof boundaries. It does not decode private package payloads, parse screenshots, inspect Chrome, call Yeeflow APIs, sign, install, import, or upgrade packages.
