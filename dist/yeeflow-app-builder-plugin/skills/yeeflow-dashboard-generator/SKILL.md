...
Dashboard Page Layouts Workbench controlled-slot rule: preserve `page_title_header`, top `section_content_area`, `main_work_queue_section`, `main_work_queue_wrapper`, `primary_working_area`, and optional `right_side_panel`. Business content may change only inside registered Workbench slots such as `page_title_content`, `Operations`, `section_content_area`, `section_title_header`, `dashboard_standard_filter_group`, `kpi_cards_kpi_row`, `kpi_card_wrapper`, `primary_working_area`, `right_side_panel`, and `chart_cards_section`. The Header has a non-negotiable direct-child contract: `page_title_content` keeps `attrs.style.widthtype = [null,"2",null,"1"]` (inline), while any planned Header action must be inside a direct `Operations` child. Clone `btn_operation_primary` for the primary create/commit action and `btn_operation_normal` for secondary actions; never append a generic Container/action directly under `page_title_header` or hand-create a visual substitute. Remove `Operations` only when no Header action exists. Before `component_save`, fail closed on `DASH_WORKBENCH_PAGE_TITLE_CONTENT_NOT_INLINE`, `DASH_WORKBENCH_HEADER_OPERATIONS_MISSING`, `DASH_WORKBENCH_HEADER_ACTION_OUTSIDE_OPERATIONS`, or `DASH_WORKBENCH_OPERATION_BUTTON_VARIANT_INVALID`; after save, Designer-open the page and browser-click each Header operation before reporting `browserActionRuntime` / `actionsUsable: true`. Remove unused copied modules, empty `section_content_area`, empty `chart_cards_section`, and empty `right_side_panel`. Put grouped chart-like Data Analytics templates in `chart_cards_section` under `primary_working_area` or `right_side_panel`, with no more than three chart-like analytics templates per chart section. Pivot table templates are table-like and must use `content_card_wrapper > section_content_area`, preferably in `1_columns_section`; never place `data_analytics_pivot_table_standard` / `pivot_table_standard` inside `chart_cards_section`. When Workbench has multiple global/page-level Data Filters, place them inside `dashboard_standard_filter_group` copied from `docs/reference/data-filter-standard-filter-group.template.json` as the standard `flex_grid` with 4 desktop columns, 2 tablet columns, 1 mobile column, 16px gaps, and `displayLabel = [null,false]`; never emit simplified `columns.count/type/minmax` Grid settings. If `right_side_panel` is removed or empty, normalize `main_work_queue_wrapper` to a single `1fr` column so the generated page does not retain an empty right-side column.

Dashboard Page Layouts master-detail workspace controlled-slot rule: use `dashboard-page-layouts-two-panel-workspace` for inbox-style list/detail pages with a left dataset list and a right selected-record detail panel. Use `dashboard-page-layouts-three-panel-workspace` when the selected record needs a main detail area plus an additional right-side information/action panel. Preserve `left_panel`, `content_panel`, `left_panel_data_items_wrapper`, `current_item_wrapper`, page temp variable `vCurrentItemID`, and the selection action that writes the clicked left-panel Collection item ID to `vCurrentItemID`. `current_item_wrapper` must bind to the same source dataset as `left_panel_data_items_wrapper`, limit records to `1`, and filter the record ID by `vCurrentItemID`. Business content may change only inside registered master-detail slots, including `left_panel_caption_title`, `left_panel_caption_add_button`, `left_panel_filter_group`, `left_panel_filter_control`, `left_panel_data_item`, `main_content_page_title`, `current_item_main_header_operations`, `page_title_header`, `current_item_subject`, `page_title_description`, `dashboard_standard_filter_group`, `current_item_fields_grid`, `current_item_standard_field`, `current_item_large_field`, `primary_working_area`, `content_card_wrapper`, `section_title_header`, `section_content_area`, `chart_cards_section`, and for three-panel layouts the additional right-panel operation/title/content slots. Keep `left_panel_caption_add_button` only for create-capable sources such as Data lists or Document libraries; remove it for Form reports, Data reports, or other read-only/reporting sources. `content_panel_empty` is the editable empty-selection state shown when no left-panel item is selected; its image, title, and description may be mapped to the app domain. Remove empty filter groups, unused Operations, empty `section_content_area`, empty `chart_cards_section`, empty `kpi_metrics_wrapper`, and empty copied section modules. Place grouped chart-like Data Analytics templates in `chart_cards_section`, with no more than three chart-like analytics templates per chart section. Place Pivot table templates in `content_card_wrapper > section_content_area`, not in `chart_cards_section`.

For an existing live Dashboard, do not save only a generated `LayoutView`. Read the full component, use `classifyLiveDashboardResourceMode()` before mutation, then use `normalizeLiveDashboardDetail()` / `validateLiveDashboardDetail()` from `scripts/lib/dashboard-live-component-resource-sync.mjs`. The runtime-authoritative surface is `LayoutInResources[].Resource`; an empty `LayoutView` is a valid embedded-only mode and must be preserved rather than filled or compared as a second runtime source. Require the embedded resource `ID = RefId = LayoutID`. For a generated Custom Code Dashboard, require exactly one parseable embedded resource, `main -> content` page topology, and one `codein` control with a non-empty `attrs["codein-script"]`. After `component_save`, get the component again and run `validateLiveDashboardPostSave(intended, readback, { profile: "custom-code" })`; `DASHBOARD_RESOURCE_NOT_MATERIALIZED`, `DASHBOARD_RESOURCE_ID_MISMATCH`, `DASHBOARD_ROOT_STRUCTURE_INVALID`, `DASHBOARD_CODEIN_MISSING`, and `DASHBOARD_POSTSAVE_DRIFT` block handoff. Then Designer-open and browser-smoke each claimed action. Generated reports must keep `apiAccepted`, `persistedReadback`, `designerOpen`, and `browserActionRuntime` separate; only the last permits `actionsUsable: true`.
...
Standalone `.ydp` generation must also be plan-first. Before generating a standalone Dashboard export, create a Dashboard artifact plan that declares `Artifact Type: dashboard`, page-layout template selection, section plan, source lists, Collection/Data table/Data Analytics/KPI/Summary/filter/action plans, dataset-presentation template IDs and business rationale, page-scoped dependency namespace plan, shared builder family, generation contract, and proof boundary. Project that plan into trace JSON with `artifactType: "dashboard"`, `dashboard.pageLayoutTemplateId`, `dashboard.sections[]`, component-template selections, `sharedBuilder.required = true`, validator list, and `conformance.planToActualRequired = true`. Run `scripts/validate-standalone-artifact-plan-trace.mjs --plan <dashboard-plan.md> --trace <dashboard-plan.trace.json>` before standalone handoff. Treat `STANDALONE_ARTIFACT_PLAN_MISSING`, `STANDALONE_ARTIFACT_TRACE_MISSING`, `STANDALONE_ARTIFACT_SHARED_BUILDER_MISSING`, and `STANDALONE_ARTIFACT_PLAN_TO_ACTUAL_NOT_REQUIRED` as hard failures.

## Executable Standalone YDP Workflow

Never write a generated `.ydp` directly. The authoritative executable path is `node scripts/build-ydp-wrapper.js <dashboard-build-result.json> <output.ydp> --plan <standalone-dashboard-plan.md> --trace <standalone-dashboard-plan.trace.json> --application-plan <core-application-plan.md>`. The standalone plan/trace is validated through `validate-standalone-artifact-plan-trace.mjs`; the separate Core application plan must match `sourceBinding.applicationPlanSha256` and supplies Dashboard conformance context. Never swap or merge these documents. The input must be a `wrapper-ready` result from the shared full-YAPK Dashboard body builder and must carry its body, Core v1.1 export-proven 11-field outer metadata, issued dependency map, credentialed identity provenance, and complete Core source-binding hashes. Do not invent plan-trace or shared-builder fields in the Core result. Six-field fixture-minimum YDP remains readable for compatibility but is not generated-final wrapper-ready output. Bare body JSON is not accepted. The wrapper runs `validate-ydp.js` before writing, prepares both YDP and report temporary files, rereads bytes, enforces canonical UTF-8 and semantic round-trip, reruns all YDP/resource gates, and commits both outputs only after both temporary files succeed. Missing executable entrypoints are capability blockers; never fall back to manual `JSON.stringify` wrapping. Static success is not import, Designer-open, rendering, or runtime proof.
...
Dashboard App Plans must include both an exact approved Collection presentation template ID and a registry-matching business rationale for each dataset region. The exact ID and rationale must be present in the concrete Dashboard Record Display Control Selection region/control row; mentioning the ID only in a separate Dashboard Golden Reference Selection or summary table is insufficient. Immediately before any API ID request, the aggregate `scripts/validate-pre-id-allocation-readiness.mjs --plan <yeeflow-app-plan.md>` gate must run `validate-dashboard-dataset-presentation-golden-references.mjs` and fail closed on missing or vague Collection references. The generator must not output `collection_control_responsive`, `collection_control_responsive_multiple_select`, `collection_control_responsive_card_grid`, or another approved ID as a bare value without explaining the selected reference's business signal, such as dense row/column scanning, work queue, record list, native desktop/tablet table plus mobile cards, card browsing, responsive card browsing, multiselect/bulk operations, or primary operations table. Use `collection_control_responsive` for native sortable table columns on computer/laptop/tablet plus cards on mobile, and `collection_control_responsive_multiple_select` when bulk selection/actions are required. `collection_control_grid_table` and `collection_control_grid_table_with_multiselect` are retired migration-only IDs and must never be selected for a new plan. Treat missing rationale as a planning/generation defect, not as a manual post-generation repair item.
...
For `collection_control_responsive`, clone `grid_table_col_wrapper` from `docs/reference/collection-control-responsive.template.json`. This is the native responsive Collection reference: preserve `tablecols`, `header`, `body`, `table`, pagination, and `list-display-preference: "default"`; computer/laptop/tablet display Table view and mobile displays Card view. The reference does not define business columns: the App Plan dataset row must explicitly supply ordered `Display Fields`, which resolve against the selected source schema and determine the Table column count, titles, bindings, and Card item fields. Clone compatible native column shapes only for those selected fields, remove unused sample columns, and fail on missing or unresolved selections; never cycle/reuse source fields to retain the template's sample column count. Every native column needs a title, boolean `sortingEnabled`, and item content; use `dynamic-user` for user fields and a real numeric field for progress. Retained `grid_table_col_item_operations` must preserve `attrs.common.zidx = [null, 2]` so item-operation clicks are not intercepted by a whole-row click. Do not add an ad hoc Flex Grid header or `grid_col_item` pair. Omit write actions for Form Report/Data Report sources; any Delete action needs the bounded confirmation flow.
...
When a Dashboard or Data List custom form plans Tabs or a related-activity Gantt, select the formal references in `docs/reference/tab-gantt-control-golden-references.json` rather than assembling the controls from a loose prompt. Use `tab_control_workspace` for peer content sections and `gantt_control_linked_activity` for a parent record's related activity schedule. Clone `docs/reference/tab-control-workspace.template.json` or `docs/reference/gantt-control-linked-activity.template.json`, then map only target labels, content, resolved list/layout IDs, and real source fields.

Tab hard rules: the root is `aktabs`; direct children are `ak-tabs-tab`; every tab has a stable ID, label, and non-empty content; and exactly one tab is both runtime and Designer default. Gantt hard rules: source `attrs.data.list.AppID`, `ListSetID`, and `ListID` resolve together; Start/End map to `FieldType = Datetime`; Dependency maps to a same-source Lookup with `Rules.multiple = true`; Percent Complete maps to `FieldType = Decimal` plus `Type = percent` and range `0..1`; Parent is optional but, if set, maps to a same-source Lookup with `Rules.multiple = false`. An Add-enabled linked Gantt must filter activities by the current parent `ListDataID`, pass that same value into the Activity parent lookup, and resolve `linkLayout` to a concrete Type 1 Activity View layout.

Run `node scripts/validate-tab-gantt-golden-references.mjs <decoded-app-or-fixture.json> --strict` and `node scripts/test-tab-gantt-golden-references.mjs`. The source configuration is readback-proven; generated Data List form Gantt rendering, scheduling, Add defaults, task open behavior, and dependency execution still require focused runtime evidence. See `docs/standards/tab-gantt-control-golden-reference-standard.md`.
...
Dashboard Form Action Query Data v1.3/v1.4 is export-proven by `My latest event`. Build through `scripts/lib/form-action-query-data-utils.cjs`, bind on-load through `formAction.onLoad`, and target declared temp variables/temp collections only. The current-user pattern may use `op: "11"` with `right: null`; dependent single queries read an earlier `__temp_` ID with `showCus: false` and require `isNullOrEmpty(temp) == false`. Preserve action step order and require concise generated step names. Pagination is shared with Approval/Data List/Document Library hosts: omitted Page Size 100, explicit 1..1000, omitted Page Number 1, and non-default Page Number uses `querydata_pageindex`. Multiple rows may be selected into temp JSON with explicit `querydata_fields[]`; use `JSONStringfy` for text, not Collection/Data Table binding. Add a Custom Code renderer only with explicit business rationale and separate runtime proof. Run `validate-form-action-query-data-dashboards.mjs --strict-generated` before signing.
At App Plan time, reject `Query Data -> temp JSON/temp collection -> Collection/Data Table`. For read-only recent records or reverse-related rows, bind the approved dataset control directly to its Data List source with the planned filter/sort/limit. Require an explicit result consumer for every temp JSON plan, and require Lookup-backed chained query IDs to name the stored target record `ListDataID`, not the display title. Generation Readiness must run the Query Data plan validator automatically.

Dashboard Collection clones must satisfy `docs/standards/dashboard-collection-schema-dependency-closure-standard.md`. Recursively validate every `__ctx_coll` expression, not only Dynamic controls and Grid cells. Remap display expressions to real target fields; remove complete unprovable template conditions; keep header/item/track parity after pruning. Retained operations require their complete namespaced dependency graph. Accept semantic namespaced variables such as `var_<page>_<region>_isDeleteConfirmed`, but reject every undeclared `__temp_` reference. Do not retain unplanned page-layout KPI modules or source-domain temp variables.

Dashboard Form Actions may use selected-resource Set Data List `add`, `edit`, and `remove` against Data Lists or Document Libraries. Dashboard does not support current-record mode, and every consumed status/item/count output must target a declared page temp variable. Edit/remove must have bounded filters. Prefer a real Button or intentional clickable Container trigger with an exact action binding. Form Action Set Data List cannot expand Sub List rows. Plan, build, and validate through `docs/standards/form-action-set-data-list-golden-reference-standard.md`; never copy Approval workflow-variable result targets into a Dashboard.

Dashboard Form Actions may use V1.7 `listitem`, `openform`, and `opendashboard` only through the shared Open Resource contract. Dashboard cannot use Current item and all source expressions must resolve to declared page temp variables. New approval submission may receive target defaults/query parameters; submitted form is ID-only. Dashboard targets must resolve Type 103. Preserve the shared open-mode/size enum and run `validate-form-action-open-resource.mjs` before signing.

Dashboard Form Actions may use export-backed `print` and `barcode` steps through the shared builder. A multi-record print target must use `dashboard-print-multi-record-table-v1`, with a Collection as the repeated dataset and `table-v2` inside each item; preserve table merges and current-item QR semantics. Barcode result/error targets are declared Dashboard temp variables. When scanned values filter a Collection, materialize the `op=9` variable-expression filter and reject undeclared or unconsumed output. Use only `multiple`, `select`, or `auto` scan mode and Auto or `ean-13` type until more exports extend the enum.

Compose the print module inside an unchanged `dashboard-page-layouts-v1.1` shell. Keep `dashboard-print-multi-record-table-v1` as dedicated print provenance, retain the standard content-card title and content slots, and use the shared trigger helper for planned actions that lack a bound control. Verify `Allow Scan` survives field normalization into stringified Text field Rules, then require the complete `yapk-first-generation-preflight.mjs` regression in addition to focused Print/Barcode validation.
