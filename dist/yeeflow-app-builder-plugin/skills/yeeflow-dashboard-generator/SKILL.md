...
When a Dashboard dataset region uses Collection, choose exactly one approved dataset presentation reference from `docs/reference/dashboard-dataset-presentation-golden-references.json` and carry it as blueprint/resource provenance. Use the selected template as the starting structure; do not simplify it or invent a new Collection shape. The allowed choices are responsive card grid, card multiselect toolbar, grid-table, grid-table with multiselect, grid-table with search, and Event Pipeline Grid-Table.
