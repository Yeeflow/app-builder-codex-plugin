...
For native Collection toolbar alignment and row-menu overflow/contrast, use `docs/standards/collection-toolbar-style-standard.md` and the opt-in `scripts/lib/collection-toolbar-style.mjs` helpers. Put single-control CSS on its own control; shared styles use classes, never duplicate CSS IDs. Do not enlarge all card button descendants to style Add. Preserve Select placeholder hiding while aligning empty/selected states. Enable native Drop bar autoposition and resolve desktop/mobile placement independently. Dark menu foreground requires computed-color proof. Run `scripts/test-collection-toolbar-style.mjs`; desktop save/readback/render proof does not establish mobile, hover/active or Edit execution.
...
Collection Designer-editability is a separate hard contract. Never assemble `attrs.tablecols` from a field list or a visual approximation. For every Collection, read the selected Golden Reference artifact first and retain its native Collection/control tree. Where that artifact uses Designer-native `list-column` entries (currently `collection_control_responsive_multiple_select`), every retained/generated column must have `type: "list-column"`, a unique UUID `id`, and a unique UUID `mapkey`; all non-selection columns must contain an item-context Dynamic child with `attrs.source = "3"` and a real `attrs["obj-f"]` field. This is template-aware: do not inject `list-column` nodes into a different approved template that does not export them. On Dashboard updates, patch approved business slots in the existing cloned subtree and preserve all unknown native attributes; never replace a Collection with a hand-built lookalike. Run the Collection Designer-column contract before save and after `component_get` readback. `DASHBOARD_COLLECTION_DESIGNER_COLUMN_INVALID` and `DASHBOARD_COLLECTION_DESIGNER_COLUMN_DYNAMIC_BINDING_INVALID` are fail-closed. Then open the Designer and verify every column/Dynamic control is independently selectable and no `formcraft.control.undefined` appears. API acceptance, persisted readback, and browser row rendering do not prove Designer editability.
...
### KPI/Summary/Text temporary-variable closure

Treat declaration and runtime reference as different schema layers. Declare each page temp variable once as identical raw `tempVars[].id` and `.name`, for example `var_findings_open`; never declare `__temp_var_findings_open`. For a Dashboard Summary, write `attrs.save_var` as `{ exprType:"variable", type:"expr", id:"__temp_var_findings_open", name:"var_findings_open" }`. For the visible KPI Heading/Text, bind `attrs.headc.title.variable[]` using that exact same runtime-id/raw-name pair. Do not use raw text `var_findings_open`, `__temp_var_findings_open`, a double prefix such as `__temp___temp_var_findings_open`, or a `{prefix,value}` object in the Summary/KPI Text property unless that exact property has separate export proof. `{ prefix:"__temp_", value:"var_findings_open" }` is valid only in property families that explicitly use prefix/value serialization, and its value remains unprefixed.

Before `component_save`, run `scripts/inspect-dashboard-summary-control-contract.mjs` and `scripts/validate-runtime-binding-lessons.mjs`. Treat `SUMMARY_TEMP_VAR_DECLARATION_SYSTEM_PREFIX`, `SUMMARY_SAVE_VAR_RUNTIME_PREFIX_MISSING`, `SUMMARY_SAVE_VAR_DOUBLE_RUNTIME_PREFIX`, `SUMMARY_SAVE_VAR_NAME_MISMATCH`, `SUMMARY_TEMP_VAR_DECLARATION_MISSING`, `SUMMARY_VISIBLE_BINDING_MISSING`, `DASHBOARD_TEMP_VAR_ID_SYSTEM_PREFIX`, `DASHBOARD_TEMP_VAR_DOUBLE_RUNTIME_PREFIX`, and `DASHBOARD_TEMP_VAR_REFERENCE_UNRESOLVED` as fail-closed. Reread after save and report API acceptance, persisted closure, Designer-open, and browser action/value runtime as separate evidence levels; readback alone does not prove KPI refresh.
...
## Dashboard Filter Dependency Closure

For every generated or live-updated Dashboard with Search, Select, Radio, Checkbox, Date, Range, or hierarchy filters, create a filter applicability matrix before writing the resource. Each row must name the declared `filterVars[].id`, the producing control, each consumer region, the consumer source list and field, and whether the filter is intentionally not applicable. Use exactly one builder for the three token forms: declaration `filter_<page>_<name>`, control binding `__filter_<declaration>`, and consumer-expression `{ id: "__filter_<declaration>", name: "<declaration>" }`. A declaration must never itself begin with `__filter_`.

Do not apply a field from one Data List directly to a Collection, Summary, chart, or pivot that reads another Data List. For example, a Finding Risk filter cannot filter a MAP Collection merely because MAP has a Finding lookup. Either mark that consumer not applicable, or require a real mirrored MAP field maintained by a Workflow/synchronization rule or a separately runtime-proven related-query pattern. Never generate an unproven cross-lookup condition.

Before `component_save`, validate the complete closure: every producing control resolves to one declared filter variable; every planned consumer has a Search `attrs.data.fulltext[]` or Select-like `attrs.data.filter[]` condition with a complete `key / pre / left / op / right / showCus` shape; every `left` resolves on the consumer's own source list; and no declaration is unused. Treat `DASHBOARD_FILTER_VAR_ID_RUNTIME_PREFIX`, `DASHBOARD_FILTER_CONTROL_WITHOUT_FILTER_VAR`, `DASHBOARD_FILTER_VAR_DECLARED_NOT_CONSUMED`, and `DASHBOARD_FILTER_CONSUMER_INVALID_FIELD` as fail-closed. Produce a readable binding audit that names intentionally inapplicable regions instead of silently omitting or inventing filters.

After save/readback, do focused browser smoke on the real page: default unfiltered state; one known Select value; Search; two-filter intersection; and clear/reset recovery. Record `apiAccepted`, `persistedReadback`, `designerOpen`, and `browserActionRuntime` separately. Do not call filters usable, or call the Dashboard action-ready, from the first two levels alone.
...
### Lookup Dynamic Field Rule

For every Collection, Kanban, or Timeline `dynamic-field` with `attrs.source = "3"`, resolve `attrs["obj-f"]` against the host Data List before materializing the control. If that field is `lookup`, resolve the Lookup configuration's target list and configured display field, then emit `attrs["dis-f"]` with that exact target field. Do not assume `Title`: it is valid only when the Lookup's own display-field setting resolves to `Title`. Do not use the source lookup field label or stored target `ListDataID` as `dis-f`.

Clone/remap behavior is fail-closed: after the current application's Lookup field is selected, recalculate `dis-f` from its target-list metadata rather than preserving a copied source-template value. Before save and after readback, run the Dashboard hard gate; unresolved source/target lists, absent display fields, and mismatches all block generated-final handoff. For a changed or newly generated Lookup Dynamic field, report `apiAccepted`, `persistedReadback`, `designerOpen`, and `browserActionRuntime` separately. Designer-open must confirm that the control exposes the resolved Display field; browser smoke must render a row with an actual related target record. API save/readback alone is not display proof.
...
For new selection filters, plan the actual bound field and runtime-distinct options; do not enumerate or preselect options. Declare immediate reload dependencies or a valid apply button. Reset defaults include only staged filters. Keep the existing select-filter/Collection empty-value failure gate and validate the encoded resource with the runtime-contract validator. Preserve existing dependency-closure guidance.
