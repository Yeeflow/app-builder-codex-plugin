...
Use `event_portfolio_dashboard_golden_reference` as the default Dashboard Golden Reference construction style for polished operations, portfolio, status, pipeline, request, facility, service, project, vendor, and contract dashboards unless the Page Function Plan clearly requires another plugin-contained pattern. The default structure is registered in `docs/reference/dashboard-golden-reference-registry.normalized.json` and traces to `dashboard_default_shell_event_portfolio_ref`, `dashboard_header_band_event_portfolio_ref`, `dashboard_filter_group_event_portfolio_ref`, `dashboard_kpi_cards_event_portfolio_ref`, `dashboard_content_section_event_portfolio_ref`, and `dashboard_collection_grid_table_event_portfolio_ref`. Before blueprint generation, create a Dashboard Golden Reference Selection artifact for every Dashboard page, mapping each PFP business region to a selected reference section with source list, fields, filters, metrics, actions, and App Plan field mappings. Every Dashboard blueprint must reference that selection and every major section must declare `derivedFromGoldenReference`. Generated resources must preserve inspectable structure/provenance for the selected reference and include the required default regions. Treat the default as reusable structure and style only: adapt source lists, fields, filters, metrics, labels, actions, sorting/grouping, and mobile behavior to the current App Plan and Page Function Plan, and do not copy Marketing Event-specific fields or sample data into unrelated apps.
