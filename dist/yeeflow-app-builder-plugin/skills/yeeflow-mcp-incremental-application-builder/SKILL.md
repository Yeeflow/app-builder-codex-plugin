...
Build a live Yeeflow application as a resumable sequence of explicit, user-authorized changes. Treat the hosted `yeeflow_mcp` tool schemas and live contracts as authoritative; never invent operation names, payload fields, component types, shared-resource types, IDs, or GUIDs.
...
4. Materialize only the planned change through the type-specific generator or a contract-shaped generic materializer. Every page-shaped resource must first have an explicit App Plan layout choice and must be cloned from it: Data List and Document Library Forms (New/Edit/View/Detail/Print), Dashboards, Approval submission/task pages, and Public Forms. Preserve the selected page-layout provenance marker; a generic container, role/name inference, or silent default is a validation failure. Validate locally before saving. Application bootstrap validation includes workspace/title, FontAwesome `IconUrl`, identity strategy, and theme/update-contract gates.
...
Before `component_save`, run `scripts/lib/dashboard-live-component-resource-sync.mjs` through `classifyLiveDashboardResourceMode()`, `normalizeLiveDashboardDetail()`, and `validateLiveDashboardDetail()`. `LayoutInResources[].Resource` is runtime-authoritative; preserve the observed mode, including a valid blank `LayoutView`, instead of synchronizing or comparing `LayoutView` by default. Require the selected resource `ID = RefId = LayoutID`. For a Custom Code Dashboard, validate with `{ profile: "custom-code" }`: exactly one embedded Resource, `main -> content`, and one `codein` with non-empty `attrs["codein-script"]`. After save, get the component again and use `validateLiveDashboardPostSave(intended, readback, options)`; fail closed on `DASHBOARD_RESOURCE_NOT_MATERIALIZED`, `DASHBOARD_RESOURCE_ID_MISMATCH`, `DASHBOARD_ROOT_STRUCTURE_INVALID`, `DASHBOARD_CODEIN_MISSING`, or `DASHBOARD_POSTSAVE_DRIFT`. Persisted readback is not browser-action proof.
