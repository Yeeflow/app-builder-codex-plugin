...
The Yeeflow App Plan must be generated only from the reviewed Functional Specification and must follow `docs/standards/app-plan-standard-template.md`. The App Plan converts business requirements into Yeeflow-supported resources and defines the resource generation order. It is the Yeeflow resource generation contract, not a free-form project plan, generic project plan, page design document, or ad hoc script plan.

Keep the document responsibility split strict. The Functional Specification describes Dashboard/page needs from the business perspective only: roles, business jobs, information and actions needed, filtering/grouping/sorting priorities, mobile needs, and access requirements. It must not select concrete Dashboard template IDs or golden reference IDs. The Yeeflow App Plan declares Dashboard pages as Yeeflow resources and must reference the matching Page Function Plan entry with `pageFunctionPlanRef`, `dashboardFunctionRef`, or an equivalent stable reference ID. It must not embed Dashboard section/control/template details or Dashboard golden-reference selection.

The Page Function Plan must be generated from the reviewed Functional Specification and Yeeflow App Plan before Application Design System, full-page canonical design images, page implementation blueprints, page/resource generation, decoded resource-vs-blueprint parity, package generation, signing, install/import/upgrade, or runtime proof. It must cover every UI-required Dashboard page, Approval submission form, planned Approval task form, required Approval print page, and planned custom Data list or Document library form. Form Reports are not required canonical UI design surfaces. The Page Function Plan defines page-level function, content, controls, data source, fields, actions, layout intent, desktop/mobile behavior, and App Plan traceability by stable App Plan names. It prevents dashboard, approval-form, task-form, print-page, data-list-form, and document-library-form generation from guessing what content or actions belong on the page.

Treat the Page Function Plan as the canonical page-level implementation contract after the App Plan. Every App Plan Dashboard page, Approval submission/task/print surface, and custom Data list/Document library form must reference one Page Function Plan entry, and every Page Function Plan entry must map back to one App Plan resource/surface. Approval submissions must carry field controls, editable/read-only state, required/default/dynamic/validation behavior, sub lists, approved form actions, and Save as draft/Submit buttons. Task forms must state task-specific actions such as Approve/Reject or Complete plus field-state differences. Print pages must be print-specific and omit unsupported interactive controls. New/Edit list/library forms must stay scoped to current-resource fields plus Save/Cancel unless an App Plan-supported exception is explicit. View/detail/custom forms may include related regions only with source, current-item binding, fields, filters, actions, and opening behavior. Document library forms must include metadata, upload/edit, preview, and view behavior where applicable.

The Page Function Plan must also define structured click action metadata for every visible interactive `Container`, `Button`, or `action_button`. Keep `actions` as stable business action names for App Plan traceability, and put generation-ready action details in `interactiveActions[]`, `clickActions[]`, `actionBindings[]`, or equivalent structured fields. Use only plugin-known Container/Button action mappings from `docs/studies/container-button-action-settings.md`, `docs/studies/container-button-action-runtime-proof.md`, `scripts/inspect-container-button-actions.mjs`, and `docs/studies/normalized/container-button-actions/`: `action-type: "2"` Link, `action-type: "5"` Add list item, `action-type: "6"` Open dashboard, `action-type: "8"` Open approval form, and `action-type: "1"` Form action binding only where the host/form-action shape is supported or explicitly proof/deferred. Every planned interactive action must declare control label, control type, interaction purpose, action type/code, target resource type/name, target identifiers such as `ListID`, `PageID`, `ProcKey`, `LayoutID`, or form action ID, open mode, modal/custom size if needed, passValues/queryParams/setVars if needed, proof status, and fallback/deferred reason when unsafe. Resource generation must consume these Page Function Plan action definitions and must not generate visible action-looking Containers or Buttons without action metadata; if a safe action cannot be generated, defer it or render it as non-interactive text with a reason.

Dashboard Page Function Plan entries must also select trained Dashboard templates instead of inventing page structure. Each Dashboard page must declare structured `pageFunctionPlanId`, `appPlanDashboardRef`, `dashboardPagePattern`, `dashboardGoldenReference`, and `dashboardSectionTemplates[]`. Section template IDs must come from `docs/templates/yeeflow-ui-section-template-library.normalized.json` or documented Dashboard standards, such as `dashboard_header_action_bar`, `kpi_card_row`, `progress_summary_card`, `business_alert_card`, `data_table_section`, `kanban_status_board`, `collection_card_board`, `quick_links_icon_list`, `recent_activity_timeline`, or `three_column_workspace_shell`. `standard_dashboard_page_shell` is the documented standard Dashboard page shell equivalent for `dashboardPagePattern`. Dashboard generation must consume these template selections as part of the implementation contract and must not infer sections only from the App Plan resource list when a Page Function Plan exists. KPI/summary metrics should map to `kpi_card_row` or another existing KPI/Summary template; work queues should map to `collection_card_board`, `data_table_section`, `kanban_status_board`, or `three_column_workspace_shell` when appropriate; quick actions should map to `quick_links_icon_list`; alerts should map to `business_alert_card`; and activity/history should map to `recent_activity_timeline`. Do not use `three_column_workspace_shell` for simple dashboards without meaningful left/main/right panel content. Do not make `multi_column_form_workspace_shell` the default for normal dashboards.

Dashboard Page Function Plan entries that declare high-quality, Marketing Event, Event Portfolio, portfolio/status, operational-table, rich table, or runtime-fidelity intent must also carry the plugin-contained fidelity lessons into generation. The Page Function Plan must declare applicable Marketing Event/Event Portfolio fidelity references, KPI/Summary binding requirements, Data Filter requirements, Collection grid-table requirements, badge/progress/avatar/person treatment, real Yeeflow action metadata, KPI formatting, semantic `nv_label`/designer traceability, and the runtime proof boundary. For Event Portfolio-style portfolio/status/operational tables, generation must produce rich table/card treatment rather than plain scaffold tables, render status/progress/person fields with the supported Yeeflow treatments where applicable, preserve table header hierarchy and row density, and use Collection grid-table style only when source list, fields, row context, and detail/open actions are specified. Use only plugin-contained standards, studies, and references; do not depend on private Marketing Event artifacts.

When the business need is a high-quality portfolio, operations, status, pipeline, event, project, vendor, contract, service, or request management Dashboard, the Page Function Plan may select `dashboardGoldenReference: event_portfolio_dashboard_golden_reference`. This is a whole-Dashboard golden reference, not a decorative style label. If selected, generation must consume the full contract: page purpose, source data lists, Data Filter controls and filter variables, KPI cards, Summary/KPI binding or fallback boundary, Collection grid-table sections, Collection source and item context, Dynamic controls inside Collection item templates, badge/progress/person treatments, table header hierarchy, row density and spacing, detail/open actions with real Yeeflow action metadata, semantic `nv_label`/designer traceability, and runtime proof boundary. Do not satisfy this reference with generic cards, static KPI values, plain Data tables, scaffold sections, placeholder controls, or action-looking visuals without real metadata.

Dashboard Page Function Plan sections that use visible Text controls must declare structured `textStyleContract[]` entries before page blueprint/resource generation. Cover important roles such as page title, section title, KPI label, KPI value, helper text, table/collection headers, table/collection cell text, badge text, action labels, empty-state text, and note/description text. Each entry must define text role, business purpose, content source, static text or dynamic binding details, expected native heading/Text control shape, typography token, text color token, supported font weight/style or proof/deferred boundary, width behavior, semantic `nv_label`, and proof status. Dashboard Text generation must use the plugin-supported native `heading` / label `Text` pattern with `attrs.headc.title.value` or `attrs.headc.title.variable[]`, `attrs.heads.ty`, string-token `attrs.heads.color`, and supported width behavior. Do not generate unsupported ad hoc `type: "text"` controls, generic placeholders such as `Title` or `Description`, static mock values for dynamic KPI/runtime content, unsupported color shapes, or visually indistinguishable KPI/table/section text roles. If a typography, color, or style property is not proven by plugin references, mark it `export-learning-required`, `runtime-proof-required`, or deferred instead of emitting invented Yeeflow properties.

The Application Design System must be generated from the Functional Specification, App Plan, and Page Function Plan before page/resource generation. It defines the overall application visual style, selected Yeeflow application layout, page density and spacing principles, typography hierarchy, button/action styling intent, form layout conventions, dashboard/card/table/list conventions, mobile layout principles, and cross-page consistency rules. It guides later page design and resource generation without inventing unsupported Yeeflow properties. Every Application Design System must select exactly one supported Yeeflow `applicationLayoutType`: `application-layout-1-vertical-nav`, `application-layout-2-horizontal-nav`, `application-layout-3-header-nav`, or `application-layout-4-no-nav`. It must also declare structured `selectedApplicationLayout`, `applicationChromeStyleId`, `headerMode`, `navigationMode`, `navigationPanelMode`, `contentSafeArea`, `dashboardChromeRules`, and `formSurfaceChromeRules`. It must define app-wide `applicationChrome` settings for header, navigator menu, and content area before Dashboard/application page generation: header background/text/icon/title typography, navigator background/text/icon/hover/active/selected/group styles, and content area background. `applicationChrome` must keep `designIntent`, `supportedGeneratedProperties`, `exportLearningRequired`, `runtimeProofRequired`, and `deferredProperties` separate. Use Yeeflow tokens such as `var(--c--primary-light)`, `var(--c--primary)`, `var(--c--background)`, `var(--c--text)`, and `var(--c--text-normal)` where possible. Resource generation may only write plugin-known/export-proven app shell paths currently represented by `LayoutView.attrs.appearance.bgc`, `LayoutView.attrs.appearance.color`, `LayoutView.attrs["navigator-menu"].bgc`, `LayoutView.attrs["navigator-menu"].color`, and `LayoutView.attrs["navigator-menu"].position`. Header title typography, icon styling, hover/active/selected navigator states, and other chrome polish remain design intent unless a plugin-contained standard, normalized reference, or export-proven registry lists the exact generated property path; otherwise mark them `export-learning-required`, `runtime-proof-required`, or `deferred` and do not emit them into generated Yeeflow resources. Dashboard/application pages inherit this selected app-level layout and chrome and must include header/navigation/content-safe-area expectations. Approval forms and Data list/Document library forms are form surfaces and must not invent app header/navigation unless explicitly plugin-supported or deferred with proof boundary. Do not invent arbitrary sidebars, custom nav bars, floating navigation, custom top bars, unsupported app shells, or per-page Dashboard chrome overrides.
...
Do not proceed to Application Design System, full-page canonical design images, page implementation blueprints, Yeeflow resource/package generation, decoded resource-vs-blueprint parity, package/sign/upgrade, or runtime proof until the Functional Specification review gate, App Plan review gate, Page Function Plan review gate, Business Clarification Gate, Generation Readiness Review, Functional Specification to App Plan traceability gate, and App Plan to Page Function Plan traceability gate pass. Use `scripts/validate-functional-specification.mjs`, `scripts/validate-app-plan-resource-order.mjs`, `scripts/validate-page-function-plan.mjs`, `scripts/validate-business-clarification-gate.mjs`, `scripts/validate-generation-readiness-review.mjs`, `scripts/validate-functional-spec-to-app-plan-traceability.mjs`, and `scripts/validate-app-plan-page-function-traceability.mjs` for executable gate checks when the artifacts are available. These gates prove planning and page-function readiness only; they do not prove package validity, signing/API acceptance, install/upgrade success, visual fidelity, or runtime behavior. Do not let temporary scripts or ad hoc generation logic bypass plugin knowledge, templates, validators, or export-proven references.
...
Generated-final `.yapk` output must use API-issued numeric content IDs from `GET /utils/generate/ids?count=<n>` and must emit a redacted `dist/<app-name>-id-provenance-report.json` with `sourceMarker: "api-generated"`, path-to-purpose mappings, duplicate checks, unused-ID accounting, generator provenance metadata, and no non-API IDs. Planning and Blueprint stages use logical refs; optional local-preview/local-draft packages are local-only and must never be promoted into generated-final by patching/replacing IDs. Generated-final generation is a fresh ID-first build from approved logical refs plus a complete `logicalRef -> apiIssuedId` map. Local ID generation, hardcoded generated IDs, copied sample/export IDs, random values, timestamps, UUID fallback, and deterministic local-only seeds are forbidden for generated-final `.yapk`. After API ID allocation, finalization must recursively replace nested runtime-bearing local draft placeholders and unresolved logical references, and generated-final packages must contain no `local-draft`, `localDraft`, `local-draft-*`, `sourceMarker: local-draft-no-api`, equivalent draft sentinel, or unresolved logical reference in decoded package JSON, parsed page resources, form `Ext`/`DefResource`, control bindings, table/action links, open-dashboard/open-form targets, workflow/navigation metadata, theme payloads, arrays, or version fields. Runtime navigation groups must include API-issued `ID`, `AppID`, `ListSetID`, `Type: "classes"`, `Title`, `Icon`, and `list[]`; children must include `AppID`, `Title`, `ListID`, `ListSetID`, and `Type`, with dashboards/pages as `Type: 103` plus `LayoutID`, approval forms as `Type: 105`, and data lists as `Type: 1`. Run `scripts/validate-yapk-id-provenance.mjs`, `scripts/validate-generated-final-draft-placeholders.mjs --mode generated-final`, and `scripts/validate-yapk-navigation-runtime-metadata.mjs`; stop before signing, install, upgrade-check, or handoff if any gate fails. `setsign`/`verifysign` and install acceptance do not prove ID provenance, recursive placeholder/logical-ref replacement, or navigation runtime metadata completeness.
...
The app plan must cover the standard template sections in order: Plan Status, Application Purpose, Target Users and Roles, Business Process Overview, Capability Coverage Plan, Application Navigation, Data Model and Lists, Forms and Approval Forms, Dashboards/Pages/Reports, UI/UX and Control Mapping, Golden Template and Reference Strategy, Actions and Workflow Logic, Permissions, Integrations and External Services, Document Libraries and Attachments, Reports and Analytics, Data Validation Checklist, Generation Contract and Hard Gates, Generation Validation Plan, Proof Boundary, Assumptions, Deferred or Runtime-Proof Items, and Recommended Next Prompt.
...
Generation must not proceed until the Functional Specification completeness gate, App Plan review gate, Page Function Plan review gate, Application Design System review gate, Business Clarification Gate, Generation Readiness Review, Functional Specification to App Plan traceability gate, App Plan to Page Function Plan traceability gate, and App Plan control/action/property gates pass, or the user explicitly authorizes direct generation from the visible reviewed Functional Specification, App Plan, Page Function Plan, Application Design System, and assumptions while accepting the documented planning-readiness risks.
