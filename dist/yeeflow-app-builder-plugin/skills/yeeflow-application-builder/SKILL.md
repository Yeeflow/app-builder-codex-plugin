...
Standard operational dashboards must preserve the Event Portfolio golden-reference structure: `Main > Content`, clean header band, horizontal filter/action row, KPI card row, section header/actions, and grid-table Collection content area. Adapt colors, tokens, FontAwesome icons, source lists, fields, metrics, filters, and actions to the current application domain; do not copy Marketing Event field names into unrelated apps. Dashboard blueprints and generated resources must keep machine-checkable `derivedFromGoldenReference` provenance for the selected shell and major sub-regions.
