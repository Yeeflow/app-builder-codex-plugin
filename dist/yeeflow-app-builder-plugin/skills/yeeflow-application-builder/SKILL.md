...
Dashboard filters are generated with the same copy-then-map discipline. For every planned Dashboard filter, copy an approved v1.1/Event Portfolio filter module into an approved v1.1 business-content slot such as `section_content_area`, then map `attrs.data.list`, `attrs.data.field`, `display_f`, `value_f`, placeholder text, label text, and consumer filter linkage to the app domain. Do not insert ad hoc filter containers, do not copy `event_portfolio_filter_group` directly under root `Content`, and do not invent new filter layout modules. Preserve label typography `attrs.lab.ty = [null, "xs-light"]`, tuple label layout `attrs.lablay = [null, "top"]`, placeholder color, supported radius, fixed-width positioning with custom width 200, and canonical v1.1 `Content` padding before generated-final preflight.
