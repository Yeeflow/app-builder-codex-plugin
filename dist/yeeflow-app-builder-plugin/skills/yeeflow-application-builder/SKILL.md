...
Dashboard Page Function Plan entries must also select trained Dashboard templates instead of inventing page structure. Each Dashboard page must declare a structured `dashboardPagePattern` and structured `dashboardSectionTemplates[]`. Section template IDs must come from `docs/templates/yeeflow-ui-section-template-library.normalized.json` or documented Dashboard standards, such as `dashboard_header_action_bar`, `kpi_card_row`, `progress_summary_card`, `business_alert_card`, `data_table_section`, `kanban_status_board`, `collection_card_board`, `quick_links_icon_list`, `recent_activity_timeline`, or `three_column_workspace_shell`. `standard_dashboard_page_shell` is the documented standard Dashboard page shell equivalent for `dashboardPagePattern`. Dashboard generation must consume these template selections as part of the implementation contract. KPI/summary metrics should map to `kpi_card_row` or another existing KPI/Summary template; work queues should map to `collection_card_board`, `data_table_section`, `kanban_status_board`, or `three_column_workspace_shell` when appropriate; quick actions should map to `quick_links_icon_list`; alerts should map to `business_alert_card`; and activity/history should map to `recent_activity_timeline`. Do not use `three_column_workspace_shell` for simple dashboards without meaningful left/main/right panel content. Do not make `multi_column_form_workspace_shell` the default for normal dashboards.
