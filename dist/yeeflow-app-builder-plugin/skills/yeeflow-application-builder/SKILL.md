...
Approval form pages must select approved Approval Form Layouts v1.1 templates in the App Plan before generation. Use `docs/reference/approval-form-layout-templates.json`. The single submission page must select `approval_form_layout_submission_v1_1`; every task page must select `approval_form_layout_task_v1_1`. Generated approval pages must clone the selected `pageurls[].formdef` template and may edit only `page_title_content`, `Operations`, `section_content_area`, and `section_title_header`. Preserve root background, root zero padding, hidden form-title CSS, `main > content`, `content` custom width 1280, page title section, all content-card section shells, and the locked `action_panel_flow_history_wrapper` with Action panel plus workflow history controls. Approval forms must not use Data Analytics templates, charts, pivot tables, Summary/KPI analytics, or `kpi_metrics_wrapper`.
