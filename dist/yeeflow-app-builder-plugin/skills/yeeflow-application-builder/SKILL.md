...
After planning gates pass and business decisions are answered or `user-default-approved-for-generation`, run the pattern-library UI generation stage before Page Implementation Blueprints. This default stage must create an Application Design System with `docs/standards/application-design-system-template.md`, select approved Yeeflow UI Section Templates from `docs/templates/yeeflow-ui-section-template-library.normalized.json`, and produce a pattern-selection validation/review report. Generate the Application Design System before template selection, and require every selected surface template and Page Implementation Blueprint to reference it. The Application Design System must use one official Yeeflow `applicationLayoutType` from `docs/standards/yeeflow-application-layout-design-rules.md`, must include `applicationChromeStyleId`, `headerMode`, `navMode`, `navBackgroundMode`, `contentSafeArea`, and `layoutRuleSource`, and must reject arbitrary custom SaaS shells or free-form layout names. Dashboard surfaces must select dashboard templates; Approval Submission/Task/Print surfaces must select approval/form/print-appropriate templates; Data List and Document Library New/Edit surfaces must use form-body templates for primary editable fields; related-record, Sub List, Collection, Kanban, and Timeline sections must use matching item-template or collection-control patterns. Run `scripts/validate-ui-pattern-selection.mjs --selection <pattern-selection.json>` before Page Implementation Blueprints, then run `scripts/validate-blueprint-ui-pattern-conformance.mjs --blueprint <page-implementation-blueprint.json> --pattern-selection <pattern-selection.json>` before Yeeflow resource generation. These gates prove pattern/blueprint readiness only; they do not prove Yeeflow resource serialization, package validity, signing/API acceptance, install/upgrade success, or runtime behavior.

Generated PNG design images and HTML previews are optional review evidence, not default implementation sources. Use PNG canonical design artifacts only when the user explicitly requests visual review artifacts. Use the HTML-first high-fidelity UI Surface Contract workflow only when the user explicitly asks for HTML previews, screenshots, or prototype evidence. Optional HTML previews must be generated from UI Surface Contracts, the Application Design System, and selected Yeeflow UI templates; they must use design tokens and approved UI pattern templates, render desktop/mobile variants, and be polished modern business UI rather than low-fidelity scaffolds. Capture desktop/mobile screenshots from validated HTML previews when that optional evidence flow is requested; screenshots are evidence, not the primary implementation contract. Run `scripts/validate-ui-surface-contracts.mjs`, `scripts/validate-html-preview-layout.mjs`, and `scripts/compare-blueprint-to-ui-surface-contract.mjs` for the optional HTML flow. Do not proceed to Page Implementation Blueprints if selected template validation fails, and do not proceed to Yeeflow resource/package generation if blueprint pattern conformance or optional blueprint-to-contract parity fails. Keep proof boundaries separate: pattern validation, contract validation, HTML/DOM validation, screenshot evidence, blueprint parity, package schema, signing/API acceptance, install/upgrade success, and runtime proof are separate proof layers.
