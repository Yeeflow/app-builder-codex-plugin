...
Before dashboard page blueprints or resources are generated, create a Dashboard Golden Reference Selection artifact for every planned dashboard page. The default operational dashboard construction reference is `event_portfolio_dashboard_golden_reference` from `docs/reference/dashboard-golden-references.json`; use its export-shaped `_ak_c` / `_ak_c_opt` tree, header, filter group, KPI wrapper, KPI row, pipeline section, grid-table Collection region, campaign/readiness section, and secondary grid-table container unless an explicit justified alternative is selected. The selection maps App Plan/Page Function Plan business requirements to app-specific source lists, KPI fields, filters, grid/table fields, and actions, while keeping Page Function Plan business-oriented and avoiding low-level control properties. Blueprint/resource generation must clone and normalize the selected export tree before domain mapping; semantic shells or provenance-only reconstructed structures are not acceptable.
