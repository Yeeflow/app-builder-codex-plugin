...
The App Plan must also include a Data List Form Layout Template Selection table for each custom Data List form. New/Edit form rows must select `data_list_form_layout_new_edit_v1_1`; View form rows must select `data_list_form_layout_view_item_v1_1`. It must also include a Form Fields Layout Template Selection table for each generated current-record field group, selecting `data_list_form_fields_grid_v1_1` and declaring PC/laptop, tablet, and mobile column counts plus any full-row field controls. The App Plan should describe business sections and related data needs, but it must not contain generated `ListID`, `LayoutID`, `FieldID`, JSON payloads, or low-level runtime properties. Run `scripts/validate-data-list-form-layout-template.mjs --plan <yeeflow-app-plan.md>` and `scripts/validate-data-list-form-fields-template.mjs --plan <yeeflow-app-plan.md>`; keep generation blocked when selected templates or responsive field-grid rules do not match the form usage.
