...
Data List creation is incomplete when it has fields alone. Before handoff or generated-final release, run `node scripts/validate-data-list-completion-contract.mjs <app.yapk|decoded.json>`: every Type 1 Data List must have a default Type 0 view plus same-list Type 1 New, Edit, and View forms, and `ListModel.LayoutView.add/edit/view` must resolve to those concrete form layouts. API save acceptance or field readback does not substitute for this structural completion contract.
