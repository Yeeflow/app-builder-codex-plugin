...
- Workspace add/edit/delete/sort capabilities are mutating and must remain blocked by generic read-only helpers.
...
Use `scripts/yeeflow-workspace-list.mjs` for workspace discovery. It calls only documented `GET /workspaces/{category}` and reports count, title, category, status, and redacted ID previews without saving raw responses.
