...
For Markdown App Plans, the conformance validator must treat only explicit canonical resource declarations as planned resources. Valid resource declarations come from the canonical requirement-to-resource mapping table, the Resource Generation Order table, recognized top-level resource tables such as Form Reports or Schedule Workflows, and numbered Data List / Approval Form resource subsections. Field rows, control rows, action detail rows, task-form rows, implementation notes, assumptions, validation notes, deferred/runtime-proof sections, template headings, and explanatory prose are not standalone generated resources. Strict mode still fails real missing resources, extra generated resources, and partial resource-name mismatches when those resources are declared through the canonical plan contract.
