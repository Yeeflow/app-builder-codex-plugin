...
Dashboard-only upgrades of an existing complete application must be emitted as full upgrade packages, not sparse Dashboard-only packages. The package must preserve Data Lists, Forms, FormNewReports, DataReports, Groups, Tags, Metadatas, Agents, Connections, Knowledges, Themes, Components, PortalInfo, root identity, and non-target pages unless explicitly declared in scope. Scope proof must include a non-Dashboard unchanged diff artifact before signing readiness, and runtime proof must confirm Data Lists and Forms were not deleted.
