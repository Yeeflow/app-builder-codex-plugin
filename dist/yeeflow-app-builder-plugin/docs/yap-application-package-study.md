...
Canonical schema file: `schemas/yap-schema.json`.

Keep this separate from YAPK validation, which uses `schemas/yapk-schema.json`. YAP packages use the YAP wrapper, `[______gizp______]` gzip `ListExportResult`, `Defs`, and `SimplePortal: null`.
...
### Child Data List `Title` Field Runtime Rule

Generated `.yap` child data lists must keep Yeeflow's native `Title` field metadata. The business display label can change, but the storage field must remain the native primary/display field:
...
Do not treat `Title` as a normal generated business field with `Status: 1`, `IsSystem: false`, or `IsIndex: false`. Heep Hong IT eWorkflow Option A v7 showed that this bad metadata can still allow import, navigation, list metadata loading, definitions loading, layouts loading, and grid headers. The row query endpoint still fails:

```text
api/crafts/datas/{AppID}/{ListID}/query -> 400
```

Heep Hong Option A v8 restored the native `Title` pattern on all four child data lists and fixed the runtime data-grid query issue. Generators should map conceptual fields such as `Request No.`, `Name`, or `Equipment Name` to `Title` only while preserving native metadata; other business fields should use `Text1`, `Text2`, etc.
