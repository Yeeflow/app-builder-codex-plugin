...
Expected plugin: `Yeeflow App Builder` version `0.8.5`.
...
```sh
node scripts/inspect-codex-plugin-cache-metadata.mjs --root . --expect-version 0.8.5
...
New Dashboard pages must start from Dashboard Page Layouts v1.1 (`docs/reference/dashboard-page-layout-templates.json`) as the page shell and may map Event Portfolio Golden Reference component regions into `section_content_area` containers. Validate the page shell and section contract with:
