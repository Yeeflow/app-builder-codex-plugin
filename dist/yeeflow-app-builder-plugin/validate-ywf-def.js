...
function validateDecodedDef(def, options = {}) {
  ...
  function validatePagesAndControls() {
    ...
    pages.forEach((page, pageIndex) => {
      ...
      if (page && page.id) pageById.set(page.id, { page, pagePath });
  ...
  function validateTaskUrls() {
    childshapes.forEach((shape, index) => {
      ...
      if (type === "MultiAssignmentTask" || type === "CandidateTask") {
        ...
        const taskPageEntry = pageById.get(taskurl);
        const taskPage = taskPageEntry && taskPageEntry.page;
        const taskPagePath = taskPageEntry ? taskPageEntry.pagePath : "$.pageurls";
        if (!taskPage) {
          addIssue(errors, "WORKFLOW_TASKURL_PAGE_NOT_FOUND", `${label}.properties.taskurl must resolve to a pageurls[] entry`, p, { taskurl });
          return;
        }
        if (!approvalPageIds.has(taskurl)) {
          addIssue(errors, "WORKFLOW_TASKURL_PAGE_TYPE_INVALID", `${label}.properties.taskurl must reference a pageurls[] entry with type = 2`, taskPagePath, { taskurl, pageType: taskPage.type });
          addIssue(errors, "APPROVAL_TASKURL_BAD_PAGE", `${label}.properties.taskurl must reference an approval task page id`, p);
        }
        if (taskPage && taskPage.type === 2 && taskPage.pagetype !== 1) {
          addIssue(errors, "WORKFLOW_TASK_PAGEURL_PAGETYPE_INVALID", "Assignment task pageurls[] entries must use pagetype=1; only embedded formdef.pagetype should be 2.", `${taskPagePath}.pagetype`, { taskurl, pagetype: taskPage.pagetype });
          addIssue(errors, "APPROVAL_TASK_PAGE_OUTER_PAGETYPE_INVALID", `${label} referenced task page must use outer pagetype = 1`, taskPagePath, { taskurl, pagetype: taskPage.pagetype });
        }
        if (taskPage && taskPage.type === 2 && taskPage.formdef && taskPage.formdef.pagetype !== 2) {
          addIssue(errors, "WORKFLOW_TASK_FORMDEF_PAGETYPE_INVALID", "Assignment task embedded formdef.pagetype must use 2.", `${taskPagePath}.formdef.pagetype`, { taskurl, formdefPagetype: taskPage.formdef.pagetype });
        }
