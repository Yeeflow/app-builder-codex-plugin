...
import { loadStoredToken, resolveOAuthConfig, summarizeStoredToken } from "./lib/yeeflow-oauth-client.mjs";

if (isMainModule()) {
  const args = parseArgs(process.argv.slice(2));
  loadDotenvFile(fs, args.dotenv || ".env.local");
  const env = resolveYeeflowEnvironment(process.env);
  const oauth = summarizeStoredToken(resolveOAuthConfig(process.env), loadStoredToken(resolveOAuthConfig(process.env)));
  const decision = decideApplicationDelivery({
    requestType: args.requestType || "new-app",
    explicitPackageType: args.packageType || "",
    apiAuthPresent: Boolean(oauth.authenticated || env.apiKey),
    targetWorkspaceSelected: Boolean(env.workspaceId || args.workspaceId || args.selectedWorkspaceId),
...
export function decideApplicationDelivery(input = {}) {
  ...
  const apiAuthPresent = Boolean(input.apiAuthPresent ?? input.apiKeyPresent);
  const targetWorkspaceSelected = Boolean(input.targetWorkspaceSelected ?? input.workspaceIdPresent);
  ...
  if (requestType === "existing-app-change") {
    const upgradeReady = apiAuthPresent && targetWorkspaceSelected && targetConfirmed && autoUpgradeConfirmed;
    return {
      requestType,
      packageType: "yapk",
      ...
      recommendedNextStep: upgradeReady
        ...
        : apiAuthPresent && targetWorkspaceSelected
  ...
  const canAskAutoInstall = packageType === "yapk" && apiAuthPresent && targetWorkspaceSelected;
