...
const tempVariableDashboard = decodedDashboard({ filter: { summaryConsumes: true, consumer: true } });
const tempVariablePage = JSON.parse(tempVariableDashboard.Pages[0].LayoutInResources[0].Resource);
tempVariablePage.tempVars = [{ id: "var_open_resources", name: "var_open_resources", idx: "temp-open", type: "number" }];
tempVariablePage.children[0].children.push({ id: "kpi_open_resources", type: "heading", attrs: { headc: { title: { variable: [{ exprType: "variable", id: "__temp_var_open_resources", name: "var_open_resources" }] } } } });
tempVariableDashboard.Pages[0].LayoutInResources[0].Resource = JSON.stringify(tempVariablePage);
assert.equal(validateDashboardBindings(tempVariableDashboard).filter((finding) => finding.severity === "error").length, 0, "A semantic temp declaration and a single __temp_ runtime expression must close correctly");
const prefixedTempDeclarationDashboard = structuredClone(tempVariableDashboard);
const prefixedTempPage = JSON.parse(prefixedTempDeclarationDashboard.Pages[0].LayoutInResources[0].Resource);
prefixedTempPage.tempVars[0] = { ...prefixedTempPage.tempVars[0], id: "__temp_var_open_resources", name: "__temp_var_open_resources" };
prefixedTempDeclarationDashboard.Pages[0].LayoutInResources[0].Resource = JSON.stringify(prefixedTempPage);
expectCode("temp declaration must not include runtime prefix", validateDashboardBindings(prefixedTempDeclarationDashboard), "DASHBOARD_TEMP_VAR_ID_SYSTEM_PREFIX");
const unresolvedTempReferenceDashboard = structuredClone(tempVariableDashboard);
const unresolvedTempPage = JSON.parse(unresolvedTempReferenceDashboard.Pages[0].LayoutInResources[0].Resource);
unresolvedTempPage.children[0].children.at(-1).attrs.headc.title.variable[0].id = "__temp_var_missing";
unresolvedTempPage.children[0].children.at(-1).attrs.headc.title.variable[0].name = "var_missing";
unresolvedTempReferenceDashboard.Pages[0].LayoutInResources[0].Resource = JSON.stringify(unresolvedTempPage);
expectCode("unresolved temp runtime reference", validateDashboardBindings(unresolvedTempReferenceDashboard), "DASHBOARD_TEMP_VAR_REFERENCE_UNRESOLVED");
const doubledTempReferenceDashboard = structuredClone(tempVariableDashboard);
const doubledTempPage = JSON.parse(doubledTempReferenceDashboard.Pages[0].LayoutInResources[0].Resource);
doubledTempPage.children[0].children.at(-1).attrs.headc.title.variable[0].id = "__temp___temp_var_open_resources";
doubledTempPage.children[0].children.at(-1).attrs.headc.title.variable[0].name = "__temp_var_open_resources";
doubledTempReferenceDashboard.Pages[0].LayoutInResources[0].Resource = JSON.stringify(doubledTempPage);
expectCode("double-prefixed temp runtime reference", validateDashboardBindings(doubledTempReferenceDashboard), "DASHBOARD_TEMP_VAR_DOUBLE_RUNTIME_PREFIX");
