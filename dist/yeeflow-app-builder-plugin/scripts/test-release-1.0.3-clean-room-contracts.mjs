...
assert.equal(summary.attrs.save_var.id, `__temp_${summary.attrs.save_var.name}`);
assert.ok(pageResource.tempVars.some((variable) => variable.id === summary.attrs.save_var.name && variable.name === summary.attrs.save_var.name));
