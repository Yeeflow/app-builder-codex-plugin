...
console.log(JSON.stringify({ status: "pass", test: "test-form-action-query-data-plan-gates.mjs", cases: 15 }, null, 2));
...
function row(action, step, host, trigger, mode, resultType, result, mapping, countType, count, notes = "Test", pageSize = 100, pageNumber = 1) {
  ...
  return `| ${action} | ${step} | ${hostResource} | ${hostForm} | ${host} | ${trigger} | Query Data | ${mode} | Data List | Event Planning | Status = Confirmed | Date desc | ${resultType} | ${result} | ${mapping} | ${countType} | ${count} | ${pageSize} | ${pageNumber} | Current page session | None | export-proven | ${notes} |`;
}
