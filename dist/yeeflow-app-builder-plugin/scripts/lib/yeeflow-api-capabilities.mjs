...
export const YEEFLOW_API_CAPABILITIES = [
  ...
  cap("workspaces.listByCategory", "GET", "/workspaces/{category}", "Get workspaces by category", true, ["path:category"], [], "Apifox/OpenAPI", "OAuth read-only workspace discovery for target selection. Documented categories are settings and flowcraft; app/package workflows use flowcraft. Summarize title/displayName/category/status/count and redacted ID previews only.", "oauth"),
  cap("workspaces.get", "GET", "/workspaces/{category}/{id}", "Get workspace by id", true, ["path:category", "path:id"], [], "Apifox/OpenAPI", "Read-only workspace lookup by category and id. Do not print raw workspace objects, tenant IDs, private URLs, or full workspace IDs.", "oauth"),
  cap("workspaces.applications.list", "GET", "/workspaces/{category}/{id}/applications", "Get workspace applications", true, ["path:category", "path:id"], ["query:appID"], "Apifox/OpenAPI", "Read-only application discovery scoped to one workspace. appID supports only 30 or 41 and defaults to 41. Summarize application title/type/status/count and redacted application ID previews only.", "oauth"),
  cap("workspaces.add", "POST", "/workspaces/{category}", "Add workspace", false, ["path:category", "body:workspace"], [], "Apifox/OpenAPI", "Workspace mutation. Out of scope for automation; require explicit admin confirmation if ever implemented."),
  cap("workspaces.edit", "PUT", "/workspaces/{category}/{id}", "Edit workspace", false, ["path:category", "path:id", "body:workspace"], [], "Apifox/OpenAPI", "Workspace mutation. Out of scope for automation; require explicit admin confirmation if ever implemented."),
  cap("workspaces.delete", "DELETE", "/workspaces/{category}/{id}", "Delete workspace", false, ["path:category", "path:id"], [], "Apifox/OpenAPI", "Destructive workspace mutation. Out of scope for automation; require explicit strong destructive confirmation if ever implemented.", CAPABILITY_AUTH, { confirmationLevel: "strong" }),
  cap("workspaces.sort", "POST", "/workspaces/{category}/sort", "Sort workspaces", false, ["path:category", "body:sort"], [], "Apifox/OpenAPI", "Workspace sort mutation. Out of scope for automation; require explicit admin confirmation if ever implemented."),

  cap("applications.delete", "DELETE", "/applications/{id}", "Delete application by id", false, ["path:id"], ["query:appID"], "Apifox/OpenAPI", "Destructive application deletion. appID supports only 30 or 41 and defaults to 41. Use only the dedicated guarded helper after workspace-scoped application readback, exact application ID and title matching, and explicit strong confirmation. Never execute from the generic capability caller.", CAPABILITY_AUTH, { confirmationLevel: "strong" }),

  cap("positions.create", "POST", "/positions", "Add a position", false, ["body:position"], [], "OpenAPI", "Creates a job position. Require explicit user confirmation and confirmed system-admin permission; never run automatically during workflow generation.", CAPABILITY_AUTH, { confirmationLevel: "admin" }),
  cap("positions.list", "GET", "/positions", "Get positions", true, [], [], "OpenAPI", "Read-only position lookup."),
  cap("positions.update", "PUT", "/positions/{id}", "Update a position", false, ["path:id", "body:position"], [], "OpenAPI", "Updates a job position. Require explicit user confirmation and confirmed system-admin permission; never run automatically during workflow generation.", CAPABILITY_AUTH, { confirmationLevel: "admin" }),
  ...
  cap("positions.users.assign", "POST", "/positions/{id}/users", "Assign users to a position", false, ["path:id", "body:assignment"], [], "OpenAPI", "Changes job-position user assignments. Require explicit user confirmation and confirmed system-admin permission.", CAPABILITY_AUTH, { confirmationLevel: "admin" }),
  ...
  cap("positions.users.remove", "POST", "/positions/{id}/users/remove", "Remove users from a position", false, ["path:id", "body:assignment"], [], "OpenAPI", "Changes job-position user assignments. Require explicit user confirmation and confirmed system-admin permission.", CAPABILITY_AUTH, { confirmationLevel: "admin" }),
...
function cap(name, method, path, summary, readOnly, requiredParams, optionalParams, source, notes, auth = CAPABILITY_AUTH, options = {}) {
  return {
    name,
    ...
    confirmationLevel: options.confirmationLevel || (readOnly ? "none" : "standard"),
    auth,
    requiredParams,
    optionalParams,
    source,
