...
Canonical YAP schema validation uses `schemas/yap-schema.json`. Keep it separate from YAPK validation, which uses `schemas/yapk-schema.json`. Do not hardcode versioned schema filenames in runtime validation logic.
