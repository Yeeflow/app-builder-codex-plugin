...
| Generation mode | Yes | Draft / Offline Mode is local unsigned draft only, or Final / Authorized Generation Mode has explicit live API authorization, target workspace, and API-issued IDs allocated before resource generation | Resource generation/signing/install |
| Schema validation | Yes | Package schema passes | Signing |
| API-issued ID provenance | For generated-final `.yapk` | All generated IDs are API-issued at initial generation and reported; local-first remap is not the primary path | Signing/install |
...
| Generation mode and ID timing | Draft vs Final mode, authorization, workspace, and ID source timing | `validate-generation-mode-id-provenance.mjs` | pass/fail | Mode and ID timing proof only |
