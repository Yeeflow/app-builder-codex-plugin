...
Within each generated Data List custom-form resource, `filterVars[]` and `tempVars[]` share one variable-id namespace. Their IDs must remain unique after removing `__filter_` / `__temp_`, lowercasing, and removing spaces and punctuation. A collision within either array or across both arrays is a generated-final blocker (`PAGE_SCOPE_VARIABLE_ID_DUPLICATE`). Apply this check independently to every custom form belonging to the Data List.
