...
For Data List and Document Library New/Edit rows, primary editable fields are the form body. Do not list generic regions such as `Primary form fields`, `Main form fields`, `Editable fields`, or `Document metadata fields` in `relatedRegions`, `lowerPageBusinessRegions`, `allowedRegions`, or Blueprint hints when they merely duplicate the current form's own fields. Do not duplicate the primary action bar in lower regions. If a lower region is valid, it must be an App Plan-mapped related record, Sub List, document/task/list relationship, or row/item action region with row/current-item context.
