...
For Markdown App Plans, the conformance validator must treat only explicit canonical resource declarations as planned resources. Valid resource declarations come from the canonical requirement-to-resource mapping table, concrete Resource Generation Order rows, recognized top-level resource tables such as Form Reports or Schedule Workflows, and numbered Data List / Approval Form resource subsections. Field rows, control rows, action detail rows, task-form rows, implementation notes, assumptions, validation notes, deferred/runtime-proof sections, template headings, navigation group labels, navigation order headers, status/applicability values, placeholder values such as `Not applicable`, and explanatory prose are not standalone generated resources. Aggregate planning metadata such as `Edit Item and View Item forms for all lists`, `Operational views`, `Grouped app navigation`, `Placeholder app groups and permission intent`, or dashboard/view category summaries must not be parsed as resource names. Comma-separated concrete resource names in a canonical resource cell may be split into individual planned resources, but names containing `and` such as `Rooms and Areas` must remain intact. Strict mode still fails real missing resources, extra generated resources, navigation group/item mismatches, and partial resource-name mismatches when those resources are declared through the canonical plan contract.
