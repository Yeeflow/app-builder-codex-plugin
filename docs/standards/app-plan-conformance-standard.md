...
The App Plan must also be internally reference-complete. Any resource referenced by Navigation, Data List lookup fields, Data List Views, reverse-related Collection sections, Dashboard record-display regions, Data Analytics sources, workflow actions, or form actions must resolve to a planned generated resource or an explicitly deferred/external dependency. A Data List navigation target such as `Risk Categories` must appear in the Data Lists and Document Libraries plan unless it is a planned Data View with a valid host list. A lookup field target such as `Audit Observations.Risk category -> Risk Categories` must resolve to a generated Data List before materialization; the generator must not emit an empty lookup `Rules` payload for missing targets. Treat `APP_PLAN_NAVIGATION_DATA_LIST_TARGET_NOT_PLANNED`, `APP_PLAN_LOOKUP_TARGET_DATA_LIST_NOT_PLANNED`, and `FULL_APP_MATERIALIZATION_LOOKUP_TARGET_DATA_LIST_NOT_PLANNED` as generated-final blockers.
