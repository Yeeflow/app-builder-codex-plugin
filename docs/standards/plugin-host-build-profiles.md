...
Obtain actual registered App IDs from the target workspace or publishing organization. Supply a reviewed JSON file with exactly `apps` and one mapping `yeeflow`, containing only `id`. IDs must begin with `asdk_app_`, `connector_`, or `templated_apps_`, followed by an alphanumeric character and then alphanumeric, underscore or hyphen characters. The ID must belong to the unified endpoint, not one of the retired scoped connections. Directory IDs beginning `plugin_asdk_app_` are not accepted by the uploader. ID syntax checks do not establish existence, authorization or connectivity.
...
This generates `.app.json`, removes direct MCP manifest configuration **and** `.mcp.json`/`mcp.json` discovery files, and uses the manifest name as the top-level archive folder. All four capability domains use the single registered unified App. No review organization IDs are built in or silently used in production.
...
- `npm run test:plugin-host-profiles` builds and extracts both profiles, checks 27 skills, shared payload byte parity, transport metadata, invalid/missing IDs and legacy four-App mappings, and executes both archived regression entrypoints from a temporary cwd.
