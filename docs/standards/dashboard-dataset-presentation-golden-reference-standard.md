...
- Choose `collection_control_grid_table_with_multiselect` only when the App Plan explicitly requires the legacy Flex Grid multiselect contract. Generic multi-row selection, selected-count, or batch-operation requirements default to `collection_control_responsive_multiple_select`. Projects Center / Project Tasks is the export-proven source reference only, not a business-domain restriction.
