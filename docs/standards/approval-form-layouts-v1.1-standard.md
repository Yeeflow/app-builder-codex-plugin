...
Materializers and helper builders must not cap, truncate, or silently drop planned Approval form fields. Every row in the App Plan `Submission Form Fields` and `Task Form Fields` tables must be represented in the decoded `DefResource.pageurls[].formdef` unless the App Plan explicitly marks that field as unsupported, deferred, or omitted for that specific form page.

Generated Approval form field controls must carry business-specific visible labels, field names/bindings, and `nv_label` or `nav_label` values derived from the App Plan. The visible label is contractual: a technical binding such as `LoanNumber` does not prove that the planned `Loan Number` field was materialized if the displayed label drifted to `Request Number`. Source-template residue cleanup must never rewrite planned field labels, titles, placeholders, bindings, or display metadata. Task form fields should be readonly unless the App Plan explicitly states that assignees must edit them. Field controls must use zero margin, full-row spans for Multiple line, Rich text, and Sub List fields, and appropriate control families for the planned field type.
...
Each Approval submission page, task page, print page, Data List workflow task page, and Schedule workflow task page is a single dependency scope. When cloning approved component templates into one of these pages, rename template-owned `filterVars`, `tempVars`, `actions`, and `formAction` entries with a form/page/section namespace before merging them into the form resource, and rewrite every in-template `__filter_...`, `__temp_...`, and action reference. Do not allow two Search/Data Filter controls in one Approval/workflow page to produce the same filter variable unless they are intentionally the same control instance.
