...
```sh
node scripts/inspect-codex-plugin-cache-metadata.mjs --root . --expect-version 0.6.32
```
...
```text
yeeflow/yeeflow-app-builder/0.6.32
```

If Codex references an older cache path such as `yeeflow/yeeflow-app-builder/0.6.31` in answers or login guidance, refresh or reinstall the plugin from Git ref `stable` with the sparse paths above. Do not follow stale cache-path commands.
