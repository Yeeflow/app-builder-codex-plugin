...
- Workflow Condition editor variable-condition update: when the left side is a Workflow variable selector (`left.type = 1`), the condition `group` is the editor group derived from the workflow variable type, not the raw product field category. Use `right.type = 0` for fixed direct values and `right.type = 2` for Expression editor token arrays when the right side is dynamic, function-derived, calculated, or value-processed. User, Department/groupselect, Location, Cost Center, Lookup, Metadata, Dictionary, List, and Text variables use String (`s.`); Number/currency/percent use Number (`n.`); Boolean uses Boolean (`b.`); Date/time uses Date Time with `dt.=`, `dt.!=`, `dt.>=`, `dt.<=`, `dt.>`, `dt.<`, `isNull`, and `isNotNull`; File/Image/Signature use General null checks. Direct Date Time right operands should use date/time strings such as `2026-07-07 04:45:58`; null checks use `right: null`. Expression right examples include string concatenation, numeric calculation, Boolean `iif`, `currentUser`, `getUserAttr(Instance:Applicant, DepartmentID/LocationID, [])`, cross-variable comparison, and `dateAdd(now(), "month", 1)`. Condition groups support at most two layers with a top-level `left:null` / `op:"isNull"` / `right:null` wrapper and child `conditions[]`; use child `and` rows for grouped ranges and child `or` rows for alternatives. Assignment task outcome branches use task Outcome expression-button HTML, not a generic `Outcome` variable token. See `docs/studies/workflow-condition-editor-direct-value.md`.
