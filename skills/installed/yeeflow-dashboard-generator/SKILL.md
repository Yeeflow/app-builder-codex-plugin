...
For an existing live Dashboard, do not save only a generated `LayoutView`. Read the full component, use `normalizeLiveDashboardDetail()` / `validateLiveDashboardDetail()` from `scripts/lib/dashboard-live-component-resource-sync.mjs`, and save matching page JSON into `LayoutView` and the runtime-authoritative `LayoutInResources[0].Resource`. The live gate rejects stale embedded resources and incomplete master-detail selection closure. After `component_save`, read the component again and run the same gate; then browser-smoke two record selections (new record appears and prior record-specific values disappear) before reporting selection behavior as usable.
