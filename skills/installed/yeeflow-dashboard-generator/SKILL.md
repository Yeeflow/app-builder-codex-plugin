...
Dashboard Page Layouts v1.1 is the canonical page-level template for newly generated Dashboard pages. Start each Dashboard page by copying and normalizing `docs/reference/dashboard-page-layout-templates.json` template `dashboard-page-layouts-v1.1`; preserve `attrs.hideHeaderAll = true`, page background `#f4f7fb`, zero root padding, `main > content`, Full width structural containers, and the `content_card_wrapper > section_title_area + section_content_area` skeleton. Then select, remove, duplicate, or adapt sections based on business requirements. Do not build dashboards from scratch or loose prompt interpretation. Event Portfolio Golden Reference regions remain component/region references and may be mapped inside approved v1.1 business-content containers, usually `section_content_area`; do not copy Event Portfolio as a competing page-root shell. Run `scripts/validate-dashboard-page-layout-template.mjs` or the aggregate dashboard generation hard gate before signing readiness.
