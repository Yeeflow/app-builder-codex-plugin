...
Do not embed sample rows in generated-final package decoded `Childs[].ListDatas`, `Childs[].List.ListDatas`, or `Childs[].List.Items`. Emit sample data only as a separate seed artifact/script and run live seeding only with explicit user approval, after install/materialization proof, using `TriggerFlow: false` by default and before/after count reporting.
