...
Data Analytics presentation is also closed-list. Before signing, install, upgrade-check, or handoff, run `scripts/validate-data-analytics-golden-references.mjs --package <file.yapk>`. Generated Pie chart, Column chart, Bar chart, Line chart, and Pivot table controls must clone one approved source template from `docs/reference/data-analytics-golden-references.json` and carry approved template provenance. Chart-with-title templates must preserve their wrapper subtree, style, layout, and typography; only the chart container data binding and title Text value may change. Pivot table output must preserve the exported pivot-table style settings. Data Analytics templates are allowed on Dashboard pages and Data List forms only, never Approval forms. On Dashboard Page Layouts v1.1 pages, place Data Analytics templates only inside `2_columns_section` or `3_columns_section`; do not put analytics controls directly under root `Content`, page title/header regions, Operations, KPI wrappers, Collection-only regions, or copied source-app shells.
