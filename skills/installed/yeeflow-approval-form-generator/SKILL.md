...
- Use the current Workflow Designer v2 graph style for every generated approval workflow. Set root `lineType = "rounded"` and `graphver = 2`; keep `graphzoom` from the export/reference or default rather than treating zoom differences as defects. Every `SequenceFlow` must set `properties.linetype = "rounded"`, include `properties.documentation` as a string (empty is valid), and use `dockers = []` so the new designer can auto-route rounded lines. Use `vertices[]` only for return/backward lines, overlap avoidance, node crossing, or long reroutes; do not restore old orthogonal/full-docker routing.
