...
Approval form plans should use section-level form design, not just a form title. Require each section that affects generation to include purpose, layout, and a control table with `Field ID`, `Display Name`, `Type`, `Binding`, `Required`, `Read Only`, `Default Value`, `Auto Fill / Source`, and `Description`. Require workflow node tables with `Step Name`, Yeeflow node `Type`, description, email enabled state, task/decision settings, and transition rules. Use compact Field ID + Display Name tables only for read-only applicant/profile context sections with no default or autofill behavior that must be generated.
