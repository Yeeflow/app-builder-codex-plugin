...
For Yeeflow Custom Service learning, use `yeeflow-custom-service-generator` after studying real `.ycs` exports and the product runtime spec. Custom Service is a backend Node.js 22 style surface with `main({ connections, params, modules })`; it is not a Custom Code control and not a Form Action Custom Code step. Parse `.ycs` `DraftConfig` as a JSON string, keep `DraftCode` as service code, enforce no Node built-ins, no hardcoded credentials, `modules.yeeSDKClient` for Yeeflow system interactions, and `connections[connectionId]` for authorized external calls. Current Custom Service script training is export-proven and validator-backed. Invocation schema is export-proven for Approval Submission form Form Action `invokeservice`, Approval workflow `InvokeCode`, custom data list form Form Action `invokeservice`, Data list workflow `InvokeCode` with `properties.connections[]`, and Scheduled Workflow `InvokeCode` with a List workflow-variable input plus text workflow-variable output; Task form and Dashboard invocation remain product-rule/user-guidance-backed until focused exports or runtime proof. AI Agent and Copilot invocation still require future focused integration proof.

For Yeeflow Service Portal learning and generation, use `yeeflow-service-portal-generator` after studying real `.yapk` exports that include `PortalInfo`. Service Portal is an application-level external portal surface, not a public form and not internal app navigation. Current Service Portal knowledge is export-proven from `Supplier Management-v1.2.yapk`: decoded top-level `PortalInfo`, `ListSet.Ext3.externalPortal`, a Type `128` portal user system list, portal groups, portal source resources, portal menus, list form routing, portal permissions, and dashboard `ExtPortal` data bindings. Service Portal may expose current-app Data Lists, Document Libraries, and Dashboard pages only; do not expose Approval forms or Form reports. Run `scripts/inspect-service-portal-yapk.mjs --package <package.yapk> --json` for Service Portal packages and keep runtime portal-login/resource-access proof separate from export-backed schema learning.

For full application planning, Custom Code and Custom Service must be planned before generation in the standard App Plan `Custom Code and Custom Service Planning` section. Use `docs/standards/custom-capability-app-plan-planning-standard.md` to decide whether the requirement should stay native, use a Custom Code control, use a Form Action Custom Code step, or use Custom Service. Placeholder rows such as `Not planned`, `N/A`, `None`, or `Not applicable` are planning-only and must not materialize into custom resources, invocation steps, workflow nodes, variables, navigation entries, or package artifacts.
...
For the v1.1 focused Form Action Query Data baseline, distinguish raw export evidence from normalized generation templates. A UI-configured count-only step may retain hidden Sub list mappings in the export; do not promote that residue. Normalize to one of the explicit modes in `docs/standards/form-action-query-data-golden-reference-standard.md`, keep single-to-temp targets prefixed with `__temp_`, permit omitted `querydata_type` as the exported single default, and reject count-only steps that still write a record collection. Keep Workflow Action Query Data out of this focused Form Action training.

For the v1.2 Custom Data List Form Query Data baseline, compare host-specific wrappers rather than copying Approval targets. Learn page-load `formAction.onLoad`, field-change `control_event_rule`, current-record field targets encoded as `____customListFields_<FieldName>`, and current-record Sub list output through `__list_`. Verify every temp/current/Sub list row target against the host form. Record the business-selection rule: Collection/Data Table for read-only reverse relations; Query-to-Sub-list only for editable working copies such as quotation line items. Dashboard uses the separate v1.3 focused contract; Workflow Action Query Data remains a future focused-learning round.
...
Workflow Query Data focused learning from V1.6 proves Approval count-only and single-to-variable nodes plus Scheduled multiple-to-List-variable, Loop list, Invoke Custom Service, and email composition shapes. Preserve the distinction between export-proven structure and runtime execution. Record discrepancies between business descriptions and actual exports; specifically, do not promote a described Lookup filter when the exported node has `filters: []`. Only Loop through list items is export-proven by this sample.

The V1.7 addendum export-proves Data List Workflow QueryData with current-record identity, count branching, ContentList edit/add, Sub List iteration, Loop through multiple values, Loop for fixed times, LoopIndex/LoopItem expressions, and nested LoopBody MailTask/ContentList/Delay actions. It also proves non-null FlowMappings FieldName when the field is a real flowstatus field. Do not elevate export proof to trigger, mutation, email, or Delay runtime proof.
...
For the v1.3/v1.4 Dashboard Query Data baseline, learn the Dashboard page wrapper, temp-only outputs, current-user query, selected-field temp JSON, newest-record query, chained related-record query, not-empty step guard, count-driven Dynamic Display, `JSONStringfy`, and optional Custom Code rendering. Page Size/Page Number are shared across Approval, Data List, Document Library, and Dashboard: default 100/max 1000, default page 1, and non-default page uses `querydata_pageindex`. v1.4 also proves the Type 16 Document Library custom-form wrapper and Approval Task Form wrapper. Workflow Action Query Data remains a separate focused-learning round.
The planning-decision follow-up adds a mandatory capability-selection layer before Query Data mode selection. Read-only multi-row regions use direct Collection/Data Table sources; editable copied rows use current-record Sub Lists; temp JSON requires an explicit text/calculation/Custom Code consumer and cannot feed Collection/Data Table; chained Lookup IDs use target `ListDataID`. Generation Readiness automatically invokes the Query Data plan validator and fails Query Data intent without the standard planning table.

Form Action Set Data List learning must extract host, trigger, operation, target mode/resource, mappings, filters, execution condition, continue semantics, and optional status/item/count outputs from a real export. Normalize only privacy-safe fixtures. Preserve the boundary between page `setdatalist` and Workflow `ContentList`: the page step cannot expand Sub List rows or multi-file values. Document Library learning must preserve `_Path`, native Upload File `Text4`, and the one-file-per-step boundary. Training is incomplete until the App Plan table, shared builder, generated-final materializer, package validator, preflight gate, source/dist tests, and host-specific skills all agree with `docs/standards/form-action-set-data-list-golden-reference-standard.md`.
...
## Full fixed-snapshot resource catalog

For Product 14.5 schema adoption, consult `docs/standards/product-schema/README.md` and `inventory.json`. The 159 first-priority projections are local structural inputs only; unresolved product extensions block validation. Use `scripts/product-schema/cli.mjs` for explicit local drafts/patches. Require product semantic adapters, confirmed MCP mapping, persisted readback and focused runtime evidence before promoting a resource. Never treat catalog coverage as implemented support.
