...
## Explicit Page Layout Selection Contract

Every generated custom Data List Form must explicitly select and materialize a Data List Form Layout before resource generation. `New` and `Edit` require `data_list_form_layout_new_edit_v1_1`; `View`, `Detail`, and `Print` require `data_list_form_layout_view_item_v1_1`, except a deliberate full-page Workbench View which requires `data_list_form_layout_workbench` plus `Open in: Full page`. Public Forms must explicitly select and materialize `public-form-page-layout-standard`. A missing selection, inferred template, generic container, or silent default is a generated-final failure; never create the form and expect a later Designer edit to supply its page layout.
...
For any live MCP create/update that assigns a Data List Lookup field, do not pass a display title, number, code, label, or arbitrary string. First read the Lookup `Rules` and its target `listid`; query/read the target list; resolve one target record and retain its returned `ListDataID`; then write that ID only. The write intent must retain `seedValueType: "lookup"`, `requiresLookupListDataIDResolution: true`, `storedValueField: "ListDataID"`, `mustNotUseDisplayTextAsStoredValue: true`, the configured `targetListId`, and `resolvedListDataID`.

After the write, re-read the host row and verify the persisted Lookup field exactly equals `resolvedListDataID` and resolves in the target list. Capture redacted target inventory, write intents, and persisted rows in a closure-evidence file and run `node scripts/validate-live-lookup-write-closure.mjs --input <evidence.json>`. The validator fails closed with `LOOKUP_WRITE_UNSTRUCTURED`, `LOOKUP_WRITE_TARGET_LIST_MISMATCH`, `LOOKUP_WRITE_TARGET_RECORD_UNRESOLVED`, `LOOKUP_WRITE_PERSISTED_READBACK_MISSING`, `LOOKUP_WRITE_PERSISTED_VALUE_MISMATCH`, or `LOOKUP_REFERENCE_PERSISTED_UNRESOLVED`. If available MCP tools cannot safely obtain the target-row readback, stop; never substitute the display value.

Report `apiAccepted`, `persistedReadback`, `lookupReferenceResolved`, and `browserActionRuntime` separately. The first three do not establish user-visible form behavior; require focused browser runtime proof for that claim.
...
For every choice field, materialize each planned business value as a separate `Rules.choices[]` entry and keep `color_choices` value/order parity. Split App Plan display lists using comma, semicolon, Chinese comma, ideographic comma, or line break. Remove planning annotations such as `Planning Default` / `规划默认`; defaults belong in the plan's `Default Value` column. Run the choice-field option hard gates before signing. See `docs/standards/choice-field-option-materialization-standard.md`.
...
For a Data List custom form with peer sections or a schedule of related activity records, use `tab_control_workspace` and `gantt_control_linked_activity` from `docs/reference/tab-gantt-control-golden-references.json`. Clone their full templates, resolve target `AppID`/`ListSetID`/list/layout IDs and fields, and do not copy source-form Custom Code, document modules, labels, records, or IDs.

Validate Tabs as `aktabs` with `ak-tabs-tab` children, stable IDs/labels, non-empty contents, and exactly one runtime/Designer default. Before selecting or saving Gantt fields, require `attrs.data.list.AppID`, `ListSetID`, and `ListID` for the selected source Data List. Validate Gantt against that source: Start/End are `Datetime`; Dependency is same-source Lookup with parsed `Rules.multiple = true`; Progress is `Decimal` storage with `percent` control and normalized `0..1` range; Parent is optional but must be same-source Lookup with `Rules.multiple = false` when mapped. A linked parent/child Gantt must filter through the current parent `ListDataID`, and Add defaults must use `passvalues` for that same relationship lookup. `linkLayout` must resolve to a Type 1 Activity detail/View form.

Data List creation is incomplete when it has fields alone. Before an MCP create/update call, create a Data View contract for every planned Type 1 list, including hidden support resources: default Type 0 view name, ordered visible fields, query/search coverage, default sort or explicit no-sort choice, executable fixed filter or explicit all-record choice, consumer/purpose, and New/Edit/View Type 1 form routing. `Hidden from navigation` never permits `Layouts: []`.

Immediately after each MCP save, re-read the persisted list and run `node scripts/validate-data-list-completion-contract.mjs <app.yapk|decoded.json>`. Every Type 1 Data List must have exactly one default Type 0 view with a meaningful title, `Ext1.Url = "default"`, parseable `LayoutView`, resolved visible columns, and query coverage for every visible column; it must also have same-list Type 1 New, Edit, and View forms, and `ListModel.LayoutView.add/edit/view` must resolve to those concrete form layouts. A successful API response or fields-only readback is not completion evidence. Report `apiAccepted`, `persistedReadback`, `designerOpen`, and `browserActionRuntime` separately; only the last two establish actual View/action usability.
...
For barcode-capable identifiers such as ISBN, plan a normal Data List Text/input field with `Allow Barcode / QR Scan = Yes`. The shared field builder emits stringified `Rules` with `allowScan:true`; any other field/control type is invalid. This field affordance is distinct from a Form Action `barcode` step. Custom Data List/Document Library forms may host planned Barcode Scan actions, but the focused Print Page training proves Dashboard print targets only; record-specific item printing remains export-learning-required.
