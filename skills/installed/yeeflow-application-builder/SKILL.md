...
When using `scripts/materialize-full-app-generated-final.mjs`, pass the approved Markdown `functional-specification.md`, approved Markdown `yeeflow-app-plan.md`, output directory, and an API-issued ID manifest. The materializer emits the generated-final `.yapk`, decoded resource JSON, ID provenance report, and generation report. It never signs, installs/imports, upgrades, seeds data, runs Version Management proof, or runs browser/runtime proof. Fixture-ID mode is regression-only and is not signing/install eligible.
