...
The Page Function Plan must be generated from the reviewed Functional Specification and Yeeflow App Plan before Dashboard Pattern Library / Golden Reference selection, Application Design System, full-page canonical design images, page implementation blueprints, page/resource generation, decoded resource-vs-blueprint parity, package generation, signing, install/import/upgrade, or runtime proof. It must cover every UI-required Dashboard page, Approval submission form, planned Approval task form, required Approval print page, and planned custom Data list or Document library form. Form Reports are not required canonical UI design surfaces unless explicitly planned as navigable/custom UI surfaces.

Treat the Page Function Plan as the canonical business/page-function contract after the App Plan. It defines what each page must accomplish, which roles use it, which business questions it answers, which App Plan data sources and fields it uses, which filters/sorts/grouping/summary metrics are required, which main and secondary data regions are needed, which business actions users need, how role/permission behavior affects the page, and what mobile behavior is expected. It must not prescribe low-level Yeeflow control nesting, exact Container/Grid/Text/Button property shapes, unsupported properties, CSS, arbitrary custom controls, Dashboard visual implementation, or generated resource JSON.

Every App Plan Dashboard page, Approval submission/task/print surface, and custom Data list/Document library form must reference one Page Function Plan entry, and every Page Function Plan entry must map back to one App Plan resource/surface. Dashboard entries must include Page Overview, Primary Users, Business Questions, Data Sources and Field Usage, Page Filters, Summary Metrics, Main Data Region, Secondary Data Regions, Mobile Support, Design Intent, and Responsibility Boundary. Approval entries must state field usage, editable/read-only state, required/default/dynamic/validation behavior, sub lists, business action intent, and Save as draft/Submit or task-specific actions such as Approve/Reject/Complete. Print pages must be print-specific and omit unsupported interactive controls. New/Edit list/library forms must stay scoped to current-resource fields plus Save/Cancel unless an App Plan-supported exception is explicit. View/detail/custom forms may include related regions only with source, current-item binding, fields, filters, actions, and opening behavior. Document library forms must include metadata, upload/edit, preview, and view behavior where applicable.

Dashboard Pattern Library, Dashboard golden references, Container/Button action mappings, Dashboard Text control style contracts, UI control-property standards, and Event Portfolio/Marketing Event fidelity lessons remain required downstream generation standards. They must be applied after the Page Function Plan when creating page designs, implementation blueprints, and Yeeflow resources. Create a required Dashboard Golden Reference Selection artifact for every Dashboard page between Page Function Plan and blueprint generation. The selection must declare the dashboard page, source Page Function Plan id, selected page/section golden reference IDs, mapping from each PFP business region to a reference section, source data list, fields, filters, metrics, actions, and App Plan field mappings. Use `event_portfolio_dashboard_golden_reference` as the default Dashboard Golden Reference construction style for polished portfolio, operations, status, pipeline, request, facility, service, project, vendor, and contract dashboards unless the page function clearly requires another plugin-contained pattern. The default reference traces to `dashboard_default_shell_event_portfolio_ref`, `dashboard_header_band_event_portfolio_ref`, `dashboard_filter_group_event_portfolio_ref`, `dashboard_kpi_cards_event_portfolio_ref`, `dashboard_content_section_event_portfolio_ref`, and `dashboard_collection_grid_table_event_portfolio_ref` in `docs/reference/dashboard-golden-reference-registry.normalized.json`. Every Dashboard blueprint must reference the selected Dashboard Golden Reference Selection, and every major blueprint section must declare `derivedFromGoldenReference`. Generated Dashboard resources must preserve inspectable provenance/structure for the selected reference and include the expected default regions: `Main`, `Content`, header band, filter group when required, KPI cards wrapper when required, content section, and grid-table Collection when a portfolio/work queue/list region is required. Do not clone Marketing Event-specific fields or sample data into unrelated apps; map source lists, fields, filters, metrics, labels, actions, sorting/grouping, and mobile behavior to the current App Plan and Page Function Plan. Do not let page/resource generation infer Dashboard sections, form fields, related regions, or page actions only from the App Plan when a Page Function Plan exists. Also do not overfit PFP wording into unsupported controls or property paths; use the Application Design System, Dashboard Pattern Library / Golden Reference, implementation blueprints, and verified UI/YAPK hard gates for concrete control, layout, binding, style, and resource JSON decisions.
