...
Before Yeeflow resource generation, choose the generation mode from `docs/standards/final-generation-api-issued-id-mode-standard.md`. Draft / Offline Mode is the default safe mode: it must not call live Yeeflow APIs, may use local draft IDs, and can produce only local unsigned draft packages. Draft output must remain blocked for generated-final signing, install, import, upgrade, or handoff because API-issued ID provenance is absent. Final / Authorized Generation Mode is allowed only after explicit user authorization for live Yeeflow API use and a target workspace. In final mode, call Yeeflow ID APIs before resource generation, use API-issued IDs directly during initial generation, and do not generate local IDs first and replace them later as the primary path. Run `scripts/validate-generation-mode-id-provenance.mjs --report <generation-mode-id-provenance.json>` before generated-final readiness claims.

After planning gates pass and business decisions are answered or `user-default-approved-for-generation`, run the root-token and pattern-library UI generation stage before Page Implementation Blueprints. This default stage must use the Yeeflow Root Token Reference from `docs/standards/yeeflow-root-token-reference.md` and `docs/standards/yeeflow-root-token-reference.normalized.json`, create an Application Design System with `docs/standards/application-design-system-template.md`, select approved Yeeflow UI Section Templates from `docs/templates/yeeflow-ui-section-template-library.normalized.json`, and produce a pattern-selection validation/review report. Root Token Reference review is required before Application Design System approval. Generate the Application Design System before template selection, require Primary/Secondary/Neutral/status/typography/spacing/border decisions before UI Pattern Library selection, and require every selected surface template and Page Implementation Blueprint to preserve token intent. The Application Design System must use one official Yeeflow `applicationLayoutType` from `docs/standards/yeeflow-application-layout-design-rules.md`, must include `applicationChromeStyleId`, `headerMode`, `navMode`, `navBackgroundMode`, `contentSafeArea`, and `layoutRuleSource`, and must reject arbitrary custom SaaS shells or free-form layout names. Dashboard surfaces must select dashboard templates; Approval Submission/Task/Print surfaces must select approval/form/print-appropriate templates; Data List and Document Library New/Edit surfaces must use form-body templates for primary editable fields; related-record, Sub List, Collection, Kanban, and Timeline sections must use matching item-template or collection-control patterns. Run `scripts/validate-yeeflow-root-token-usage.mjs --design-system <application-design-system.md>` and `scripts/validate-ui-pattern-selection.mjs --selection <pattern-selection.json>` before Page Implementation Blueprints, then run `scripts/validate-blueprint-ui-pattern-conformance.mjs --blueprint <page-implementation-blueprint.json> --pattern-selection <pattern-selection.json>` and `scripts/validate-yeeflow-root-token-usage.mjs --blueprint <page-implementation-blueprint.json>` before Yeeflow resource generation. Yeeflow resources should use token-compatible property values where supported and must preserve token intent in blueprint/resource parity evidence. These gates prove token, pattern, and blueprint readiness only; they do not prove Yeeflow resource serialization, package validity, signing/API acceptance, install/upgrade success, or runtime behavior.

Generated PNG design images and HTML previews are optional review evidence, not default implementation sources. Use PNG canonical design artifacts only when the user explicitly requests visual review artifacts. Use the HTML-first high-fidelity UI Surface Contract workflow only when the user explicitly asks for HTML previews, screenshots, or prototype evidence. HTML/PNG evidence cannot override Root Token Reference decisions, the Application Design System, selected Yeeflow UI templates, or Page Implementation Blueprints. Optional HTML previews must be generated from UI Surface Contracts, the Application Design System, and selected Yeeflow UI templates; they must use root tokens and approved UI pattern templates, render desktop/mobile variants, and be polished modern business UI rather than low-fidelity scaffolds. Capture desktop/mobile screenshots from validated HTML previews when that optional evidence flow is requested; screenshots are evidence, not the primary implementation contract. Run `scripts/validate-ui-surface-contracts.mjs`, `scripts/validate-html-preview-layout.mjs`, and `scripts/compare-blueprint-to-ui-surface-contract.mjs` for the optional HTML flow. Do not proceed to Page Implementation Blueprints if selected template validation or root-token validation fails, and do not proceed to Yeeflow resource/package generation if blueprint pattern conformance, blueprint token usage, or optional blueprint-to-contract parity fails. Keep proof boundaries separate: root-token validation, pattern validation, contract validation, HTML/DOM validation, screenshot evidence, blueprint parity, package schema, signing/API acceptance, install/upgrade success, and runtime proof are separate proof layers.

Generated UI icons must use Yeeflow-supported FontAwesome classes such as `fa-regular fa-file` or `fa-solid fa-check`. Do not invent SVG icons, emoji icons, image icons, or arbitrary custom icon names for normal application UI. The Application Design System must declare icon source, preferred FontAwesome family, size rule, color token rule, and surface usage; pattern selection must declare icon purpose/class/fallback; Page Implementation Blueprints must map every icon as Yeeflow control type `icon` with FontAwesome class, semantic purpose, tokenized color/size, action binding when clickable, and label/tooltip intent for icon-only actions. If the exact icon class is uncertain, mark `runtime-proof-required`, `export-learning-required`, or `deferred`.

For Approval forms and Data List custom form/detail artifacts, full-page height is not enough. Require page-specific business semantics before blueprint readiness: manifest rows must declare `primaryBusinessObject`, realistic `semanticFieldExamples`, `fieldValueSemanticsStatus`, `businessRegionEvidence`, `lowerPageBusinessRegions`, `formPurposeDifferentiators`, `templateReuseRiskStatus`, and `pageSpecificQualityEvidence`. Field labels and values must match business meaning; do not put status values in title/name fields, task/status labels in owner/person fields, document filenames in date fields, person/vendor names in status fields, review comments in document/evidence fields, or status labels in related-record fields. Lower-page form/detail regions must contain planned business data, records, history, evidence, or actions such as approval history, document checklists, related renewal tasks, linked records, audit trails, reviewer comments, or print signature blocks; blank space, `Page end`, generic notes, and design-stage explanation text do not satisfy full-page coverage. Similar forms may share visual style, but they must declare purposeful functional differences when their business purpose differs.

Lower-page form/detail business regions must be pattern-concrete before Page Implementation Blueprints. The selected template and blueprint contract must map realistic rendered rows, cards, timeline events, checklist rows, document table/cards, activity feed rows, signature rows, read-only field groups, or another Yeeflow-control-shaped UI pattern. Optional PNG/HTML evidence may show the visual rendering, but manifest source-list notes or field-name lists are not enough if they only describe text such as `Source: Contract Documents`, `Document name, type, status`, or `Show signed contract evidence`. Each lower-page region must declare `visualPattern`, `selectedTemplateId`, `plannedYeeflowControl`, `renderedExampleCount` or explicit empty-state fallback, `renderedExampleSummary`, `displayedBusinessFields`, `actionsShown`, `visualConcretenessStatus`, `antiPlaceholderStatus`, and `blueprintMappingHint`. Intentional empty states must map to an empty-state component with reason and next action. `readyForBlueprint: true` is blocked when lower-page pattern concreteness fails or needs human review without explicit deferral reason, fallback, and proof impact.

Lower-page visual concreteness is not enough if the region inherited the wrong business semantics. Every lower-page business region must keep `sourceListOrDataSource`, `regionPurpose`, `displayedBusinessFields`, `displayedFields`, `actionsShown`, `behavior`, `proofImpact`, and `blueprintMappingHint` aligned to the same business object or to an explicitly documented related-record relationship. Contract/Linked Contracts regions sourced from Contracts must not use Renewal Task fields/actions such as `Task`, `Due date`, `Priority`, `Open task detail`, `Mark complete`, or a `Renewal Tasks filtered by current Contract` mapping hint. Document, Approval History/Route, Audit Activity, Print Signature Block, and Checklist regions must use their matching field/action semantics. If `displayedBusinessFields` and `displayedFields` differ, include `fieldAliasMap`, `semanticFieldMapping`, `runtime-proof-required`, `export-learning-required`, or explicit deferred proof details.

Full-page design artifacts must also pass surface responsibility and App Plan field/action coverage gates before Page Implementation Blueprints. Every blueprint-ready manifest row must declare `appPlanResourceRef`, `sourceResourceType`, `sourceListOrFormName`, `surfaceResponsibility`, `plannedFieldCoverage`, `requiredFieldsShown`, `missingPlannedFields`, `fieldCoverageStatus`, `plannedActions`, `actionsShown`, `missingRequiredActions`, `actionCoverageStatus`, `forbiddenRegionsPresent`, `forbiddenRegionStatus`, `surfaceResponsibilityStatus`, and `appPlanTraceabilityStatus`. Approval Submission forms must show planned editable submission fields, Sub List controls as Sub Lists, Save as draft, and Submit. Approval Task forms must show task context plus Approve/Reject or Complete actions. Approval Print pages must be read-only and print-oriented. Data List New/Edit forms must prioritize current-list add/edit fields and Save/Cancel or Save/Submit actions over decorative regions. Data List View/Detail forms must show current-record display fields and only explicitly planned related regions. Document Library forms must use document/file metadata, file upload/preview/open/download behavior, and document-specific actions. Logic-only items such as validation checklists, route preview, audit activity, workflow history, and routing rules must not appear as visible UI unless explicitly planned for that surface.

For New/Edit surfaces, primary editable fields are the form body and must be represented as field groups/control mappings from selected form-body templates, not lower-page regions. Do not generate generic `Primary form fields`, `Main form fields`, `Editable fields`, or `Document metadata fields` regions, fake cards made from field/action names, or duplicate Save/Cancel, Save/Submit, Upload/Save, Submit, Approve/Reject, or Complete action bars. Related/lower-page regions must be real App Plan-related regions with selected templates, source list, parent/current item binding, and row/item action context. Labels, placeholders, sample values, default values, and read-only current values must remain distinct; do not render a label such as `Start date` as the field value unless it is explicitly placeholder semantics. If a UI Surface Contract or HTML preview is wrong, HTML-to-Blueprint parity passing does not prove readiness: pattern selection, contract validity, HTML validity, control mapping validation, Blueprint pattern conformance, and Blueprint parity are separate gates.

Full-page canonical image completeness is not enough if the image is hard to read. Every blueprint-ready artifact must include `visualUsabilityStatus`, `textOverflowStatus`, `overlapStatus`, `spacingStatus`, `mobileUsabilityStatus`, `responsiveLayoutEvidence`, `textWrappingStrategy`, `containerBoundaryEvidence`, and `visualUsabilityFindings`. Text overflow, button/badge/table label overflow, card/timeline/form label collision, element overlap, bad spacing, clipped edge content, or mobile layout pressure block `readyForBlueprint: true` unless explicitly deferred with reason, fallback, and proof impact. Long text must wrap, truncate with intentional ellipsis, or receive a wider/responsive layout. These gates prove design artifact readiness only; they do not prove Yeeflow package validity, signing/API acceptance, install/upgrade success, or runtime rendering.
...
Generated-final `.yapk` output must use API-issued numeric content IDs from `GET /utils/generate/ids?count=<n>` during initial resource generation and must emit both `dist/<app-name>-generation-mode-id-provenance.json` and a redacted `dist/<app-name>-id-provenance-report.json` with `sourceMarker: "api-generated"`, path-to-purpose mappings, duplicate checks, unused-ID accounting, generator provenance metadata, and no non-API IDs. Local ID generation, hardcoded generated IDs, copied sample/export IDs, random values, timestamps, UUID fallback, deterministic local-only seeds, and local-first IDs later remapped as the primary finalization path are forbidden for generated-final `.yapk`. Runtime navigation groups must include API-issued `ID`, `AppID`, `ListSetID`, `Type: "classes"`, `Title`, `Icon`, and `list[]`; children must include `AppID`, `Title`, `ListID`, `ListSetID`, and `Type`, with dashboards/pages as `Type: 103` plus `LayoutID`, approval forms as `Type: 105`, and data lists as `Type: 1`. Run `scripts/validate-generation-mode-id-provenance.mjs`, `scripts/validate-yapk-id-provenance.mjs`, and `scripts/validate-yapk-navigation-runtime-metadata.mjs`; stop before signing, install, upgrade-check, or handoff if any gate fails. `setsign`/`verifysign` and install acceptance do not prove generation mode authorization, ID provenance, or navigation runtime metadata completeness.
...
8. Application Design System and UI Pattern Library selection gate
9. Page Implementation Blueprints
10. Blueprint pattern conformance and control-property validation
...
Do not blindly implement every requested detail if it would make the package unsafe, unimportable, or dependent on unavailable tenant data. Defer features only when they are optional, integration-heavy, advanced, unclear, explicitly moved to a later phase by the user, or likely to make the full planned package hard to import and test. For generated applications, the approved app plan is a contract: every planned list, form, workflow, page, report, agent/tool, integration, permission/admin resource, and navigation group/item must be implemented, explicitly deferred with a reason, or marked unsupported/unproven in the plan-conformance report. Strict App Plan conformance for Markdown plans must read only canonical resource declarations as planned resources; field/control/action rows, task form detail rows, implementation notes, assumptions, validation notes, deferred/runtime-proof sections, template headings, navigation group labels, navigation order headers, status/applicability values, placeholder values such as `Not applicable`, aggregate category summaries, and explanatory prose are not standalone generated resources. Strict mode still fails real missing, extra, navigation group/item, or partially mismatched resources when those resources are declared by the canonical App Plan contract.
