...
Approval form planning must include section names, section purpose, layout, controls, bindings, required flags, read-only flags, default values, autofill/source rules, descriptions, forms list, workflow node types, email settings, task/decision settings, and transition rules. Profile/context sections may use a compact Field ID + Display Name table only when they are read-only context and have no default/autofill behavior that affects generation; editable or auto-filled sections need full control metadata.
