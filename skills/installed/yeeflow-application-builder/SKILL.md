...
Every full app plan must follow `docs/app-plan-standard-template.md` when present. The Northpeak Resource Operations plan is the style reference: numbered sections, plan status, purpose, roles, business process, capability coverage, navigation, data lists/fields, forms, dashboards/pages, UI/UX control mapping, golden template/reference strategy, actions/workflow logic, permissions, integrations, document/attachment decisions, reports, validation, proof boundary, assumptions, deferred/runtime-proof items, and recommended next prompt. Do not produce app plans that are only brief object lists. If the user explicitly asks for a lightweight plan, still include `Generation Contract and Hard Gates`.
...
After generation, compare the package against the plan. Do not return the final package until planned data lists, important fields, forms, approval form and task pages when required, dashboards/pages, navigation groups, print pages, major controls, workflows/actions, Data table display columns, padding/layout quality, permissions/integrations, and required bindings are present or explicitly documented as deferred with a reason and workaround.
