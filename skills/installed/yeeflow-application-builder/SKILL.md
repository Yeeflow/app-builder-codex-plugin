...
After planning gates pass and business decisions are answered or `user-default-approved-for-generation`, run the Full-page Canonical Design Artifacts stage before Page Implementation Blueprints. This stage must create an Application Design System with `docs/standards/application-design-system-template.md`, full-page canonical design images for every required Dashboard page, Approval Submission form, planned Approval Task form, planned Approval Print page, and planned Data List custom form, a Design Image Manifest with `docs/standards/design-image-manifest-template.md`, and a design-stage validation/review report. Generate the Application Design System before any canonical image, and require every manifest row and canonical artifact to reference it. The Application Design System must use one official Yeeflow `applicationLayoutType` from `docs/standards/yeeflow-application-layout-design-rules.md`, must include `applicationChromeStyleId`, `headerMode`, `navMode`, `navBackgroundMode`, `contentSafeArea`, and `layoutRuleSource`, and must reject arbitrary custom SaaS shells or free-form layout names. Dashboard artifacts must pass selected layout/chrome fidelity, while Approval and Data List form artifacts are complete form pages and do not require app header/navigation. Structural design coverage is not enough: every blueprint-ready design artifact must also pass modern visual quality checks for hierarchy, spacing, composition, KPI/Summary treatment, analytics, realistic records, action placement, responsive readability, and anti-pattern avoidance. Form Reports are excluded from required canonical design image coverage and remain standalone Form Report resources. Run `scripts/validate-full-page-design-artifacts.mjs --manifest <design-image-manifest.json>` before Page Implementation Blueprints. This design-stage gate proves visual-contract readiness only; it does not prove Yeeflow resource serialization, package validity, signing/API acceptance, install/upgrade success, or runtime behavior.

For complex business applications, prefer the HTML-first high-fidelity UI Surface Contract workflow after App Plan approval and before Page Implementation Blueprints. Generate the Application Design System, then UI Surface Contracts with `docs/standards/ui-surface-contract-template.md`, then high-fidelity HTML previews with `docs/standards/html-preview-design-standard.md`. This workflow inherits every Full-page Canonical Design Artifact gate; it must not replace or bypass existing requirements for official dashboard layout/chrome, no-app-chrome form surfaces, full-page/page-end completeness, modern visual quality, surface responsibility, App Plan field/action coverage, forbidden-region checks, semantic consistency, lower-page visual concreteness, visual usability, text overflow/overlap/spacing/mobile-pressure checks, clipping checks, and template reuse risk. HTML previews must be generated from UI Surface Contracts and the Application Design System, use design tokens and approved UI pattern templates, render desktop/mobile variants, and be polished modern business UI rather than low-fidelity scaffolds. Capture desktop/mobile screenshots from the validated HTML previews using Playwright or equivalent local browser automation; screenshots are evidence, not the primary implementation contract. Run `scripts/validate-ui-surface-contracts.mjs`, `scripts/validate-html-preview-layout.mjs`, and `scripts/compare-blueprint-to-ui-surface-contract.mjs`. Do not proceed to Page Implementation Blueprints if UI Surface Contract, HTML/DOM layout, visual quality, inherited full-page design gates, screenshot evidence, or layout validation fails. Do not proceed to Yeeflow resource/package generation if blueprint-to-contract parity fails. Keep proof boundaries separate: contract validation, HTML/DOM validation, screenshot evidence, blueprint parity, package schema, signing/API acceptance, install/upgrade success, and runtime proof are separate proof layers.
...
Lower-page form/detail business regions must be visually concrete in the canonical design image before Page Implementation Blueprints. The PNG/design contract must show realistic rendered rows, cards, timeline events, checklist rows, document table/cards, activity feed rows, signature rows, read-only field groups, or another Yeeflow-control-shaped UI pattern. Manifest source-list notes or field-name lists are not enough if the canonical image only shows explanatory text such as `Source: Contract Documents`, `Document name, type, status`, or `Show signed contract evidence`. Each lower-page region must declare `visualPattern`, `plannedYeeflowControl`, `renderedExampleCount`, `renderedExampleSummary`, `displayedBusinessFields`, `actionsShown`, `visualConcretenessStatus`, `antiPlaceholderStatus`, and `blueprintMappingHint`. Intentional empty states must show an empty-state component with reason and next action. `readyForBlueprint: true` is blocked when lower-page visual concreteness fails or needs human review without explicit deferral reason, fallback, and proof impact.
...
For HTML-first New/Edit surfaces, primary editable fields are the form body and must be represented as field groups/control mappings, not lower-page regions. Do not generate generic `Primary form fields`, `Main form fields`, `Editable fields`, or `Document metadata fields` regions, fake cards made from field/action names, or duplicate Save/Cancel, Save/Submit, Upload/Save, Submit, Approve/Reject, or Complete action bars. Related/lower-page regions must be real App Plan-related regions with source list, parent/current item binding, and row/item action context. Labels, placeholders, sample values, default values, and read-only current values must remain distinct; do not render a label such as `Start date` as the field value unless it is explicitly placeholder semantics. If a UI Surface Contract is wrong, HTML-to-Blueprint parity passing does not prove readiness: contract validity, HTML validity, control mapping validation, and Blueprint parity are separate gates.
