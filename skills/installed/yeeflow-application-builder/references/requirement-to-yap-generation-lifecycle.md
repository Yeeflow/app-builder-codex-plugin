...
For strict App Plan conformance against Markdown plans, treat only canonical App Plan resource declarations as planned resources: requirement-to-resource rows, Resource Generation Order rows, recognized resource tables, and numbered Data List / Approval Form resource subsections. Do not treat field/control/action rows, task forms, implementation notes, assumptions, validation notes, deferred/runtime-proof sections, template headings, or explanatory prose as generated resources. A strict failure must correspond to a real declared resource missing from the package, an extra generated resource not declared by the App Plan, a partial resource-name mismatch, or another explicit contract violation.
