...
Plan conformance v0.6.21-api-map.0: validation must separately report schema validation, package validation, workflow graph validation, UI/materialization validation, and plan-conformance validation. A package can pass schema/package checks and still fail the approved app plan. Run `scripts/validate-app-plan-conformance.mjs --plan <plan> --package <package>` for generated apps with an approved plan. Missing planned resources are errors unless explicitly deferred with a reason. Grouped navigation requested by the plan is warning-level in default Research mode only when grouped `LayoutView` export shape is unproven; strict mode must fail it. Do not let schema/package pass imply plan-conformance pass.
