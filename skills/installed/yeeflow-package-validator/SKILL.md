...
When validating a plugin-only clean-room generation attempt, run `scripts/inspect-full-app-generation-entrypoints.mjs` before concluding that no full-app generation path exists. The registry at `docs/reference/full-app-generation-entrypoints.json` distinguishes skill-orchestrated full-app generation and the callable `scripts/materialize-full-app-generated-final.mjs` artifact materializer from validation scripts, delivery-decision helpers, package API helpers, runtime-proof demos, and sample-specific generators. Package validators must not claim package generation capability on their own, and they must not accept helper scripts as generic Functional Spec + App Plan to YAPK generators. If the materializer exists but cannot run, classify the concrete blocker such as missing API-issued IDs, `FULL_APP_MATERIALIZATION_RESOURCE_GRAPH_NOT_IMPLEMENTED`, failed generated-final preflight, or user-requested planning-only scope. A placeholder package or schema-smoke output must never be treated as signing/install eligible. When `FULL_APP_MATERIALIZATION_RESOURCE_GRAPH_NOT_IMPLEMENTED` is reported, require exact planned resource counts, parsed resource names, missing generated-final output surfaces, `packageEmitted: false`, and `signingEligible: false`; inflated counts from field rows, dashboard section rows, metric rows, filter rows, item-template rows, validator commands, or prose are validator defects.
...
Dashboard Collection dataset presentation validation is a signing-readiness gate. For `collection_control_responsive_card_grid`, validation must require the full export-shaped template from `docs/reference/collection-control-responsive-card-grid.template.json`: `collection_control_responsive_card_wrapper` subtree, `card_col_body` Collection root, responsive layout/filter/fulltext shape, valid Dynamic control field-type mapping, no `dynamic-image` without an Image field, and item operation actions/delete-confirmation only when operations are present. For `collection_control_card_with_multiselect_toolbar`, validation must require the full export-shaped template from `docs/reference/collection-control-card-with-multiselect-toolbar.template.json`: `card_with_multiselect_toolbar_wrapper` subtree, locked `card_col_item_multi_select`, Collection root actions, selected-state temp variables, page actions, form actions, filter variables, and only the allowed editable regions. For `collection_control_grid_table`, use `docs/reference/collection-control-grid-table.template.json` as the export-shaped source of truth. Copy `grid_table_col_wrapper` and all descendants, not a simplified header-plus-Collection lookalike. Business mapping may edit only `grid_table_col_title_wrapper`, `op_normal`, `grid_table_col_header`, `grid_col_item`, and optional `grid_table_col_item_operations`. Preserve style/layout/typography outside those regions, keep `grid_table_col_header` and `grid_col_item` column count/width/property parity, map field types to Dynamic user/image/file/field controls, and omit item operation menus for Form Report/Data Report display-only sources. If item operations are present, every button must bind to a valid action; edit/delete require matching Collection actions, and delete requires the confirmation temp variable/conditional delete flow.

Data Analytics golden-reference validation is a signing-readiness gate. Run `scripts/validate-data-analytics-golden-references.mjs --package <package.yapk>` whenever Pie chart, Column chart, Bar chart, Line chart, Area chart, or Pivot table templates are generated or upgraded. The approved templates are `data_analytics_pie_chart_with_title`, `data_analytics_column_chart_with_title`, `data_analytics_bar_chart_with_title`, `data_analytics_line_chart_with_title`, `data_analytics_area_chart_with_title`, and `data_analytics_pivot_table_standard`. Fail packages that place Data Analytics controls on Approval forms, omit approved template provenance, omit the chart wrapper/title contract, simplify chart/pivot controls, or place analytics templates outside `2_columns_section`/`3_columns_section` on Dashboard Page Layouts v1.1 pages.

For `collection_control_grid_table_with_multiselect`, validation must require the full export-shaped template from `docs/reference/collection-control-grid-table-with-multiselect.template.json`: `grid_table_col_multiselect_wrapper` subtree, locked `grid_table_col_item_select`, Collection root actions, selected-state temp variables, page actions, form actions, filter variables, and matching `grid_table_col_header` / `grid_col_item` column definitions. Do not accept simplified card/grid-table multiselect approximations as generated-final package content.
...
Generated custom Data List forms must pass Data List Form Layouts v1.1 validation before signing readiness. Run `scripts/validate-data-list-form-layout-template.mjs --package <package.yapk> --plan <yeeflow-app-plan.md>` when a package contains Type `1` custom Data List forms. New/Edit layouts must use `data_list_form_layout_new_edit_v1_1`; View layouts must use `data_list_form_layout_view_item_v1_1`. Fail packages that omit template provenance, mutate locked template regions, place business controls outside `page_title_content`, `Operations`, `section_content_area`, `section_title_header`, or `kpi_card_wrapper`, include related Collections/Data Analytics/KPI on New/Edit forms, omit View Item `page_title_section` or `kpi_metrics_wrapper`, copy Dashboard page root shells into Data List forms, or attach `ListModel.LayoutView.add/edit/view` to the wrong template family. API install acceptance does not prove Data List form layout fidelity.

Generated custom Data List form field groups must also pass Data List Form Fields Grid v1.1 validation before signing readiness. Run `scripts/validate-data-list-form-fields-template.mjs --package <package.yapk> --plan <yeeflow-app-plan.md>`. Fail packages that place current-record field controls outside `form_grid_fields_wrapper`, place `form_grid_fields_wrapper` outside the `section_content_area` of `content_card_wrapper`, `content_card_60_wrapper`, or `content_card_40_wrapper`, omit explicit zero margin on field controls, omit business-specific field-control `nv_label`/`nav_label` values, let Multiple line/Rich text/Sub list controls fail full-row column span, generate Sub list controls without `data_list_form_control_sublist_v1_1` provenance and locked style parity, use tablet columns greater than PC/laptop columns, use mobile columns other than one, or skip the App Plan Form Fields Layout Template Selection table.
...
Dashboard template materialization fidelity is part of signing readiness. The dataset-presentation gate must reject generated packages where selected templates are reconstructed by helpers instead of cloned from approved artifacts. For `collection_control_grid_table_with_multiselect`, fail missing Full width wrapper/caption/content layers, locked spacing drift, source-domain text residue from Projects/Tasks, static `grid_table_col_item_select` icons without a select/toggle action, invalid Designer filter condition shape, unresolved filter bindings, or missing selected-item variables/actions. KPI wrappers must contain approved `kpi_cards_kpi_row` / `event_portfolio_kpi_planned_events` materialization; helper-created `ops_kpi_*` cards are blockers, not warnings.
