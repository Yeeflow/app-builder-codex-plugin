...
For Tab or Gantt requirements in a generated application, dispatch through `docs/reference/tab-gantt-control-golden-references.json`. `tab_control_workspace` is a reusable `aktabs` structure; `gantt_control_linked_activity` is a parent-record form pattern with a resolved Activity source, current-parent filter, Add default propagation, and detail layout link. Do not use source app IDs, labels, Custom Code, document controls, or raw resources as defaults.

Generated-final checks are fail-closed: Tabs need valid `ak-tabs-tab` children and exactly one default. Gantt Start/End require `Datetime`; Dependency requires a same-source multi Lookup; Progress requires Decimal/percent in the 0..1 range; optional Parent requires a same-source single Lookup. Current-parent filter and Add `passvalues` must use the same child lookup, and `linkLayout` must resolve to a concrete Type 1 detail/View form. Run `node scripts/validate-tab-gantt-golden-references.mjs <decoded-app-or-fixture.json> --strict` before packaging and retain the runtime proof boundary in `docs/standards/tab-gantt-control-golden-reference-standard.md`.
