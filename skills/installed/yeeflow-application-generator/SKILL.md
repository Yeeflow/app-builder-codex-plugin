...
When package automation is requested, use OAuth and resolve the target workspace from `--workspace-id`, optional local/manual `YEEFLOW_WORKSPACE_ID` if present, or user-selected `flowcraft` workspace discovery. Do not auto-install without explicit confirmation. For existing app changes, produce a versioned YAPK and upgrade only when the target app/package is clearly identified and approved. Package API result classes are `success`, `already_installed`, `api_rejected`, and `http_rejected`; an `already_installed` result should lead to upgrade flow, cleanup, or a renamed/new-version package, not blind retries.
