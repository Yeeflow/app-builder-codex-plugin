...
Dashboard Collection planning must use exact approved template IDs in the final App Plan, not prose-only descriptions. A planning draft may state a generic multiselect/batch requirement, which defaults to `collection_control_responsive_multiple_select`; the final selected-template cell must record that chosen ID. Use `collection_control_grid_table_with_multiselect` only when the plan explicitly requests legacy Flex Grid compatibility. Earlier support tables may describe display needs, but the canonical record-display/template-selection table is the generation contract. The App Plan generator must write registry-matching rationale at generation time for every selected Collection presentation; do not emit a template ID-only row that later requires manual editing. Use business signals from the selected reference, such as native desktop/tablet table plus mobile cards, dense row/column scanning, work queue, card browsing, multiselect/bulk operation, or primary operations table.
