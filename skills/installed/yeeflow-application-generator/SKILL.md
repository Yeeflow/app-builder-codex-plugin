...
## Retired Collection References

`collection_control_grid_table` and `collection_control_grid_table_with_multiselect` are retired migration-only IDs. This rule supersedes legacy examples later in this historical skill text: new Dashboard plans and generated resources must select `collection_control_responsive` or `collection_control_responsive_multiple_select` respectively. Existing packages may retain the old provenance for compatibility inspection only; never recreate their Flex Grid structures for new materialization.
...
Before requesting any API-issued IDs, run `scripts/validate-pre-id-allocation-readiness.mjs --plan <yeeflow-app-plan.md>`. This mandatory fail-closed gate runs the official Data List form-layout, Data List form-fields, Approval form-fields, and Dashboard dataset-presentation App Plan validators. Do not call `GET /utils/generate/ids`, create an ID manifest, materialize a package, sign, install, or mutate a tenant when the gate reports an error. A generated Data List plan must contain exact `#### Data List Form Layout Template Selection` and `#### Form Fields Layout Template Selection` subsections, select `data_list_form_layout_new_edit_v1_1` for New/Edit, select `data_list_form_layout_view_item_v1_1` or an approved Workbench for View, and select `data_list_form_fields_grid_v1_1` with valid responsive columns. Approval `Submission Form Fields` and `Task Form Fields` tables must each contain a `Choice Values` column; Choice/select values belong there and never only in Validation. Every concrete Dashboard Collection region/control row must contain exactly one approved dataset presentation reference and its registry-matching rationale; a reference mentioned only in a separate Dashboard Golden Reference Selection or summary table does not satisfy pre-ID readiness. Use `collection_control_grid_table` for dense operational audit-table review when the rationale calls for dense row/column scanning without bulk selection.
...
Generated applications must use the package-level application layout golden reference `application-layout-sidebar-workspace-1` by default unless the user explicitly selects another approved Yeeflow application layout. The App Plan must declare `Application layout template: application-layout-sidebar-workspace-1` in the Application Navigation Plan. The materialized package must serialize this template into decoded `ListSet.LayoutView` with `sortVer: 1`, 46px header height, `[null, "h6-semi-bold"]` header typography, the reference header/navigation colors, and left navigation menu position. This application layout contract is separate from Soft outline controls in `Themes[]`. Every visible business navigation group and menu item must have a suitable FontAwesome icon; hidden process/task entries may keep Yeeflow built-in process icons. Run `scripts/validate-application-layout-template.mjs --package <package.yapk>` or the aggregate first-generation preflight before signing readiness.

Generated applications must also declare an `Application Color Pattern Selection` in the App Plan and materialize it into the Type `0` `application style.Config` JSON string in `Themes[]`. The App Plan selects only Primary, Secondary, and Neutral base colors; each must use `Light Model` exactly `Luminance`. If the user provides approved brand colors, use them when they pass readability ranges. If the user does not request custom branding, infer a business-appropriate palette from the application domain and explain the rationale instead of copying the generic defaults. For example, Business Travel / approval / reimbursement apps should prefer Primary `#1E40AF`, Secondary `#0F766E`, Neutral `#94A3B8`; vendor/procurement apps can use Primary `#0F766E`, Secondary `#1D4ED8`, Neutral `#94A3B8`; asset/operations apps can use Primary `#1D4ED8`, Secondary `#0F766E`, Neutral `#94A3B8`. The generic fallback `#0065FF`, `#00D1FF`, and `#B3B7C0` is allowed only when the business domain cannot be inferred or when the user explicitly requests/approves the Yeeflow default palette. Success, warning, and danger remain Yeeflow semantic colors and must stay green, yellow, and red. Reject base colors that are too light or too dark: Primary/Secondary must stay in OKLCH lightness `0.35-0.82` with `0.42-0.68` preferred, and Neutral must stay in OKLCH lightness `0.65-0.88` with chroma no greater than `0.06`. Run `scripts/validate-application-control-style-template.mjs --package <package.yapk> --plan <yeeflow-app-plan.md>` or the aggregate first-generation preflight before signing readiness.

Standalone generated-final materialization must not hand-build simplified Dashboard pages. For every planned Dashboard page, start from the App Plan selected page layout template in `docs/reference/dashboard-page-layout-templates.json`; use `dashboard-page-layouts-v1.1` by default, and use `dashboard-page-layouts-workbench`, `dashboard-page-layouts-two-panel-workspace`, or `dashboard-page-layouts-three-panel-workspace` only when the App Plan explicitly selects that template. Preserve the selected page structure, remove empty Operations regions, and place business record-display components only inside approved selected-layout business slots. For v1.1, use `content_card_wrapper > section_content_area`; for Workbench, use approved slots such as `primary_working_area`, `right_side_panel`, `section_content_area`, and `chart_cards_section`; for master-detail workspace pages, preserve `left_panel`, `content_panel`, `left_panel_data_items_wrapper`, `current_item_wrapper`, and the `vCurrentItemID` selection contract. The left-panel Collection item action must write the clicked item ID into `vCurrentItemID`; `current_item_wrapper` must bind the same source dataset, limit records to `1`, and filter record ID by `vCurrentItemID`. Keep `left_panel_caption_add_button` only for create-capable Data list or Document library sources; remove it for Form reports, Data reports, or other read-only/reporting sources. `content_panel_empty` is the editable empty-selection state and may have domain-specific image/title/description text. Remove unused copied `content_card_wrapper`, `content_card_60_wrapper`, `content_card_40_wrapper`, Workbench/workspace section modules, empty `chart_cards_section`, empty `kpi_metrics_wrapper`, empty filter groups, and empty right/detail panels instead of leaving title-only sections whose `section_content_area` is empty. Parse Dashboard dataset rows from the App Plan and materialize the exact selected Collection presentation template from `docs/reference/collection-control-*.template.json`; do not collapse multiple App Plan selections into `collection_control_grid_table`. The generated Collection root must carry explicit dataset presentation provenance, source resource/list binding, dataset-region metadata, page-level dependencies, and template-specific action/temp-variable contracts. Rewrite all Collection action/detail layout placeholders, including `{{DetailLayoutID}}`, `{{layout}}`, `{{LayoutID}}`, `{{layoutId}}`, `{{PageID}}`, and `{{pageId}}`, to concrete generated layout IDs or remove the unsupported edit/open action. Before any signing readiness claim, the materialized package must pass Dashboard Page Layout template validation, Dashboard Collection template conformance, Dashboard Golden Reference conformance, aggregate Dashboard hard gates, YAPK package validation, ID provenance, navigation runtime metadata, and generated-final resource completeness.

Before writing `Pages[].LayoutInResources[].Resource`, the materializer must perform a final recursive Dashboard ID remap and provenance scan. Remap package-local numeric IDs copied from source templates inside `attrs.data.list`, `attrs.querydata_list`, Collection/filter sources, Dashboard/custom action steps, local Collection actions, detail layout refs, and Data Analytics visible/runtime source refs to current generated resources. If any large source-template ID remains and is not in the current API-issued manifest, stop generated-final emission; do not add stale source IDs to the manifest to make validation pass.

Dashboard Collection planning must use exact approved template IDs, not prose-only descriptions. The App Plan may include human rationale such as "dense selectable queue" or "responsive card browsing", but the selected-template cell must contain exactly one approved ID such as `collection_control_grid_table_with_multiselect`, `collection_control_responsive_card_grid`, `collection_control_card_with_multiselect_toolbar`, `collection_control_grid_table`, or `Event Pipeline Grid-Table`. Earlier support tables may describe display needs, but the canonical record-display/template-selection table is the generation contract. The App Plan generator must write registry-matching rationale at generation time for every selected Collection presentation; do not emit a template ID-only row that later requires manual editing. Use business signals from the selected reference, such as dense row/column scanning, work queue, record list, card browsing, responsive card browsing, multiselect/bulk operation, or primary operations table.

When using the standalone materializer, parse App Plan Data List fields, Custom Data List Forms, Dashboard Filters, and Record Display Control Selection as generation inputs, not documentation-only text. Generated data lists must include the planned business fields with YAPK-canonical storage names by `FieldIndex`, runtime-visible `Rules.choices` for choice fields, and default view `layout[]` / `query[]` entries. Data List field-table parsing must accept both canonical headings such as `Field label | Field name | Field type` and legacy generated-plan headings such as `Display Name | Storage Name | Control`; `Storage Name` is an alias for the internal field key, and `Control` may infer the Yeeflow field type when `Field type` is absent. If a recognizable business field table cannot be parsed, fail planning/materialization validation instead of silently emitting only the native `Title` field. Generated custom forms must attach to the App Plan host list. Generated Dashboard filters must be real export-shaped controls inside approved v1.1 business-content slots, but page-level `select-filter` variables must not be consumed by Collection `attrs.data.filter[]` conditions with `op`/`operator = "9"` until an export-proven empty-value bypass contract exists. Prefer proven `search-filter` / `attrs.data.fulltext[]` linkage or visible unfiltered Collections, because runtime evidence showed empty select-filter variables can clear all rows on initial load. Generated grid-table Collections must replace `{{DetailLayoutID}}` with a concrete Type `1` layout from the same source list and must preserve wrapper gap contracts on the Collection parent container.

App Plan `Choice Values` cells are ordered enums, not one runtime label. Use the shared choice-field option parser for full-app and standalone generation: split comma, semicolon, Chinese comma, ideographic comma, and line-break lists into one `Rules.choices[]` entry per value; remove planning annotations such as `Planning Default` / `规划默认`; and keep `Rules.color_choices` synchronized by value and order. A choice default belongs in `Default Value`, never in an option-label annotation. Treat `CHOICE_OPTION_CONTAINS_MULTIPLE_VALUES`, `CHOICE_OPTION_PLANNING_ANNOTATION_PRESENT`, `CHOICE_OPTION_DUPLICATE`, `CHOICE_COLOR_OPTIONS_MISSING`, and `CHOICE_COLOR_OPTIONS_MISMATCH` as signing blockers. See `docs/standards/choice-field-option-materialization-standard.md`.

Post-install seed companion artifacts must model lookup fields as target-record references, not display strings. For every `Type: "lookup"` field, emit a structured seed value with `seedValueType: "lookup"`, `requiresLookupListDataIDResolution: true`, `storedValueField: "ListDataID"`, target list metadata, and display-field metadata. The live seed writer must create/read the target list rows first, then write the target row `ListDataID` into dependent lookup fields, and read back that the referenced target record exists. Do not write lookup display titles, numbers, or labels as seed values.
...
For every standalone or packaged Dashboard, materialize `tempVars[]` only from retained page-layout/component dependencies and actual KPI/query/action consumers. Require complete scoped `id/name/type/idx` declarations, rewrite `__temp_` references with the same namespace, and prune unused template variables after final cleanup. Name-only declarations render blank in Designer and are signing blockers.

Standalone Dashboard `.ydp` handoff must use `node scripts/build-ydp-wrapper.js <dashboard-build-result.json> <output.ydp> --plan <standalone-dashboard-plan.md> --trace <standalone-dashboard-plan.trace.json> --application-plan <core-application-plan.md>`. The first plan belongs only to the standalone plan/trace validator; the separate Core application plan must match `sourceBinding.applicationPlanSha256` and drives Dashboard conformance gates. Do not pass a bare body or manually construct the outer. Generated-final output requires Core v1.1's export-proven 11-field profile; the six-field fixture-minimum profile is compatibility-read only. The shared full-YAPK Dashboard builder result must be `wrapper-ready` and carry issued dependency/identity closure plus complete Core source-binding hashes. The wrapper must pass `validate-ydp.js` before write and after byte round-trip, prepare both output files before commit, and return both `.ydp` and its validation report. Static validation does not prove import, Designer rendering, or runtime behavior.

For Dashboard Summary/KPI count metrics, `ListDataID` is a Yeeflow system record field used for safe record-count aggregation. It does not need to appear as a business field row in `Childs[].Fields[]`, but the Summary control must still carry complete designer-shaped `ListDataID` metadata and must be registered in the dashboard resource `ReportIds[]` and `exts[]`. Do not keep page-level `filterVars[]` copied from template dependencies unless a generated filter or consumer actually uses that variable.

Generated custom Data List forms must use Data List Form Layouts v1.1 as the page-level source of truth. New Item and Edit Item custom forms must clone `data_list_form_layout_new_edit_v1_1` from `docs/reference/data-list-form-layout-new-edit.template.json`; Standard View Item custom forms must clone `data_list_form_layout_view_item_v1_1` from `docs/reference/data-list-form-layout-view-item.template.json`. Full-page Workbench View Item custom forms must clone `data_list_form_layout_workbench` from `docs/reference/data-list-form-layout-workbench.template.json`, must be assigned only to `ListModel.LayoutView.view`, and must open as Full page. Preserve the selected template root shell, page background, root full-width/zero-padding contract, `main > content`, section card structure, widths, spacing, typography, and locked non-business regions. Business content may be inserted or changed only inside `page_title_content`, `Operations`, `section_content_area`, `section_title_header`, and `kpi_card_wrapper`. Remove unused copied `content_card_wrapper`, `content_card_60_wrapper`, `content_card_40_wrapper`, `1_columns_section`, `2_columns_section`, `3_columns_section`, and `2_columns_60/40_section` modules after materializing the business content. Do not leave title-only sections, empty `section_content_area` containers, unused Operations containers, or stale source-template labels such as `Active Loan Pipeline`. New/Edit forms are for current-record editing only and must not include related Collections, Data Analytics, KPI regions, or Dashboard page shells. View Item forms may include current-record details plus related data, approved Dashboard Collection templates, Data Analytics templates, and KPI cards, but only inside approved business slots. Every generated custom form must carry `dataListFormLayoutTemplateId` or `derivedFromDataListFormLayoutTemplate`, and signing readiness must run `scripts/validate-data-list-form-layout-template.mjs --package <package.yapk> --plan <yeeflow-app-plan.md>`.

Reverse-related Collection sections are the standard way to show child rows on a parent Data List View Item form when the child list has a lookup to the parent. During App Plan parsing and materialization, build an incoming lookup index by target list and require a `Reverse-Related Collection Selection` table row whenever the plan expects parent-detail child-record display. For each approved reverse-related row, clone an approved grid-table Collection into the parent View Item or Workbench View Item form, source it to the child list, filter `childLookupField == current host ListDataID`, add search only when the `search-filter.binding` is consumed by Collection `attrs.data.fulltext[]`, and add child-create actions only when the Add `action_button` targets the child list and `attrs.passvalues[]` defaults the child lookup field to current host `ListDataID`. Do not generate this pattern on New/Edit forms, do not write parent display values as lookup defaults, and do not leave a read-only Collection when the App Plan requested Add/search behavior. Before signing readiness, run `scripts/validate-data-list-form-layout-template.mjs --package <generated-final.yapk> --plan <yeeflow-app-plan.md>` so planned reverse-related rows fail if they were not materialized in the generated package.

Every generated business Data List or Document Library must have explicit App Plan rows for custom New/Edit and View forms. New/Edit rows must select `data_list_form_layout_new_edit_v1_1`; View rows must select `data_list_form_layout_view_item_v1_1` or `data_list_form_layout_workbench`; Workbench View rows must explicitly state Full page opening. Do not rely on Yeeflow default New/Edit/View layouts, and do not treat a single showcase custom form as coverage for other generated business lists.

Generated native Data table controls must use the approved Data table golden reference family. Select `data_table_control_standard_scroll`, `data_table_control_standard_no_scroll`, or `data_table_control_caption_scroll` from `docs/reference/data-table-golden-references.json` in the App Plan, clone the selected export-shaped `data-list` control from `docs/reference/`, preserve locked style/layout attrs, and remap only source list, display columns, and allowed caption text. Data table templates may appear on Dashboard pages, Custom Data List forms, Approval submission/task/print pages, Data List workflow task forms, and Schedule workflow task forms. They are for lightweight tabular display only; do not use them as substitutes for approved Collection/card/multiselect/Kanban/Timeline templates. Generated controls must carry both `templateId` and `dataTableTemplateId`, must have non-empty `attrs.listarr` with real `Field` and `FieldName`, and must pass `scripts/validate-data-table-golden-references.mjs --package <package.yapk> --plan <yeeflow-app-plan.md>` before signing readiness.

For every generated Type `1` custom Data List form, write the complete rendered form JSON to both `Layouts[].LayoutView` and `Layouts[].LayoutInResources[0].Resource`. The two JSON surfaces must be equivalent after stable normalization. `LayoutView` is the runtime/Designer loading source; a placeholder such as `source: "minimal-resource-graph"` is invalid even when `LayoutInResources[0].Resource` contains the full form.

Generated Approval form pages and workflow task forms must use Approval Form Layouts v1.1 as the page-level source of truth. The single Approval form submission page must clone `approval_form_layout_submission_v1_1` from `docs/reference/approval-form-layout-submission.template.json`; every Approval form task page, Data list workflow task form, and Schedule workflow task form must clone `approval_form_layout_task_v1_1` from `docs/reference/approval-form-layout-task.template.json`. Preserve the selected template root shell, page background, root full-width/zero-padding contract, hidden form-title CSS, `main > content`, `content` custom width 1280, section card structure, widths, spacing, typography, and the locked `action_panel_flow_history_wrapper` containing Action panel plus workflow history controls. Business content may be inserted or changed only inside `page_title_content`, `Operations`, `section_content_area`, and `section_title_header`. Remove unused copied section/card modules after materializing business content; do not leave empty `section_content_area` containers, title-only cards, or unused Operations containers. Approval forms and workflow task forms must not include Data Analytics templates, charts, pivot tables, Summary/KPI analytics, or `kpi_metrics_wrapper`. Every generated task page `formdef` must carry `approvalFormLayoutTemplateId` or `derivedFromApprovalFormLayoutTemplate`, and signing readiness must run `scripts/validate-approval-form-layout-template.mjs --package <package.yapk> --plan <yeeflow-app-plan.md>`.
...
Approval form `Submission Form Fields` and `Task Form Fields` tables in the App Plan are generation inputs. The materializer must inject those planned fields into `Forms[].DefResource.pageurls[].formdef` inside the selected Approval Form Field Layout wrapper; a shell-only Approval form with titles, workflow skeleton, and empty business content is not signing eligible. Do not cap, truncate, or silently drop planned Approval form fields because a table is long; every row must materialize unless the App Plan explicitly marks that field unsupported, deferred, or omitted for that exact form page. Planned visible business labels are canonical and must survive materialization exactly at business-label level: a technical binding such as `LoanNumber` is not enough when the displayed label drifted away from the App Plan's `Loan Number`. Source-template cleanup must not rewrite materialized planned field labels, placeholders, titles, or display metadata. Task forms must include every Submission form business field as readonly review context unless the Functional Specification or App Plan explicitly excludes that field from that task. Task forms may add task-only fields only when the Functional Specification or App Plan names those fields and explains the task purpose. Do not leave unrelated source-template labels such as loan/pipeline field captions in a non-loan Approval form after materialization. Task field controls must be explicitly readonly unless the App Plan states that assignees edit them.

Approval workflow publish-readiness is a generated-final hard gate. Every generated Approval form must emit a Designer-publishable `DefResource` with `flowPage`, `variables.basic/listref/filter`, submission and task `pageurls[]`, Start node task URL aliases, `MultiAssignmentTask` task URL aliases, array-shaped `usertaskassignment`, approve metadata, approved and rejected paths, an `EndRejectEvent` rejected path, complete graph refs, and non-overlapping node positions. Applicant line-manager assignments must use the export-proven `ApplicantUserID` + `LineManager` expression button shape. Approved and Rejected SequenceFlow conditions must use Designer-readable `conditioninfo[]` rows with `left` referencing the current task Outcome, `op = "s.="`, and `right` as the matching `Task outcome:Approved` or `Task outcome:Rejected` button; simplified `{label,value}` outcome objects are not publish-ready. Run `scripts/validate-approval-workflow-publish-readiness.mjs --package <package.yapk>` before signing readiness. Do not treat signing, upgrade submission, Version Management updates, or API acceptance as proof that the live Designer `ProcModel` / `DefBlob` is publish-ready. For upgrades that change Approval workflows, final proof requires a live Designer/DefBlob summary after Version Management `Succeed` that matches the package-side task, assignee, Approved condition, and Rejected condition summary, plus Designer open and publish-success evidence.

Approval form generation must also satisfy the current standalone `.ywf` designer-shape contract before full-app packaging. Materialized Approval form controls need plain string `binding` values, current `datepicker` date controls, pair-shaped numeric widths such as `width: [null, 960]` with `widthu`, and the standard `Main > Content > Form body / Form bottom` structure with workflow controls inside `Form bottom`. Outcome transition conditions must be full Designer-readable `conditioninfo[]` rows with `left` / `op` / `right`; do not rely on labels such as `Approved` or `Rejected` as the condition shape. Job Position assignee IDs must come from live/proven mappings or remain a blocking mapping item; never invent IDs to make the package look complete. Run `validate-ywf-def.js --mode final` for standalone approval exports and include the focused `scripts/test-approval-ywf-form-structure-gates.mjs` regression when changing Approval form materialization.
...
Dashboard pages must start from the full selected Dashboard Page Layout shell before inserting Collection, filter, KPI, analytics, or golden-reference components. Do not generate a page that only contains the selected component template without the selected layout's shell, section structure, structural containers, and canonical background/padding. For `dashboard-page-layouts-workbench`, preserve `main_work_queue_wrapper`, `primary_working_area`, and optional `right_side_panel`; group multiple analytics in `chart_cards_section` with no more than three analytics templates per section. For `dashboard-page-layouts-two-panel-workspace` and `dashboard-page-layouts-three-panel-workspace`, preserve the master-detail list/detail shell, `vCurrentItemID`, left-panel selection action, selected-record detail Collection limit/filter, and editable empty-selection state. Card-style Collection templates are not grid-table templates; generate and validate them with their own export-shaped template contract instead of adding grid-table header/item requirements. If an App Plan declares Dashboard Summary/KPI metrics or filters, the generated dashboard must include real Summary/KPI and Data Filter/search/select/radio controls plus downstream consumers; a Collection-only page is incomplete even when the Collection template passes. Select filters must preserve `attrs.lab.value`, `attrs.lab.ty = [null, "xs-light"]`, `attrs.lablay = [null, "top"]`, field/display/value bindings, and a valid placeholder string or `{ "value": "..." }` object. Search-filter placeholders must use the same valid string or object shape. Never spread a placeholder string into numeric object keys.
...
For any generated Dashboard, custom Data List form, Approval submission page, Approval task page, Data list workflow task form, or Schedule workflow task form with two or more page-level Data Filter controls, clone `dashboard_standard_filter_group` and map the child filters inside it. The approved wrapper is the export-proven `flex_grid` from `docs/reference/data-filter-standard-filter-group.template.json` with 4 desktop/laptop columns, 2 tablet columns, 1 mobile column, one auto row, 16px row/column gaps, and `displayLabel = [null,false]`; simplified `columns.count/type/minmax` Grid settings are invalid. Do not scatter filters as loose siblings, do not use invented filter-row containers, and do not apply this grouping rule to Collection-local toolbar search boxes.
...
Service portal payloads are plan-gated: generate them only when Service Portal is explicitly planned in the App Plan. Use `yeeflow-service-portal-generator`, `docs/standards/service-portal-generation-standard.md`, and `docs/reference/service-portal-yapk-example.normalized.json` as the export-proven source of truth. A generated portal must materialize decoded top-level `PortalInfo`, `ListSet.Ext3.externalPortal`, portal user infrastructure, portal groups, `PortalInfo.Resources[]` for portal source/menus/listforms, and `PortalInfo.Perms[]` for group/audience resource access. Expose only current-app Data Lists (`Type = 1`), Document Libraries (`Type = 16`), and Dashboard pages (`Type = 103`); never expose Approval forms or Form reports through Service Portal. Portal menu items and groups need suitable FontAwesome icons, list form routing must resolve to same-list custom forms or `default`, and permission rows must target known portal resources and portal groups or the observed special audience `999999`. Run `scripts/inspect-service-portal-yapk.mjs --package <package.yapk> --json` for any package with `PortalInfo` before signing readiness, and report Service Portal proof as export-proven/validator-backed unless portal login, navigation, resource access, and permission behavior have focused runtime proof. App/navigation groups that must be runtime-persisted need API-issued long ids, not small local ids. Business policy variants such as leave balances should be modeled in policy data with submit validation/action metadata, not hardcoded as one universal rule. Run `scripts/validate-runtime-binding-lessons.mjs` alongside schema, plan-conformance, workflow, and UI validators.
...
Generated YAPK business sample/demo data must use a companion post-install seed artifact rather than decoded list rows. The only generated-final `Childs[].List.Items` exception is export-shaped structural root folders on Type `16` Document Libraries; every folder value must serialize as a string. Do not emit schema-forbidden `AppID` properties inside decoded `AppPackageInfo` objects such as `ListSet`, `Pages[]`, `Childs[].List`, `Childs[].Fields[]`, or `Childs[].Layouts[]`; keep wrapper `AppID = 41` and only use app references inside documented embedded dashboard control data where the runtime requires them. Root `ListSet.LayoutView` must be a JSON string when present; dashboard pages keep `LayoutView: null`; generated child `List.LayoutView` remains null unless export-proven custom form routing is complete. FieldName suffix and storage family mismatches are generated-final blockers.
...
Generated-final YAPK app generation must keep package structure, runtime seed data, and quality proof as separate stages. Never embed business seed/demo rows under `Childs[].List.Items`, `Childs[].ListDatas`, or `Childs[].List.ListDatas`; emit a companion seed artifact and require explicit post-install live seeding approval. The narrow exception is Type `16` `List.Items` structural root folders that satisfy the full export-proven folder contract and are planned as folders, not demo records. Companion seed artifacts are not installed automatically with `.yapk`. After Version Management reaches `Succeed`, first verify root and child-list materialization, then run a post-install seed writer that maps companion rows to live list/field identities, resolves identity-picker values to existing tenant users, skips or explicitly reports file/image fields that lack a real upload/reference flow, writes rows through live APIs, and reads them back. Runtime dashboard proof that depends on sample data is incomplete until this seed writer proof exists. Before signing, replace placeholder tenant metadata: wrapper `TenantID: "0"` is not signing-ready. API `Status: 0` from install/import means submitted only, not materialized. Do not claim final success until async materialization and browser runtime proof show the decoded root app opens without target-app `Install failed`, empty Components shell text, model-load errors, or empty planned dashboards after seed. If a fresh install returns an already-installed/title-collision response, stop fresh-install retry and either use a unique fresh E2E title or move to an explicit upgrade/cleanup flow with lineage proof.
...
Dashboard clone/remap hard gate: remap source action targets recursively before save, including Button/Container Add actions, Collection local `attrs.actions[]`, and item-template `attrs.control_action`. Add actions must bind the current target ListID and its New Type-1 layout. Collection opens must bind a current-list local `coll` Edit action, its Edit Type-1 layout, and `__ctx_coll` / `ListDataID`. Run `scripts/validate-dashboard-action-reference-closure.mjs` after saved component readback; `DASHBOARD_ACTION_LAYOUT_UNRESOLVED`, `DASHBOARD_COLLECTION_OPEN_ACTION_MISSING`, and `DASHBOARD_STALE_LAYOUT_REFERENCE` fail closed. API acceptance/readback does not prove action runtime: perform focused browser Add and record-open smoke before describing controls as usable.
...
Correct-project v2 runtime note: `/Users/Renger/Downloads/collection-kanban-actions-runtime-proof.v2.yap` was generated by `tools/generators/generate-collection-kanban-actions-runtime-proof.mjs` from the clean `formreport-clean` repo and user-tested successfully for import, dashboard open, edit/delete/update, selection toggle/count, bulk toolbar, bulk mark completed, and bulk delete.
...
Form Actions Phase 1 update: app-level packages may include native form actions only when using export-backed patterns. Keep form actions distinct from workflow graph actions. Use form actions for page initialization, temp UI state, Set variable defaults, and confirmation dialogs. Wire action buttons with `attrs.control_action`, page load with `formdef.formAction.onLoad`, and validate that every referenced action exists in `formdef.actions[]`. V1.7 makes `listitem`, `openform`, and `opendashboard` an export-proven, validator-backed generated baseline across Approval Submission/Task, Data List/Document Library custom forms, and Dashboard; runtime opening remains a separate proof.
...
For every App Plan Query Data Form Action row, select one explicit mode from `docs/standards/form-action-query-data-golden-reference-standard.md` and use `scripts/lib/form-action-query-data-utils.cjs`. Resolve source resources to current generated/API-issued IDs; declare every workflow/temp/Sub list/count target before serialization; and run final YWF/YAPK validation. Multiple count-only must not carry hidden field maps or result collection targets. Do not materialize Query Data on Public Forms.

Custom Data List Form Query Data must preserve the v1.2 host contracts: `formAction.onLoad` for page load, `control_event_rule` for field change, `____customListFields_<FieldName>` for current-record field targets, and `__list_` for a current-record Sub list. Query-to-Sub-list is not a generic reverse-related display pattern; select it only for editable working-copy/line-item scenarios and prefer New/Edit forms. Run the custom-form Query Data generated-package hard gate before signing.
...
Workflow Condition editor variable-condition update: for Approval form workflows, Data list workflows, and Scheduled workflows, Workflow-variable-left conditions must use the same editor contract. `left.type = 1` selects a workflow variable token, `right.type = 0` stores fixed literal values, `right.type = 2` stores Expression editor token arrays, and `group`/`op` must match the selected variable type: User/Department/Location/Cost Center/Lookup/Metadata/Dictionary/List/Text as `string` with `s.` operators; numeric values as `number` with `n.` operators; booleans as `boolean` with `b.` operators; date/time as `datetime` with `dt.=`, `dt.!=`, `dt.>=`, `dt.<=`, `dt.>`, `dt.<`, `isNull`, and `isNotNull`; File/Image/Signature as `general` null checks. Use `right.type = 2` only for dynamic/function/calculated values such as `currentUser`, `getUserAttr`, `dateAdd(now(), ...)`, cross-variable comparison, concatenation, or numeric calculation. Condition groups support at most two layers: a top-level wrapper row uses `left:null`, `op:"isNull"`, `right:null`, and child `conditions[]`; child rows use their own `pre` (`and`/`or`) and third-level groups are invalid. Assignment task outcome branches must use source-task Outcome expression-button HTML on the left and matching `Task outcome:*` expression-button HTML on the right; do not generate a generic `Outcome` variable token. Run `scripts/test-workflow-condition-editor-gates.mjs` when workflow condition generation changes.
...
Workflow Query Data v1.6 generation must distinguish four result modes: count-only, single-to-variables, multiple-to-List-variable, and multiple-to-text-variable. Use `scripts/lib/workflow-query-data-utils.mjs`; do not force every QueryData node into multiple text output. Count-only has no row target. List mode requires a List variable and ListRef/Complex Type mapping. Validate Loop list targets/bodyRef, count branch coverage, Lookup ListDataID identity, and query-derived User assignees. Literal sample email recipients are not reusable values.

V1.10 export-proves QueryData source types Data List (`1`), Document Library (`16`), and Form Report (`32`). Resolve each source to an included generated resource, preserve host-specific target namespaces, allow at most two sorts, and validate Page Number/Page Size. Form Report may be queried but does not host independent Form Actions. Data Report generation remains deferred.

V1.7 adds Data List Workflow count-and-upsert and all three Loop modes. Use `__list_` only for a Data List Sub List and `__variables_` for workflow List variables. Values/number loops serialize `loopValue.type = 2` plus an expression-token array and do not serialize `loopValue.prefix`. Every Loop `bodyRef` must resolve to a LoopBody; recursively generate and validate `LoopBody.children[]`. Repeated MailTask, ContentList, Custom Service, external calls, and Delay actions remain runtime-sensitive. New-item `FlowMappings.FieldName` may be null or a resolving `flowstatus` field, but never a normal business field.
...
For Approval workflows, calculate `graphposition` through the shared runtime-proven helper: `x=minNodeX+90`, `y=minNodeY+45`, and `width/height` equal the node-content span with the standard small-workflow minimum. Saved graphposition is not the browser DOM stage translation. Reject origin or content-span mismatch before standalone or packaged handoff.

Asia Tech and V1.7 trigger-condition learning: for data-list Add Item / new-item triggers, register the trigger through the host list `FlowMappings[]` row using `Setting = {"NewTrigger": true}`. `FieldName` may be null, or may resolve to an intentionally generated host-list `flowstatus` field. Keep `Data.Forms[].Settings = null` and `Data.Forms[].Deployed = true`. Do not bind `FlowMappings.FieldName` to a normal list field; doing so can make the frontend show an empty trigger condition and prevent the workflow designer from opening.
...
- Preserve the runtime-proven native metadata contract, not a normal Data List field template: `Title` uses `FieldIndex=0`, `Status=1`, `IsSystem=true`, `IsIndex=true`, and Rules `displayLabel + isLibrary`; `Bigint1` uses `Status=127` and `displayLabel + isNotInListFiles`; `Text1` uses `Status=119` and `displayLabel`; `Bigint2` and `Text2` use `Status=99` and `displayLabel + readonly`; `Text3` uses `Status=319` and `displayLabel + isNotInListFiles`; `Text4` uses `Status=57`, `FieldType="Text"`, `Type="file-upload"`, and Rules `displayLabel + required + isLabrary + PROP_MAXSIZE=2147483648`. Support fields other than `Title` must use `IsSystem=false` and `IsIndex=false`. Do not add legacy `Text4.Rules.isLibrary` or invent an Attachment schema.
- Run `scripts/validate-document-library-native-field-runtime-metadata.mjs --package <generated-final.yapk>` before signing. `yapk-first-generation-preflight.mjs` must expose the same `document-library-native-field-runtime-metadata` hard gate. A Type `16` resource that merely has the right field names, navigation, and folder rows is not signing-ready when this runtime metadata gate fails.
...
- Root-level generated folder rows are runtime-proven for `.yap` in `ListDatas`. The `Projects Center_1-v1.7 (1).yapk` export proves generated-final `.yapk` uses `Childs[].List.Items`: the folder ID is the object key, the row omits `ListDataID`, and values are `Title`, `Bigint1 = "0"`, `Text1 = "folder"`, `Bigint2 = ""`, `Text2 = ""`, `Text3 = "0_<lowercase folder title>"`, with `Text4` omitted and custom fields blank. Folder object-key IDs must be API-issued and represented in provenance through `decoded.Childs[index].List.Items[index].$key`.
- Parse App Plan root-folder tables into structured folder records and materialize every non-deferred root folder. Do not materialize nested desired/post-import rows until nested folders have focused runtime proof.
- Keep normal Data List `List.Items` sample rows forbidden. The generated-final exception applies only to Type `16` rows that satisfy the complete structural folder contract; malformed or file-bearing rows hard fail.
...
Workflow Designer v2 graph style update: generated Approval form workflows, Data list workflows, and Scheduled workflows must use root `lineType = "rounded"` and `graphver = 2`. Preserve/default `graphzoom` but do not treat zoom differences as defects. Every generated workflow action node must have a concise business-specific Action name; defaults such as `Assignment Task`, `Content List`, `Inclusive Gateway`, `Gateway`, or `Task` are invalid. Every generated `SequenceFlow` must include `properties.linetype = "rounded"`, a non-empty concise business `properties.documentation` Description, and `dockers = []` for designer auto-routing. Workflow diagrams are node-placement first and connector-routing second: use the golden-reference motifs (main approval backbone, local gateway fan-out, upper/lower branch lanes, lower action lanes, and local rejection endpoints) before adding any vertices. Keep layout vertices only for return/backward paths, overlap avoidance, node crossing, and long reroutes; do not emit old orthogonal line style or full-docker routes, and do not use many connector vertices to compensate for poor node placement. Direct same-row `Start -> first task` Submitted connectors and adjacent forward connectors must not carry cosmetic vertices. Cross-row return routes should use the net row-gap midpoint formula for adjacent rows; multi-row return/backward routes must choose an open adjacent row-gap midpoint and must not run their horizontal segment through an intermediate workflow row. Long connector vertical segments must choose an open adjacent column-gap midpoint and must not pass through intermediate node columns. Treat medium workflow width as an advisory readability signal, not a reason to compress standard column spacing; prefer folding into readable rows, lane offsets for repeated vertical routes, and long-label separation over forcing the graph under 2600px. Use external return lanes only for long/crowded reroutes. Fail vertex-overuse and excessive-detour layouts because they indicate the nodes should be repositioned.

Workflow layout motif fidelity: generated workflows should resemble the export-proven `Workflow actions layout.json` patterns. Inclusive Gateway branch targets must stay local to the gateway and use upper/lower lanes; multi-source normal End nodes must be local merge points beside the incoming source group; local rejected connectors to a nearby `End with rejection` should have empty/absent `vertices[]`; and visible business-condition connector labels should be concise, with full logic kept in `conditioninfo`.
...
Use `docs/studies/data-list-custom-form-fields.md`, `docs/studies/data-list-layoutview-add-form-runtime-fix.md`, `docs/studies/normalized/data-list-custom-forms/`, `docs/standards/data-list-custom-form-layoutview-runtime-source-standard.md`, and `scripts/inspect-data-list-custom-forms.mjs` when an app needs generated Data List custom forms. Generate custom forms as Data List `Layouts[]` entries with `Type = 1`, `LayoutInResources[0].ID = RefId = LayoutID`, and the complete rendered form JSON duplicated into both `LayoutView` and `LayoutInResources[0].Resource`. Yeeflow runtime and Designer read `LayoutView`; leaving `LayoutView` null, blank, or as a minimal placeholder can import but open as a blank/loading custom form. Assign forms through `ListModel.LayoutView.add/edit/view`; the export-proven patterns include separate add/edit forms and one form reused for add/edit/view.
...
Adding a Public Form must not select a reduced or hand-built application path. The full-app materializer must first generate the host Type `1` Data List with fields, views, standard Data List Form Layouts v1.1 New/Edit/View forms, Data List Form Fields Grid v1.1, resolved `List.LayoutView.add/edit/view`, workflows, navigation, permissions, and normal generated-final gates; it then attaches planned Public Forms under `PublicForms[]`. The same rule applies when no Public Form is planned. Packages with `PublicForms[]` but no resolved internal custom forms must fail `validate-yapk-package.js`, the Data List form layout gate, and first-generation preflight.
...
For Tab or Gantt requirements in a generated application, dispatch through `docs/reference/tab-gantt-control-golden-references.json`. `tab_control_workspace` is a reusable `aktabs` structure; `gantt_control_linked_activity` is a parent-record form pattern with a complete resolved Activity source identity (`AppID`, `ListSetID`, and `ListID`), current-parent filter, Add default propagation, and detail layout link. Do not use source app IDs, labels, Custom Code, document controls, or raw resources as defaults.

Generated-final checks are fail-closed: Tabs need valid `ak-tabs-tab` children and exactly one default. Gantt source identity requires `attrs.data.list.AppID`, `ListSetID`, and `ListID`; Start/End require `Datetime`; Dependency requires a same-source multi Lookup; Progress requires Decimal/percent in the 0..1 range; optional Parent requires a same-source single Lookup. Current-parent filter and Add `passvalues` must use the same child lookup, and `linkLayout` must resolve to a concrete Type 1 detail/View form. Run `node scripts/validate-tab-gantt-golden-references.mjs <decoded-app-or-fixture.json> --strict` before packaging and retain the runtime proof boundary in `docs/standards/tab-gantt-control-golden-reference-standard.md`.

For `collection_control_grid_table`, use `docs/reference/collection-control-grid-table.template.json` as the export-shaped source of truth. Copy `grid_table_col_wrapper` and all descendants, not a simplified header-plus-Collection lookalike. Business mapping may edit only `grid_table_col_title_wrapper`, `op_normal`, `grid_table_col_header`, `grid_col_item`, and optional `grid_table_col_item_operations`. Preserve style/layout/typography outside those regions, keep `grid_table_col_header` and `grid_col_item` column count/width/property parity, and after any schema cleanup or business column pruning recompute both grids' `attrs.columns["1"].list` so the desktop track count exactly equals each grid's actual `children.length`; do not keep the source template's six tracks when only three mapped columns remain. Map field types to Dynamic user/image/file/field controls, and omit item operation menus for Form Report/Data Report display-only sources. If item operations are present, every button must bind to a valid action; edit/delete require matching Collection actions, and delete requires the confirmation temp variable/conditional delete flow.

Dashboard Query Data uses the v1.3/v1.4 temp-only contract and its own generated-final gate. Across Approval, Data List, Document Library, and Dashboard hosts, omitted Page Size means 100, explicit Page Size is 1..1000, omitted Page Number means 1, and non-default Page Number uses `querydata_pageindex`. Preserve chained Dashboard query order and guard temp-dependent steps. Temp JSON is not a Collection/Data Table source; use `JSONStringfy` or a justified Custom Code renderer. Document Library custom forms share Data List Form Action contracts. Form Report/Data Report cannot host independent Form Actions.

For Form Action Set Data List, use one shared App Plan table, builder, materializer, and signing gate across Approval Submission/Task, Data List/Document Library custom forms, and Dashboard. Print Page remains focused-export-proof-required and Public Form is forbidden. Distinguish page `setdatalist` from Workflow `ContentList`: only Workflow Set Data List may expand Sub List rows or multiple files into multiple records. Selected edit/remove requires filters; add/edit requires field mappings; generated operations and target modes are explicit. Data List View Item may edit the current record, while New/Edit normally uses Set Variable plus Submit Form. Document Library Add requires single-file Upload File `Text4`; `_Path` is the export-proven folder pseudo-field. Dashboard outputs are temp-only. Use `docs/standards/form-action-set-data-list-golden-reference-standard.md`.

For Form Action Open Resource, use one shared App Plan table, builder, materializer, and signing gate across the same hosts. `listitem` targets Data List/Document Library and permits Current item only on their custom forms; selected Edit/View requires an item ID and optional custom form must belong to the target. `openform/new` may set target variables and query parameters; `openform/submitted` requires Form ID and forbids both. `opendashboard` resolves Type 103. Use `slide`, `modal`, `target`, or `new`; modal sizes are 0/1/2/3/9 and custom 9 requires width. Public Forms are forbidden. Follow `docs/standards/form-action-open-resource-golden-reference-standard.md`.
