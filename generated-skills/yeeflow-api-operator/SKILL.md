...
- Use the bundled scoped MCP route before local REST helper scripts for capabilities exposed by both surfaces: `yeeflow_app_builder_mcp` for application building, `yeeflow_operations_mcp` for operations, `yeeflow_admin_mcp` for administration, and `yeeflow_service_portal_mcp` for service-portal work.
...
- The Plugin declares scoped endpoints at `https://api.yeeflow.com/v1/mcp/app-builder`, `/operations`, `/admin`, and `/service-portal`. Authentication is negotiated by Codex through each server's OAuth flow; the Plugin must not embed or request bearer tokens, OAuth codes, API keys, cookies, Authorization headers, client secrets, or passwords.
...
- For live Document Library creation, do not call `appbuilder_component_save` with ad hoc Type `16` JSON. Use the Data List Generator's shared live Document Library materializer and two-phase merge/readback workflow. The first save contains only the validated seven-field native baseline; the second save adds validated custom fields and forms after persisted baseline readback. Reject duplicate List/Field/Layout IDs locally before either save, use `deleteMissing = false`, and validate the final MCP readback. A generic server `system error` is not proof that Document creation is unsupported.
- Do not register or call a duplicate standalone Yeeflow MCP route when the Plugin-bundled scoped route is available. If duplicate tool namespaces are visible, use the matching Plugin-bundled server for the requested product area and report the configuration conflict before any write.
...
YAPK signing is also a separate proof gate. When a generated package is being prepared for upload-ready handoff and credentials are available, use only `node scripts/yeeflow-yapk-sign.mjs --package <unsigned.yapk> --output <signed.yapk> --execute`; do not write task-local signing code. The helper owns the documented `POST /utils/apppackage/setsign` and `POST /utils/apppackage/verifysign` calls, accepts only an exact 32-byte base64 signature from supported response shapes, preserves the unsigned source, writes a separate signed output only after verification succeeds, accepts successful empty-body verification, and emits redacted diagnostics only. Never guess signing paths or print raw responses, `Resource`, `Sign`, tokens, tenant URLs, or workspace IDs. Use plugin defaults for the API base, prefer OAuth over legacy API key fallback, and resolve a target workspace only when install/import/upgrade automation is explicitly approved.
