...
Current release `0.8.105` releases Dashboard master-detail workspace action and filter binding hardening: two-panel and three-panel workspace Dashboards preserve page-layout actions/tempVars/filterVars, remove unresolved visual-only operation buttons, bind left-panel filters by business field semantics, and block mismatched Priority/Status filter bindings before signing readiness.
...
```sh
node scripts/inspect-codex-plugin-cache-metadata.mjs --root . --expect-version 0.8.105
```
